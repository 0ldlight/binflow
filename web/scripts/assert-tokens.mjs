...
//     ——token 定义层是唯一色值字面量豁免层（现 design-system/tokens/）。
...
//       色板唯一来源 = token 定义层 + Tailwind 语义类，闸语义只升不降）。
...
//     新栈 token 层整层加入豁免清单；层外新栈 tsx 仍受腿 2/3 约束
//     （色板走 Tailwind 语义类，不写 var(--bf-色系)，不写字面量）。
//     闸语义不降：豁免仅限 token 定义层目录，组件/页面 css 零放宽；
//     FE-P4 起 TSX 面零文件级豁免（MuiProvider 退役）。
//   UI Phase 1 批 1（design-system-plan §3.0/§6）：token 层迁
//     src/design-system/tokens/（六族分文件，index.css 聚合）——豁免
//     谓词随迁（旧 tw 路径 → design-system/tokens/）；同批删旧双 token
//     层（G2 清偿），任意层级 tokens.css 文件名豁免随之退役；桥接层
//     design-system/tailwind.css 零字面量，不豁免。
...
// 新栈 token 定义层（design-system/tokens/）：六族分文件，色值字面量
// 唯一合法层（frontend-rewrite P1 立位，批 1 迁径）
let newTokenLayerCount = 0

// 递归收集 src 下全部 css；design-system/tokens/ = token 定义层，豁免
function collectCss(dir) {
  ...
  for (const f of readdirSync(dir).sort()) {
    ...
    if (statSync(p).isDirectory()) found.push(...collectCss(p))
      ...
      if (relative(srcDir, p).replaceAll('\\', '/').startsWith('design-system/tokens/')) {
        newTokenLayerCount++
        continue
      }
      found.push(p)
...
console.log(
  `assert-tokens: OK — css ${cssFiles.length} 个（零硬编码色值）+ tsx ${tsxFiles.length} 个（零色值字面量、零 --bf-* 色彩 token 引用——FE-P4 起零文件级豁免）+ token 定义层 ${newTokenLayerCount} 个豁免（design-system/tokens/）`,
)
