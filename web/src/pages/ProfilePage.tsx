...
function GenerateTokenDialog({
  ...
  return (
    <Dialog open onClose={onClose} maxWidth="sm" fullWidth data-testid="profile-token-dialog" aria-labelledby="profile-token-dialog-title">
      {mint.phase === 'done' ? (
        <>
          <DialogTitle id="profile-token-dialog-title">Identity Token 已生成（仅此一次展示）</DialogTitle>
          <DialogContent dividers>
            ...
            >
              <Typography variant="body2" sx={{ mb: 1 }}>
                以 <span className="mono" lang="en">{username}</span> 身份签发（token_id{' '}
              ...
              <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mb: 1 }}>
                ...
                <CopyButton value={curlSample} label="curl 即用样例" />
              </Box>
              <Typography variant="caption" color="text.secondary">
                关闭本面板后不可再查看（服务端只存指纹）。CI 与脚本请使用此令牌，不要用控制台口令——
                docker login / curl -u / settings.xml / .pypirc 的口令位都填它；吊销联系管理员（按 token_id）。
              </Typography>
            </Paper>
          </DialogContent>
          <DialogActions>
            <Box sx={{ flexGrow: 1 }} />
            <Button variant="contained" data-testid="profile-token-done" onClick={onClose}>
              我已保存，关闭
            </Button>
          </DialogActions>
        </>
        ...
        <>
          <DialogTitle id="profile-token-dialog-title">需要二次口令（step-up）</DialogTitle>
          <DialogContent dividers>
            <Alert severity="warning" data-testid="profile-token-stepup" sx={{ mb: 1 }}>
          ...
          <DialogActions>
            <Button onClick={onClose} data-testid="profile-token-cancel">
              取消
            </Button>
            <Button
              variant="contained"
            ...
            >
              {mint.submitting ? '验证中…' : '验证并生成'}
            </Button>
          </DialogActions>
        </>
      ) : mint.phase === 'oidc-stepup' ? (
        <>
          <DialogTitle id="profile-token-dialog-title">需要重新认证（step-up）</DialogTitle>
          <DialogContent dividers>
        ...
        <>
          <DialogTitle id="profile-token-dialog-title">生成 Identity Token</DialogTitle>
          <DialogContent dividers sx={{ display: 'grid', gap: 2, pt: 1 }}>
            <TextField
              ...
              helperText={
                adminWrite
                  ? '「永不过期」仅管理员可选；非 admin 签发为有限期且受 auth.token_nonadmin_max_ttl 上限约束'
                  : '自助签发为有限期（Q11 护栏：≤ auth.token_nonadmin_max_ttl，默认 365 天）'
              }
              sx={monoInputSx}
              slotProps={{
                select: {
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography variant="caption" color="text.secondary">
                scope 固定（只读说明）：令牌携带本人全部权限——scope 参数仅经校验、不收窄权限域
              </Typography>
            </Box>
            <Divider />
            <Typography variant="caption" color="text.secondary">
            ...
            {mint.phase === 'error' && (
              <Alert severity="error" role="alert" data-testid="profile-token-error">
...
function IdentityTokenSection() {
  ...
  return (
    <Paper component="section" className="card section" elevation={1} data-testid="profile-token">
      ...
      <p className="text-2">
        CI 与脚本请使用 Identity Token——本页直接为自己签发（B-1.8：不再指到管理面）。
        明文仅生成时展示一次，关闭后不可再取（服务端只存指纹）；实例开启 step-up 时非 admin
        需二次口令。吊销与代人签发属管理面（
        <MuiLink
      ...
      <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
        <Button variant="contained" data-testid="profile-token-generate" onClick={() => setDialogOpen(true)}>
          生成 Identity Token
        </Button>
        <Typography variant="caption" color="text.secondary">
          默认 24 小时；管理面台账与按 token_id 吊销见 Access Tokens 页
        </Typography>
      </Stack>
      {dialogOpen && (
        <GenerateTokenDialog
...
function SshKeysSection() {
  return (
    <Paper component="section" className="card section" elevation={1} data-testid="profile-ssh">
      ...
      <Alert severity="info" data-testid="profile-ssh-gap">
        后端尚无 SSH 公钥端点（console-ux §9-R11 契约缺口）——如实缺位，不在此伪造增删入口；
        端点落地后本卡提供 Key 别名 / 公钥的登记与删除（对位 7.161 Profile 的 Add New SSH Key）。
      </Alert>
    </Paper>
  )
}
