  ...
  if (quota <= 0) {
    return (
      <div className="water-line">
        <span className="label">{t('已用')} <span className="mono">{formatBytes(used)}</span> {t('· 配额不限（0）')}        </span>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="water-line" style={{ marginTop: 6 }}>
        <span className="label">
          <span className="mono">{formatBytes(used)}</span> / <span className="mono">{formatBytes(quota)}</span>{t('（')}{pct.toFixed(1)}{t('%）')}{used >= quota ? t(' · 已满（写入将 413）') : pct >= 80 ? t(' · 接近上限') : ''}
        </span>
      </div>
    </div>
  )
...
function QuotaEditor({
  ...
  return (
    <div>
      ...
      <p className="field-hint">{t('正整数；0 = 不限。超限写入收到 413（message 含 used/quota）。')}</p>
    </div>
  )
}
    ...
    return (
      <div data-testid="repo-detail-page">
        <EmptyState
          ...
          action={
            <Button component={Link} to="/artifacts" variant="outlined" size="small">{t('← 前往制品浏览')}            </Button>
          }
        />
      </div>
    )
  ...
  return (
    <div data-testid="repo-detail-page">
      <p style={{ margin: '0 0 var(--bf-sp-2)' }}>
        <Link to={`/admin/repositories/${rclass}`}>{t('← 仓库')}</Link>
      </p>
        ...
        <div className="detail-head-actions">
          ...
          {rclass === 'local' && (packageType === 'generic' || packageType === 'maven') && (
            <Button
              ...
              disabled={readOnly}
              title={readOnly ? t('只读管理员不可写（服务端 403 兜底）') : t('部署到本仓（浏览器上传）')}
              onClick={() => setDeployOpen(true)}
            >{t('⬆ 部署 Deploy')}            </Button>
          )}
          <Button component={Link} variant="outlined" size="small" to={`/artifacts/${repo.key}`}>{t('浏览制品 →')}          </Button>
          {canEditConfig && (
            <Button
              component={Link}
              variant="contained"
      ...
      {mHolder && (
        <p className="page-note" data-testid="repo-manage-note">{t('ⓘ 当前会话以 manage 持有者身份管理此仓（permission target 授予）：配置可编辑；删除仓库仍是全局管理面写（服务端 403 兜底）。')}        </p>
      )}
      {readOnly && (
        <p className="page-note" data-testid="repo-detail-readonly-note">{t('ⓘ 只读管理员（readonly_admin）视角：仓库配置只读、浏览器部署（Deploy）已禁用——配置保存走单仓管理面写 （CanManageRepo write）、部署走制品写面，服务端一律 403 兜底。')}        </p>
      )}
        ...
        <div className="detail-grid">
          <div>
            <section className="card section" data-testid="repo-commands">
              <h3>{t('客户端接入（')}{repo.packageType}{t('）')}</h3>
              {commands.map((c) => (
              ...
              <p className="field-hint">{t('地址按当前访问 origin 生成（')}{window.location.origin}{t('）；命令与 docs/user 接入文档同源。')}              </p>
            </section>

            <section className="card section" data-testid="repo-usage-card">
              <h3>{t('统计')}</h3>
              {rclass === 'virtual' ? (
                <p className="text-2">{t('virtual 仓不持有自身内容——用量见各成员仓库。')}</p>
                ...
                <>
                  ...
                  {rclass === 'remote' && (
                    <p className="field-hint" style={{ marginTop: 8 }}>{t('已用 = 缓存内容逻辑字节；命中率等 remote 统计端点未开放（ux R9，显示 —）。')}                    </p>
                  )}
                  {rclass === 'local' && usage.data.quotaBytes === 0 && (
                    <p className="field-hint" style={{ marginTop: 8 }}>{t('配额 0 = 不限；在「配置」Tab 设置 quotaBytes 后此处显示水位条。')}                    </p>
                  )}
                </>
              ) : (
                <p className="text-2" title={usage.error?.message ?? ''}>{t('用量不可用（')}{usage.error ? `HTTP ${usage.error.status}` : '—'}{t('）')}                </p>
              )}
            </section>
            ...
            {rclass === 'remote' && (
              <section className="card section" data-testid="repo-remote-card">
                ...
                <div className="kv">
                  <span className="k">{t('用户名')}</span>
                  <span>{cfgStr(cfg, 'username') || t('—（匿名）')}</span>
                </div>
                <div className="kv">
                  <span className="k">{t('密码')}</span>
                  <span className="text-2">{t('不回显（NFR-S14）')}</span>
                </div>
                {cfgBool(cfg, 'allowPrivateUpstream') && (
                  <div className="warn-box" style={{ marginTop: 8 }}>{t('⚠ 已放行私网上游（allowPrivateUpstream）——SSRF 防线对该仓放宽。')}                  </div>
                )}
                <div className="kv">
                  <span className="k">{t('命中 / 未命中 TTL')}</span>
                  <span className="mono">
                ...
                <div className="kv">
                  <span className="k">{t('socket 超时 / 静默期')}</span>
                  <span className="mono">
            ...
            {rclass === 'virtual' && (
              <section className="card section" data-testid="repo-virtual-card">
                ...
                <div className="kv" style={{ marginTop: 8 }}>
                  <span className="k">{t('默认部署仓库')}</span>
                  <span className="mono" lang="en">
                    {cfgStr(cfg, 'defaultDeploymentRepo') || t('（未配置）')}
                  </span>
                </div>
                {!cfgStr(cfg, 'defaultDeploymentRepo') && (
                  <p className="field-hint">{t('未配置写路由：经此仓的部署 / 删除操作将返回 405。')}</p>
                )}
              </section>
            )}
          </div>
          ...
          {canDelete && (
            <aside>
              ...
              >
                <Typography variant="subtitle2" component="h3" color="error" sx={{ mb: 1 }}>{t('危险区')}                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>{t('删除仓库及其（可选）全部内容。制品不可变，此操作没有撤销。')}                </Typography>
                <Button
                  variant="outlined"
                  color="error"
                  size="small"
      ...
      {tab === 'config' && (
        <div>
          <section className="card section" data-testid="repo-governance-card">
            ...
            {rclass === 'local' ? (
              <div>
                ...
                <p className="field-hint">{t('exclude 优先于 include；仅 local 仓支持治理字段。')}</p>
              </div>
            ) : (
              <p className="text-2">{t('—（governance 字段仅 local 仓支持；')}{rclass} {t('的配置规范化会丢弃未知字段）')}              </p>
            )}
          </section>
          ...
          <section className="card section">
            <h3>{t('高级')}</h3>
            <div className="kv">
              <span className="k">{t('优先解析')}</span>
              <span>{cfgBool(cfg, 'priorityResolution') ? t('是（priorityResolution）') : t('否')}</span>
            </div>
            {rclass === 'remote' && (
              <>
                <div className="kv">
                  <span className="k">hardFail</span>
                  <span>{cfgBool(cfg, 'hardFail') ? t('是（上游故障直接失败）') : t('否')}</span>
                </div>
                <div className="kv">
                  <span className="k">{t('允许私网上游')}</span>
                  <span>{cfgBool(cfg, 'allowPrivateUpstream') ? t('是（SSRF 防线放宽）') : t('否')}</span>
                </div>
              </>
            )}
            ...
            {canEditConfig ? (
              <p className="field-hint">{t('字段级修改走')}{' '}
                <Link to={`/admin/repositories/${repo.key}/edit`}>{t('编辑器')}                </Link>{t('（全量替换语义——保存时整体重写 config）。')}              </p>
            ) : (
              <p className="field-hint">{t('只读呈现（readonly_admin）；编辑需全量 admin 或该仓 manage 持有者。')}</p>
            )}
          </section>
        </div>
      )}
      ...
      {tab === 'replications' && (
        ...
        <section className="card section" data-testid="repo-repl-card">
          ...
          {rclass !== 'local' ? (
            <>
              <p className="text-2" data-testid="repo-repl-na">{t('复制是 local 仓的 push 模型（源仓 → 目标实例仓，ADR-0021）——')}{rclass} {t('仓不适用。')}              </p>
              <Button component={Link} variant="outlined" size="small" to="/admin/governance/replication" data-testid="repo-repl-goto">{t('前往复制管理 →')}              </Button>
            </>
          ) : repl.status === 'loading' ? (
            <Skeleton lines={2} />
          ) : repl.status === 'forbidden' ? (
            <p className="text-2">{t('复制配置为全局管理面（GET /api/v1/replications 需 system:read）——当前会话无权查看。')}            </p>
            ...
            <>
              <p className="text-2">{t('本仓尚无复制配置。')}</p>
              {canEditConfig && (
                <Button
                  component={Link}
                  variant="outlined"
              ...
              <Table size="small" data-testid="repo-repl-table">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell component="th" scope="col">{t('目标（实例 / 仓）')}</TableCell>
          ...
          {rclass === 'local' && (
            <p className="field-hint" style={{ marginBottom: 0 }}>{t('推送状态与最近事件见')}{' '}
              <Link to="/admin/governance/replication" data-testid="repo-repl-goto">{t('全局复制页')}              </Link>{t('（10s 轮询）。')}            </p>
          )}
        </section>
      )}
