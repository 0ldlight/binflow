...
function tupleRows(props: Record<string, string[]>): { k: string; label: string; v: ReactNode }[] {
  ...
  return [
    {
      k: 'trash.time',
      label: '删除时间（trash.time）',
      v: (
        <>
          <span className="mono">{formatTrashTime(first('trash.time'))}</span>
          <span className="text-2" style={{ marginLeft: 8 }}>
            <span className="mono" lang="en">{first('trash.time') || '—'}</span>（epoch 毫秒）
          </span>
        </>
      ),
    },
    { k: 'trash.deletedBy', label: '删除者（trash.deletedBy）', v: <span className="mono" lang="en">{first('trash.deletedBy') || '—'}</span> },
    {
      k: 'trash.originalRepository',
      label: '原仓库（trash.originalRepository）',
      v: (
        <>
          <span className="mono" lang="en">{first('trash.originalRepository') || '—'}</span>
          {first('trash.originalRepository') && (
            <CopyButton value={first('trash.originalRepository')} label={`原仓库 ${first('trash.originalRepository')}`} />
          )}
        </>
      ),
    },
    { k: 'trash.originalRepositoryType', label: '原仓型（trash.originalRepositoryType）', v: <span className="mono" lang="en">{first('trash.originalRepositoryType') || '—'}</span> },
    {
      k: 'trash.originalPath',
      label: '原路径（trash.originalPath）',
      v: (
        <>
          <span className="mono" lang="en">{first('trash.originalPath') || '—'}</span>
          {first('trash.originalPath') && <CopyButton value={first('trash.originalPath')} label="原路径" />}
        </>
      ),
    },
  ]
...
export default function TrashPage() {
    ...
    const body: ReactNode = (
      <>
        <p>
          ...
          <span className="mono" lang="en">trash.*</span> 标记，原属性保留。
        </p>
        <div className="kv">
          <span className="k">条目</span>
          <span className="mono" lang="en">{TRASH_REPO}/{node.path}</span>
        </div>
        <div className="field" style={{ maxWidth: 'none', marginBottom: 0, marginTop: 12 }}>
          <label htmlFor="trash-restore-to">
            目的地覆盖 to（可选，<span className="mono" lang="en">/&lt;repo&gt;[/&lt;path&gt;]</span>）
          </label>
          <input
            id="trash-restore-to"
        ...
        <p className="field-hint" style={{ marginTop: 8 }}>
          目的地解析：to 覆盖 &gt; 属性五元组（原仓/原路径）&gt; 路径结构首段；目标仓必须为
          local，且目标已有同名文件会被覆盖（move 族 override 语义）。
        </p>
      </>
    )
    const ok = await confirm({ title: '恢复条目', body, confirmLabel: '恢复' })
    if (!ok) return
    setBusy(true)
    setOpError(null)
    try {
      ...
      toast.success(first?.message ?? '恢复完成')
      setSummary(null)
      afterMutation()
    } catch (err) {
  ...
  const doClean = async (node: ChildNode): Promise<void> => {
    const body: ReactNode = (
      <>
        <p>
          将从回收站<b>永久删除</b>{' '}
          <span className="mono" lang="en">{TRASH_REPO}/{node.path}</span>
          {node.folder ? '（整个子树）' : ''}。制品不可变，清除没有撤销；对应 blob
          离开引用集、由常态 GC 回收。
        </p>
      </>
    )
    ...
    try {
      ...
      setSummary(sum)
      toast.success(`已清除 ${formatCount(sum.removed)} 项（${formatCount(sum.files)} 文件 / ${formatCount(sum.folders)} 目录）`)
      afterMutation()
    } catch (err) {
      setOpError(err instanceof ApiError ? err : new ApiError(0, errText(err)))
    } finally {
  ...
  const doEmpty = async (): Promise<void> => {
    ...
    const body: ReactNode = (
      <>
        <p>
          将<b>清空整个回收站</b>——罐内全部条目永久删除（含所有原仓/原路径）。制品不可变，
          清除没有撤销；对应 blob 离开引用集、由常态 GC 回收。保留期（默认 14 天）内未到期的
          条目同样被清除。
        </p>
        <div className="field" style={{ maxWidth: 'none', marginBottom: 0, marginTop: 12 }}>
          <label htmlFor="trash-empty-confirm">
            输入 <b className="mono" lang="en">EMPTY</b> 以确认：
          </label>
          <input
    ...
    try {
      ...
      setSummary(sum)
      toast.success(`回收站已清空：移除 ${formatCount(sum.removed)} 项`)
      afterMutation()
    } catch (err) {
      setOpError(err instanceof ApiError ? err : new ApiError(0, errText(err)))
    } finally {
    ...
    <div data-testid="trash-page">
      <div className="page-header">
        ...
        <span className="text-2" style={{ fontSize: 'var(--bf-fs-aux)' }}>
          local 仓删除先捕获进内置仓 <span className="mono" lang="en">{TRASH_REPO}</span>（保留期默认 14 天，小时级 cron 自动清）
        </span>
      </div>
      ...
      {slot.status === 'ok' && locked && (
        <Paper component="section" sx={{ p: 2, mb: 2 }} data-testid="trash-locked">
          <Typography variant="subtitle2" component="h3" sx={{ mb: 1 }}>
            回收站未解锁（trashcan 槽 · 需要 {tierNeeded} 档）
          </Typography>
          <p className="field-hint" style={{ marginTop: 0 }}>
            当前实例的 <span className="mono" lang="en">trashcan</span> 功能槽处于锁定态（Q3 暂行 pro
            档）：删除侧<b>不捕获</b>（维持硬删语义），trash REST 族对 community 答 403 +{' '}
            <span className="mono" lang="en">X-Binflow-License-Required: trashcan</span>。装对应档位
            license 后即刻解锁（<Link to="/admin/general/license">License &amp; Add-ons</Link>）。
          </p>
        </Paper>
      )}
      ...
      {!locked && (
        <>
          <Paper component="section" aria-label="回收站内容" sx={{ p: 2, pb: 1.5, mb: 2 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 2, mb: 1, flexWrap: 'wrap' }}>
              ...
              <Button variant="outlined" size="small" data-testid="trash-refresh" onClick={() => bump()}>
                刷新
              </Button>
            </Box>

            {readOnly && (
              <p className="field-hint" data-testid="trash-readonly-note">
                ⓘ 只读管理员视角：恢复/清除/清空是管理面写操作（system:write，仅全量
                admin）；浏览只读，服务端 403 兜底。
              </p>
            )}
            ...
            {listing.status === 'error' && listing.error && (
              <>
                {listing.error.status === 404 ? (
                  <EmptyState
                    testid="trash-unused"
                    message="回收站尚未启用（未发生过捕获）"
                    hint={`内置仓 ${TRASH_REPO} 懒落库：首次捕获删除时才创建。若实例刚装好 pro license，先删一次制品再回来。`}
                  />
                ) : (
            ...
            {listing.status === 'forbidden' && listing.error && (
              <EmptyState
                message="无权限查看回收站"
                hint={`浏览走存储面（GET /api/storage/${TRASH_REPO}，管理面读门）；写操作另需 system:write。`}
              />
            )}
            {listing.status === 'ok' && listing.data && listing.data.length === 0 && (
              <EmptyState
                message={dir === '' ? '回收站是空的' : '此目录为空'}
                hint={dir === '' ? 'local 仓的删除会捕获到这里（保留期内可恢复）；本地生成物（索引/元数据）不入站。' : undefined}
              />
            )}
            {listing.status === 'ok' && listing.data && listing.data.length > 0 && (
              <Table size="small" data-testid="trash-list">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell component="th" scope="col">{adminWrite && !readOnly ? '操作' : ''}</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {listing.data.map((node) => (
                    ...
                    >
                      <TableCell>
                        ...
                        <CopyButton value={`${TRASH_REPO}/${node.path}`} label={`路径 ${node.name}`} />
                      </TableCell>
                      <TableCell>
                        <Chip size="small" className="badge neutral" label={node.folder ? '目录' : '文件'} />
                      </TableCell>
          ...
          {selected && (
            <Paper component="section" aria-label="条目详情" sx={{ p: 2, pb: 1.5, mb: 2 }} data-testid="trash-detail">
              ...
              <div className="kv">
                ...
                <CopyButton value={`${TRASH_REPO}/${selected.path}`} label="回收站路径" />
              </div>
              <div className="kv">
                <span className="k">类型 / 大小</span>
                <span>
                  {selected.folder ? '目录' : '文件'} ·{' '}
                  ...
                  {Object.entries(detail.data)
                    ...
                    .map(([k, vs]) => (
                      <div className="kv" key={`x-${k}`}>
                        <span className="k">
                          <span className="mono" lang="en">{k}</span>（随行属性）
                        </span>
                        <span className="mono" lang="en">{vs.join(', ')}</span>
                        <CopyButton value={vs.join(', ')} label={`属性 ${k}`} />
                      </div>
                    ))}
                </>
              )}
          ...
          {summary && (
            <Alert severity="success" icon={false} data-testid="trash-summary" sx={{ mb: 2 }}>
              <div className="headline">清剿完成</div>
              <div className="kv">
                <span className="k">移除</span>
                <span className="mono">{formatCount(summary.removed)} 项</span>
              </div>
              <div className="kv">
                <span className="k">文件 / 目录</span>
                <span className="mono">{formatCount(summary.files)} / {formatCount(summary.folders)}</span>
              </div>
              <div className="kv">
                <span className="k">字节</span>
          ...
          {opError && (
            <Alert severity="error" icon={false} role="alert" sx={{ mb: 2 }}>
              <div className="headline">
                <span aria-hidden="true">✗</span> 操作失败（HTTP {opError.status || '网络'}）
                {opError.status === 403 ? '——trash 族门为 system:write（仅全量 admin）；license 槽锁定时为 403 + X-Binflow-License-Required' : ''}
              </div>
          ...
          {adminWrite && !readOnly && (
            ...
            >
              ...
              <p className="field-hint" style={{ marginTop: 0 }}>
                永久清除罐内<b>全部</b>条目（含保留期内未到期项）。输入 EMPTY 强确认。
              </p>
              <Button
                variant="outlined"
                color="error"
