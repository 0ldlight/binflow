...
export function mavenTarget(f: GavForm): { dir: string; file: string; error: string | null } {
  ...
  if (g === '') return { dir: '', file: '', error: t('groupId 不能为空') }
  if (g.includes('/')) return { dir: '', file: '', error: t('groupId 用点分隔，不能包含 /') }
  if (g.split('.').some((seg) => seg === '')) return { dir: '', file: '', error: t('groupId 有空段（连续点）') }
  if (a === '') return { dir: '', file: '', error: t('artifactId 不能为空') }
  if (a.includes('/') || a.includes(':')) return { dir: '', file: '', error: t('artifactId 不能包含 / 或 :') }
  if (v === '') return { dir: '', file: '', error: t('version 不能为空') }
  if (v === '-SNAPSHOT') return { dir: '', file: '', error: t('version 不能是裸 -SNAPSHOT') }
  if (v.includes('/')) return { dir: '', file: '', error: t('version 不能包含 /') }
  if (c.includes('/') || c.includes('.')) return { dir: '', file: '', error: t('classifier 不能包含 / 或 .') }
  if (p.includes('/') || p.includes('.')) return { dir: '', file: '', error: t('packaging 不能包含 / 或 .') }
