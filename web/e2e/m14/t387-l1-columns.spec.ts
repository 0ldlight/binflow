...
test('admin: repos column selector — open/close, hide/show, guard, reset, per-page persistence', async ({
  ...
  // 开（①）：aria 语义 + 菜单项恰 8、全勾（含 Repository Type）
  const trigger = page.locator('[data-testid="repos-columns"]')
  await expect(trigger).toHaveAttribute('aria-haspopup', 'menu')
  await expect(trigger).toContainText('Columns 8/8')
  await trigger.click()
  ...
  await expect(th.filter({ hasText: 'Description' })).toHaveCount(0)
  await expect(page.locator(`[data-testid="repos-row-${key}"] td`)).toHaveCount(7)
  await expect(trigger).toContainText('Columns 7/8')
