...
错误面统一 errors JSON 信封：conan 数据面的 ghost GET 族（latest/revisions/snapshot/文件）回 404 `{"errors":[{"status":404,"message":"Not Found"}]}`——与 npm 的幽灵包 404 同构（信封族对齐 参考仓库 真身，conan 原生裸文本形态不回）。
