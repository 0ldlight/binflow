...
- **B1（批 1 启动 + BE 副线首航）**：T-382 ｜ T-392
  - **T-382** [P0] FR-124.1 D1 Set Me Up 抽屉化 `role:dev-frontend` area:web/src/components/SetMeUpDialog.tsx（壳 Dialog→Drawer；AppShell resumeOpen 分支为回归面）dep:T-381（V1 细节断言——Tab 命名/折叠形态；降级不阻塞主形态）
  ...
  - **T-392** [P1] FR-129 docker remote pull-through 首航 `role:dev-registry-adapter` area:internal/adapter/docker（/v2 remote 数据链）+ internal/repo（docker remote 配置）dep:T-363（M13 helmoci remote 缝 as-built——K54「边际成本≈0」判定成立 + T-380 转 M14 留痕在案）
