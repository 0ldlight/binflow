...
- **T-404 [P1]** replication CRUD 内嵌表单（FE）——仓库编辑页 Replications 节（R1 裁定形态：内嵌节无 modal/drawer + E1 输入 name 删除确认）+ 仓 Tab 指针升级 + 列表 Replications 列 + 行级 Run 动作；消费既有 POST/DELETE/GET + 新 PUT（启停——与 T-405 联合腿归 conductor 合并后验证）；四闸门 + axe + 锚册 v1.25 + 服务端 diff=0。
- **T-405 [P2]** replication mini-PUT（BE）——PUT /api/replications/{id} enabled 翻转（事件驱动模型下=订阅行状态位、引擎即时生效）；PUT 覆盖面（R6 字段族中置信注记）票内定案留痕；table-driven + replication 包回归 + lint 0。
