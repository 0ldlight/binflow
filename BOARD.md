...
> **M6 收尾态·收官中**（2026-08-23）。**用户终裁已落 ADR-0025**（Q2→本地 filestore 会话入 `upload_sessions` 表；Q6/Q7/Q10→认领暂行；Q8/Q9→延后 M7；用户禁用 seam→修复票）。派生三票：**T-210 done** / **T-208 done**（review APPROVE + qa PASS 5/5，提交 `f21fd74`——真实栈 curl 16 腿：禁用即存量 token 401、缺省保持、非 admin 403、session 臂 401）/ **T-209 qa PASS 5/5**（锚基线 `e7b581e`，三上轮 FAIL 确认关闭），唯余 **review2 B1 返修**（`engine.go:117` sweep 失败 `RemoveAll(uploads/)` 纯破坏路径，删行 + 钉死测试）在途——合入复验后 T-209 done、三票齐。遗留债（延后 M7）：N6/O-2 重启续传 REST 可见性（引擎能力已就位、无生产调用方）、N3 ctx 取消窄窗（fail-closed）、O-1 干净停机清会话（非回归）、N2 restart 臂注释、internal/auth 53 条既有 lint。ADR-0006 修订与勘误 `ae81c64` 已随 T-209 落。DoD #5：三票 done + 全绿后本地打 `m5-done`（落点 `5d13c3c`）+ `m6-done`；**push 外发须用户单独授权**。

## 🔨 进行中（doing）

- **T-209** [P1] filestore session 入 DB（ADR-0025 决策 5 修正，修订 ADR-0006 决策 2） `role:dev-go-storage` `area:internal/storage + internal/metadata + cmd/binflow-server` — review2 2026-08-23（原三 blocker ①snapshot purge ②ResumeSession 锁序 ③sweep 测试均确认真实修复；**新 B1**：`engine.go:117` sweep 任意失败 `RemoveAll(uploads/)` 摧毁在途上传——瞬时 DB 故障即触发，纯破坏无收益，返修轮 2 已派）+ 顺手项 N1 godoc/N5 注释/N4 sql 行尾；N2 禁改（qa 在跑）、N3 记债、N6（重启续传 REST 可见性）conductor 裁定延后 M7、N7 无害不动。**qa PASS 5/5**（锚已提交基线 `e7b581e`：三上轮 FAIL 确认关闭；kill -9 部分数据幸存、sweep/expiry/半路断开全过；O-1 干净停机清会话非回归、O-2= N6 同源延后 M7）；B1 返修增量待复验后合入。日志 reports/agents/T-209-review2.md / T-209-qa.md
