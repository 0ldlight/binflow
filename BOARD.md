...
- **T-37** [P0] docker token 流（批次 2） `role:dev-registry-adapter` `area:internal/adapter/docker(token)`
  状态：在途（v1.1 口径袋 + R7 边界）。
- **T-42** [P1] gc 旗标+GC mark 扩容（批次 3 填宽） `role:dev-go-core` `area:cmd/binflow-server`
  状态：13:5x 派发，在途。
