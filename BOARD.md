...
- **B7**：T-291 [P1] Properties Tab FE｜ T-292 [P1] 规格批次二（RPM/Helm）— 双票已派 2026-08-26 09:34（勘误：原记 09:50 系笔误）
  ...
  - **T-294 → review 2026-08-26 11:00**（自测全绿：28 包 + lint 0 + 不变量/矩阵 0 偏差〔baseline 无 cargo 行故零翻格〕+ **真实 cargo 1.98 L-r1~L-r6 全绿**〔publish cksum 对账/add+build/yank 翻转/search 官方契约〕+ 门控全链〔D3 400→pro 200→卸载 403+D1〕；七条偏差 D-1~D-7 登记（D-1 publish 200 体去 errors 键——cargo 客户端实证判失败）；**跨包显著**：internal/auth 裸 token 臂〔cargo 官方 `Authorization: <token>` 无 scheme，live 探针〕。评审员已派（auth 面优先）。conductor 门已过：build/lint 0/cargo+auth 包/双矩阵。日志 reports/agents/T-294.md）
