...
- 对标主流制品仓库：架构与概念模型保持一致（仓库模型、存储引擎、权限体系），让使用方可以平滑迁移、文档直接类推。
- **行为参考**：`reverse-src/` 下存放参考实现反编译代码（Java，不入库）。
...
BinFlow 的概念模型与主流仓库产品一一对应（落地见 DECISIONS.md ADR-0003）：
...
5. 全量仓库 REST 兼容 → **进产品范围**（分程分批交付——按域渐进扩展兼容面，自有 `/api/v1` 并行保留；排程归里程碑规划）
