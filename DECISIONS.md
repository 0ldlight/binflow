...
- 勘误五（2026-09-25，T-520，机制轴零翻动——仅联动引用面注记，无正文作废项）: ADR-0013 联动记录① 曾引本 ADR 为「无 `<key>-cache` 影子仓投影」的两桶简化依据；ADR-0051 起 virtual 解析引入 cache facet **逻辑投影**（remote-cache-projection.md §1 派生语义，internal/remote 注册表），但**缓存行仍落 remote 仓自身命名空间**——决策 1 的落盘/再验证协议与勘误一~四的 fetch 侧语义（offline/负缓存/包型 TTL/blob 豁免检索窗）**全部不动**。投影是解析层派生视图，非存储面变更；本条仅留勘指针。
...
- 勘误（2026-09-25，T-520 依 docs/reverse/virtual-resolution.md §1/§2（T-516 产出，高置信度）升级解析序，机制轴定案 = ADR-0051）: **联动记录 ① 的「两桶序」作废**——四段序（优先 local+cache 投影 → 优先 remote 本体 → 非优先 local+cache 投影 → 非优先 remote 本体，段内 real-local 先于 cache 投影）取代；「BinFlow 因无独立 `<key>-cache` 影子仓投影（ADR-0012）简化为两桶」的前提由 ADR-0051 的**逻辑投影**取代（cache facet 为解析层派生视图，零物理命名空间迁移）。首命中即返、成员 remote 命中 stale 即成员结果、探索性 miss 无缓存残留、`X-BinFlow-Resolved-From`、`virtual_members.position` 声明序语义、写路由（联动记录②）**全部维持**。详见 ADR-0051 + docs/design/virtual-four-bucket.md。
...
- 勘误三（2026-09-11，LOOP 004 L004-4 追加，conductor 裁定——收口 ADR-0049 交叉对账段登记的可选 Errata 候选）: 勘误① 中「`GET /api/v1/system/gc` 状态端点不做（P2 债务）」的 P2 债**宣告吸收**——Artifactory 路径族新面 `GET /api/system/storage/prune/status`（ADR-0049 §决策4 锚定的状态/报告面）即系统级删除任务的状态查询承载；旧 `/api/v1/system/gc` 维持现状（同步执行模型、无状态端点）**不再扩**，状态查询需求统一走新族。机制轴零翻动（同步执行/apply 极性/409 互斥/GC↔export 共锁全部维持）。
...
- 状态: Accepted（2026-08-30，M13 B0 T-359 交付，与 T-358〔webhook.md 规格票〕并行——决策基于 PRD §1.4 工作方式条款先行，wire 字面量锚点随 T-358 落地后回填，见「锚点回填位」；本 ADR 定机制不定字面量——ADR-0035/0038 同款条款。**2026-08-30 errata/backfill（T-360）**：决策 2/4 投递语义与 T-362 AC-3 断言已按 webhook.md §5/§4 官方锚点回填修订——见「锚点回填勘误」，机制条款不翻）
- 日期: 2026-08-30
...
- 决策: **八轴全 A**。要点十条：
  ...
  2. **订阅模型 = push-only**。BinFlow 主动 HTTP POST 到订阅回调 URL；2xx = 投递成功；**可重试失败 = 仅发送失败（超时/连接错）或 HTTP ≥500——4xx 不重试（单次终态），3xx 不跟随时同判终态**（**2026-08-30 errata 回填**：原「3xx/4xx/5xx/超时/连接错 = 可重试失败」句作废——官方重试条件逐字「unable to send the request or … error (**>= 500**)」〔webhook.md §5.2 retryCount 行〕；3xx 终态系「不跟随 = 未送达形态、不在重试集」的字面推论，为 T-364 as-built 实现形态，V4 待验证项在案——活体若翻仅动 deliver() 分类一臂，见「锚点回填勘误」②）；**重定向不跟随**维持（开放重定向是 Guard pinning 的绕过面，`CheckRedirect: ErrUseLastResponse`〔replication client 同款〕）。**at-least-once**：kill -9 落在「发送成功与状态落库之间」→ 重启后重投可能，接收端幂等义务文档化（tech-writer 指南）。pull 面（事件流 REST）不建。**test 端点 = 同步直发**：构造合成事件 → 同一签名/SSRF/HTTP 客户端路径单发一次（**不入 outbox、不重试、无死信**），响应携带 attempt 结果（状态码/耗时/错误类别）；记审计 `webhook.subscription.test`。
  ...
  4. **投递器 = Dispatcher（replication Engine 队列模式的复刻，非共用）**。否决轴 3-B：replication 任务是 blob 推送（payload = sha256 引用 + 凭据解析 + 平面协议族），webhook 任务是信封直发（payload = 内嵌字节 + 签名头）——泛化共用使两域任务语义互相渗透，同模式各自实现是分区纪律的代价下限。**参数定案（K50，引擎内部常量——ADR-0040「零新配置键」同姿态，旋钮化等真实需求；2026-08-30 errata 回填——原暂行值「尝试 10 / 2s 起步 ×2 递增 cap 10min / 单次 10s」系与 T-358 并行定案的 PRD 暂行值，按 webhook.md §5.2 官方锚点翻转如下，原数字存目于「锚点回填勘误」①）**：尝试上限 **5**（retryCount，**首试计入**——共 5 次尝试 = 初次 + 4 重试；「首试计入」解释置信度中〔webhook.md V3〕，as-built 按 5=总次数实现并测试固化）；重试间隔 = **固定 10s**（retryWaitMillis——**固定间隔，非指数退避**，官方无 backoff 曲线）；单次 attempt 超时 **30s**（timeoutMillis——**含建连、重定向、读响应体**，计时至 body 读完）；投递 worker 并发 **4**（引擎内部常数，非官方 wire 参数——不翻）；超上限 → status=dead（可查询、可重放：dead→pending 重置 attempts，REST + 控制台双面）+ 告警日志一行（最坏故障窗随之收敛：4×10s 重试间隔 + 5 次 attempt 预算 ≤190s，死信更早可观测）。**启动 sweep**：status='delivering' 行全部回 pending（attempts 保留——kill -9 中途行的唯一回收路径，ADR-0028 孤儿回收同姿态）。**优雅停机**：SIGTERM → 停止取新行、在途 attempt 完成或超时、行状态落库（Engine.Close 家族接线）。**queue_depth gauge = pending 行数**。
...
- **锚点回填位（T-358 webhook.md 落地后回填本节，机制条款不随锚点翻转）**：K47 订阅 REST wire（端点路径/方法/请求响应体/错误码——含单型/多型订阅形态对决策 3 UNIQUE 约束的校准）；K49 签名头名与编码 + envelope 字段集 + 过滤器语法（对决策 2/5 的字面量填充）；K48 事件清单终版与 BinFlow 触发源覆盖界（对决策 7 常量集的填充与 dormant 划界）；Q4 档位终裁（对决策 8 MinTier 的翻转点）；Q6 覆盖界终裁（对决策 7 的翻转点）。效力序：用户裁决（BOARD）> webhook.md（含 as-built 段）> 本 ADR > PRD 暂行值；分歧按 19:05 上 BOARD。**回填执行状态（2026-08-30，T-360）**：投递语义面已回填——K50 的投递参数与重试条件（决策 2/4）+ envelope 时间戳断言（T-362 AC-3）+ T-364 骨架 AC-2/AC-3 数字，共四项，见下「锚点回填勘误」；K47/K48/K49 的字面量终值（订阅面七端点 `event/api/v1/subscriptions*`、13 域 66 型覆盖界、`X-JFrog-Event-Auth` + HMAC-SHA256 hex）以 T-358 webhook.md 为准按效力序直接生效，机制条款（决策 3/5/7/8 的载体与形态）不逐条改写；Q4 取证结论在案（官方矩阵社区档 ❌ / Pro X 起 ✅——webhook.md §8 建议维持 MinTier=pro），终裁归用户窗；Q6 终裁仍开放。若后续需按 K47 校准订阅 wire 形态级分歧（如 key/handler 单订阅模型对决策 3 UNIQUE 约束的校准），另立勘误，不在本次终裁范围。
- **锚点回填勘误（2026-08-30 errata/backfill，T-360 执行；ADR-0006 勘误同款形态——不推翻决策本体，只翻 wire 字面量与验收断言）**：触发票 = T-370 登记（PRD v1.1 §0 冲突登记 + BOARD T-370 裁定材料③）→ **用户终裁 2026-08-30：对齐官方锚点**（效力序确认 webhook.md > 本 ADR）→ T-364 as-built 先行（commit 25e8afd，wire 参数已按锚点实现——本勘误为文档对齐，**零代码翻转**）。修订四项：
  ① **决策 4 投递参数**（webhook.md §5.2）：尝试上限 10 → **5（首试计入）**；2s 起步 ×2 递增、单次上限 10min → **固定间隔 10s（无退避曲线）**；单次 attempt 超时 10s → **30s（含建连/重定向/读体）**。worker 4、死信/Replay、启动 sweep、优雅停机、queue_depth 等机制条款不动。
  ② **决策 2 重试条件**（webhook.md §5.2 retryCount 行原文）：「3xx/4xx/5xx/超时/连接错 = 可重试失败」→ **仅发送失败或 HTTP ≥500 可重试；4xx 单次终态；3xx 不跟随判终态**（T-364 as-built 形态；3xx 分类为锚点字面推论，webhook.md V4 待验证——活体翻转点 = deliver() 分类 switch 一臂）。
  ③ **T-362 验收锚点骨架 AC-3**：envelope 逐字段断言删除「时间戳」——artifact 域载荷无事件时间戳字段（webhook.md §4「不要发明字段」；T-358 §1-8 取证；仅 build/xray/app_trust 域有各自时间字段）。
  ④ **T-364 验收锚点骨架 AC-2/AC-3 同步**：AC-2 退避序列观测 → **固定间隔 10s 重试序列 + 4xx 不重试负面臂**；AC-3 死信阈值 10 attempts → **5 attempts（首试计入）**。
  机制条款不动清单（终裁范围外，明示维持）：outbox 双方言两表（migration 018）/ 死信与 Replay 为 BinFlow **additive** 扩展（官方无持久死信、耗尽后弃投仅排障记录留痕——webhook.md §5.3 明示 BinFlow 形态归本 ADR）/ Guard 默认拒私网（`webhook.allow_private_target` 默认 false）/ worker 并发 4 为引擎内部常数。**as-built 增补登记（非勘误对象，备查）**：T-364 另按 webhook.md §5.1/§7 锚点落了限流三参数（token bucket 1000/s 均值 + 10000 突发；maxConcurrentHandlers 50000——超限在 Emit 缝拒新）与排障环容量 10000 / 清理 30s，均为 DispatcherOptions 内部常量，「零新配置键」姿态不变。
- **T-362 验收锚点骨架**（FR-114 实现大票：订阅 REST + 36 事件注册 + 事件源织入；AC 对 PRD FR-114 AC1~AC6）：
  ...
  - AC-3（事件织入）：generic 仓 PUT → deployed（envelope 逐字段：类型/repo/path/订阅标识——照 webhook.md；**无时间戳字段**〔2026-08-30 errata 删除原「时间戳」断言——artifact 域载荷无事件时间戳，webhook.md §4 + T-358 §1-8 取证，勿发明字段〕）；DELETE / copy / move / 属性 PUT 各触发对应事件；docker push → docker 域（dind 腿）。
...
- **T-364 验收锚点骨架**（FR-115 投递引擎票：outbox/重试/死信/签名/SSRF/可观测；AC 对 PRD FR-115 AC1~AC7）：
  ...
  - AC-2（重试语义；2026-08-30 errata 对齐 webhook.md §5.2）：接收器 500×N → 观测**固定间隔 10s** 重试序列（间隔恒定、无增长曲线）→ 恢复 200 → delivered；接收器 4xx（如 404）→ **不重试**（单次终态、attempts=1）负面臂；全程制品 PUT 零 5xx 零阻塞。
  - AC-3（死信与重放；阈值随 errata ① 翻 5）：持续失败超 **5 attempts（首试计入）** → dead 可查（REST）+ 告警日志一行 + dead_letter_total；replay（dead→pending）→ 恢复成功；订阅删除 → 在途行级联消失。
...
- **决策 7 勘误（2026-09-08，M17 W8 T-496 执行——追加式，正文原行保留，以本段为准；M16 Q11 → M17 Q5 终裁①「仅注册有源域」，build 域随 T-510 wired 后裁点已至，走决策 7 自己的「翻转路径」条款：Q6→M16 Q11→M17 Q5 编号沿革）**：注册面由「13 域 66 型全量注册 + dormant 如实标注」翻转为 **closed-domain——仅注册有 BinFlow 触发源的域**。落地态：artifact 5 wired + artifact_property 2 + docker 2 wired + build 3 = **4 域 13 型、12 wired**；docker/promoted 为有源域内唯一 dormant 幸存（可订阅、校验绿、永不触发——标注断言测试固化）；**九个无源域注销**（release_bundle / release_bundle_v2 / release_bundle_v2_promotion / distribution / destination / curation / user / xray_scan_status / app_trust——M13 过渡态曾以 dormant 注册，现 unknown-domain 400）。**存量 dormant 订阅处置（裁定票内定案）**：行保留可读（GET 逐字回显，读面永不复校验闭集）、PUT 再校验 400（unknown event domain）、永不触发（Emit 的未注册对 drop 先于订阅匹配）、不删除（安全底线——清除走操作员自己的 DELETE）。**不变项**：schema 零变化（event_type 字符串列，migration 018 不动）；dormant→wired 翻转路径维持（新裁定 + Emit 织入行即再注册，永不 schema 变更）；决策 7 机制条款（单一事实源/闭集校验/Source 标注）不动。as-built 载体：`internal/webhook/eventtypes.go`（注册表 + `DeregisteredDomains()`），断言 `closed_domain_registry_test.go` + 更新后的 TestT362EventClosedSet / TestT362RegistryTotal / TestBuildDomainRegistryAudit。**上游登记遗留**：FE 域选择器（web/src/lib/webhooks.ts 的 13 域静态副本）与用户文档的 dormant 标注面未随本勘误翻（area 纪律——归 conductor 派发 FE/docs 腿）。**同场 as-built 附记（非决策 7 对象，FR-159.2 / LC-109 C 层）**：outbox 行级 REST 落 `GET /api/v1/webhooks/outbox`（过滤/分页，system:read——readonly_admin 可读，D1 姿态不挂 feature gate）+ `POST /api/v1/webhooks/outbox/{id}/replay`（system:write + webhook 槽 feature gate；404 缺行 / 409 非死信 / 200 回显翻转后行）；审计词 +1 `webhook.delivery.replay`（决策 10 第六词，httpapi 侧字面量——归并 audit.Actions() picker 归 audit owner 一行）。


## ADR-0042: conan v1 files 通道 D-F2 存量布局迁移——启动期幂等 sweep（谓词消耗型二跑零改）、元数据路径重写零 blob 搬运、sha256 对账零损硬约束、回滚 = 元数据备份恢复（K53 定案；FR-119.2 / T-360，下游 T-371）

- 状态: Accepted（2026-08-30，M13 B1 T-360 交付；下游实现票 T-371〔dep 本 ADR + T-369，同 area 串行——D8 翻转先行，本迁移随后，无交叉写面〕）
- 日期: 2026-08-30
- 背景: **D-F2**（T-340 §4 登记，T-348 §4-3 转派）：`internal/adapter/conan/v1.go channelFileName` 以复数字面量 `segPackages`（"packages"）trim 通道路径的单数前缀 `dirPackage`（"package"）——永不命中。后果：v1 通道 PUT 的包文件落**双拼布局** `<coordinateRoot>/0/package/<pid>/0/package/<pid>/<tail>`（coordinateRoot = `<user>/<name>/<version>/<channel>` 四段、`_` 字面段同适），规格布局（conan.md §4/§3.2）应为 `<coordinateRoot>/<rRev>/package/<pid>/<pRev>/<file>`——v1 通道两修订段均为字面 `0`，即 `<coordinateRoot>/0/package/<pid>/0/<tail>`（PRD FR-119.2 的简写 `<root>/<pid>/<pRev>/<file>` 中 `<root>` 即 `<coordinateRoot>/<rRev>/package` 段，本 ADR 用全形消歧）。通道 PUT/GET 对称复用同一 trim，故 roundtrip/install 不破（T-308/T-327/T-340 全链因此一直绿）；**两枚可见后效缺陷**：① ref-search（v1 `conans/<ref>/search` 与 v2 `<ref>/search` 共用 `pkgFile(...,"conaninfo.txt")` 读取）落空 → settings/options/requires 恒空映射（T-340 e2e 实测 `settings:{}`，客户端 `-q` 过滤恒空）；② v1 包 snapshot 键携带 `package/<pid>/` 前缀（conan 1.66 参考实现为裸文件名；v2 包 files 清单键同病——同一 prefix trim 路径）。影响面：仅 conan **local** 仓（v1 数据面 local-only，conan.md §1/§7；remote pull-through 树不经 v1 通道，无双拼形态）、仅**包文件臂**（recipe export 臂不经 channelFileName，无病）、仅 M12 T-308 以来经 v1 通道写入的存量树。conan.md §1 的绿地决策「无迁移 job、无闸门」不受本 ADR 影响——那是针对 Artifactory ConanV2MigrationJob（v1→v2 协议版本布局翻新）的裁定；本迁移是 BinFlow 自有缺陷的存量自愈，目标布局即既定规格布局，零协议语义变化。约束：FR-119.2（幂等零损硬约束 + 回滚路径，K53）；物理 blob 走 checksum 寻址（ADR-0006）——逻辑路径是 nodes 表行，迁移天然是**元数据重写**而非内容搬运；双方言义务（ADR-0007——经 repo.Service→metadata 走 store 抽象，方言中立自动满足）；零新配置键（ADR-0040 姿态）；M12 转绿门维持（NFR-P60 冷启动 <2s）；部署矩阵单写者形态（ADR-0004）。
- 候选方案: （三轴）
  - 迁移时机: A) **启动期 sweep**——bootstrap 序、HTTP 监听器起前，谓词扫描 + 前缀重写，幂等可重跑；B) 惰性迁移——各读路径（通道 GET/snapshot/ref-search）检测双拼形态即时改写；C) 永久双读兼容——不迁移，读面双路径 fallback + 键名归一化。
  - 迁移载体: A) **conan 域启动 sweep 函数 + repo.Service 新窄原语**——谓词知识留 adapter 包，重写经服务接口单缝；B) 编号 SQL 元数据迁移（migration 019，DDL 链内数据改写）；C) 复用 CopyOrMove 用户面管线（M12 copy/move 族，同仓 move 语义）。
  - 回滚路径: A) **元数据备份恢复**（ADR-0011 export/import CLI——本迁移 blob 面零接触，仅恢复 metadata.db 即无损）；B) 迁移台账表（记录 moved node ids，支持精确逆向重写）；C) 就地逆向谓词重写（spec→双拼）。
- 决策: **三轴全 A**。要点七条：
  1. **时机 = 启动期 sweep，监听器起前执行**。先例 = 启动 sweep 家族（上传会话 TTL sweep/ADR-0028 孤儿回收、ADR-0041 决策 4 webhook delivering→pending sweep）——boot 期自愈是 BinFlow 既定形态。否决 B（惰性）：读路径 ×N 的永久特判 + 首读竞态（并发首读同树）+ snapshot/清单键归一化散布各端点，为一个一次性形态差异污染全部读面；否决 C（双读）：把分歧永久化，违反单一真值，且 snapshot 键与 ref-search 仍需读期归一化（等于 B 的复杂度不迁移版）。崩溃中断 → 下次启动重跑（谓词幂等，见点 4）；监听器未起 ⇒ 读面结构性见不到半迁移树。成本界：仅 conan local 仓参与（repo 清单过滤 rclass=local + 包型 conan，零 conan 仓 = 零扫描零成本），单仓 O(节点数) 路径谓词过滤（`pkgDir` 前缀扫描 + Go 内匹配），dogfood 量级毫秒~秒级，冷启动门不破。多实例竞态：部署矩阵单写者，不设锁；未来多实例形态需 PG advisory lock（留缝不实现）。
  2. **谓词与映射（迁移范围，票 AC2 口径）**：源谓词 = 节点路径匹配 `<coordinateRoot>/0/package/<pid>/0/package/<pid>/<tail>`（两修订段字面 `0`、包段同 pid 双现、tail 为文件或子目录）。目标映射 = 前缀重写 `<coordinateRoot>/0/package/<pid>/0/package/<pid>/` → `<coordinateRoot>/0/package/<pid>/0/`（即规格 `pkgFilePrefix` 形），作用于子树内**全部节点行——文件与文件夹**（文件夹行随子重写或按同路径合并，无 blob 载荷；清空后的源文件夹行随迁删除——细则归 T-371）。误报排除论证：v2 面写入的 rRev/pRev 为 32/64-hex（hash 模式）或 VCS 串（scm 模式），非字面 `0`；三重字面约束（`/0/package/` + `/0/package/` + 同 pid 重复段）使假阳性实际不可能——迁移**无条件扫描**（不设跳过旋钮：幂等、干净时零成本，无跳过理由）。
  3. **载体 = conan 域 sweep 函数 + repo.Service 一条窄原语**（轴 2-A；形如 `RewriteNodePath`/子树前缀批改写——T-371 定形）。契约钉死四点：同 blob link 平移（sha256 不变）；系统态调用无权限门；**零副作用**——不触发 copy/move observer、不触发 webhook Emit（ADR-0041 织入面——内部重布局不是用户动作，不得向订阅者发 artifact.moved）、无逐节点审计；错误显式分类（源缺失 ErrNodeNotFound / 目标已存在交调用方处置）。否决 2-B（SQL 编号迁移）：迁移链是 schema-DDL 领域，把 conan 路径形状知识（谓词）下沉 metadata 层破坏分层；一次性 watermark 使谓词有 bug 时无法重跑自愈；append-only 前向链无 down 路径。否决 2-C（CopyOrMove 复用）：用户面管线携带权限门/trash 交互/observer 通知与审计——为 boot 期内部操作引入用户面语义不成比例。
  4. **幂等与冲突处置**：幂等 = **谓词消耗型**——迁移后源谓词零命中，二跑零改（AC 断言 `moved=0`）。重写事务单位 = 单仓单事务（或分批，T-371 定；半迁移树不可见由「监听器未起」结构性保证）。目标路径已存在：同 sha256 → 源行删除、计 dedup（重传幂等形态，内容无损）；异 sha256 → **节点行时间戳新者胜**驻目标、WARN 一行记两摘要、计 conflict（被弃内容仍在 blob 库、GC grace 内可手工恢复——不构成「损」）；生产对账口径要求 conflict=0 才算绿（夹具断言恒 0，>0 = WARN 不算通过）。可观测：每仓一行 INFO `conan v1 layout sweep: repo=<key> moved=<n> dedup=<n> conflicts=<n>`（conflict>0 升 WARN——ADR-0040/0041 启动水位行同形态）。
  5. **回滚 = 元数据备份恢复**（轴 3-A）：本迁移对 blob store **零接触**（checksum 寻址红利——节点重指向、blob 不动），故仅恢复备份中的 metadata.db 即完整回到迁移前状态（ADR-0011 export = metadata.db + blobs/，单恢复 DB 腿为本迁移特有红利；blob 目录原样不动）。否决 3-B（台账表）：为一次性角落引入持久 schema（migration 019 级变更）不成比例，备份工具已在；否决 3-C（就地逆向）：逆向谓词**无法区分**「已迁移的 spec 节点」与「v2 原生写入的 spec 节点」——不可安全表达，明示不做。
  6. **零损硬约束（sha256 对账口径，K53）**：① 逐节点——迁移前抓源清单（path→sha256），迁移后每源节点在目标路径以**同一 sha256** 存在（按前缀映射折叠后两清单全等）；② 聚合——单仓节点数前后一致（dedup 臂例外：源行删但同 sha256 内容已在目标，内容无损；conflict 臂恒 0）；③ blob 面——迁移不触 blob store API，`blobs/` 文件数与清单逐字节不变。验收载体：T-340 复现脚本双拼树夹具 → 升级迁移 → 对账 + conan 1.66 install roundtrip（FR-119 AC2）。
  7. **后效验收口径（票 AC2）**：迁移 + 修复（channelFileName trim 单数 `dirPackage`——T-371 前半，与新写布局同票同二进制，sweep 不与未修复写路径打架）之后：① ref-search settings/options/requires 自 conaninfo.txt 恢复为实际字段（非空映射，`-q` 过滤腿恢复）；② v1 包 snapshot 键 = 裸文件名；③ v2 包 files 清单键同归（同一 prefix trim 路径）；④ 通道 GET 经 latest-pRev 解析直达 spec 路径。**K53 回填**：PRD §5.6 K53（D-F2 存量迁移方案）由本 ADR 关闭——启动期幂等 sweep / 谓词消耗型二跑零改 / 元数据备份恢复回滚 / sha256 对账零损；PM 下批 PRD 修订时 K53 行状态改「已定案（ADR-0042）」（本票不写 PRD 文件，登记于此）。
- 理由: 三轴共同判据 = 「单一真值 + 读面零特判 + 先例复用 + 最小新面」。时机轴 A 把一次性差异收敛在 boot 单点，B/C 让分歧活在每一读路径里；载体轴 A 是唯一不把域路径知识下沉 storage/metadata 层、也不复用用户面语义的选项；回滚轴 A 的成立完全依赖 checksum 寻址的结构红利（blob 零搬运 ⇒ 回滚面收缩为 DB 恢复），B/C 都在为「迁移可能出错」买超出备份既有能力的保险。幂等的谓词消耗型实现使「二跑零改」成为结构性质而非约定；conflict 臂的新者胜 + 零冲突绿门槛把极小概率边缘（回滚 + 重传同路径异内容）留在可观测、可手工恢复的位置，不为其扩面。
- 后果: `internal/adapter/conan` 增 sweep 入口（bootstrap 序挂载——cmd 装配归 conductor 纪律）；`repo.Service` 接口 +1 窄方法（零 observer/audit/emission 副作用契约）；T-371 实现票消费本 ADR（锚点骨架见下）；零新配置键、零新迁移编号、零 blob 面变更、零 PRD/规格改写（conan.md 规格行本就写规格布局，as-built 分歧随迁移消除）；release note 一行可选（升级自动迁移、透明）；tech-writer 无新增运维面。
- clean-room 边界: 目标布局依据 = docs/reverse/conan.md §4（反编译补充公开规范——路径拼接**行为规格**，GitLab 响应形态互证；非代码复制）；迁移算法（谓词/sweep/对账/回滚）全 BinFlow 自有；参考实现的裸文件名 snapshot 键为 conan 1.66 OSS 客户端内置服务器公开源码行为（T-340 §1 取证在案）。
- **T-371 验收锚点骨架**（FR-119.2 实现票：修复 + sweep + 原语；AC 对 PRD FR-119 AC2/AC3/AC4）：
  - AC-1（修复与新写布局）：修正后 v1 通道 PUT 包文件 → 节点落 `<coordinateRoot>/0/package/<pid>/0/<file>`（树断言）；v2 包 files 清单键裸名；新写树 ref-search settings 非空。
  - AC-2（存量迁移）：T-340 复现脚本双拼树夹具 → 启动迁移 → 规格布局；**sha256 对账三口径**（源/目标清单映射折叠全等 + 单仓节点数一致〔dedup 臂豁免〕+ blob 面零变化）；conan 1.66 install roundtrip 迁移前后等价。
  - AC-3（幂等与冲突臂）：第二次启动 `moved=0`（二跑零改）；目标同 sha256 已在 → dedup 计数内容单份；夹具 conflict=0；每仓 sweep INFO 行在场。
  - AC-4（回归与门）：conan v1/v2 全链（T-308/T-312/T-340 序列）零回归；M12 转绿门维持（冷启动无感——零 conan 仓实例启动路径零扫描断言）。

## ADR-0043: AQL 查询引擎——语言子集 EBNF、AST→IR→参数化 SQL 两段编译（nodes+node_props+blobs 惰性 join）、repo.Service 只读 ACL 缝（repo 集合 SQL 织入 + path 级行复核同源）、K63 资源门三件挂引擎、错误面 E-01 映射与 httpapi WriteTimeout 归位（T-392）；internal/search 新包契约（FR-133 / T-408，四票 T-409/411/413/415 基座）

- 状态: Accepted（2026-09-01，M15 B0 T-408 交付；与 T-407〔aql.md 规格票〕B0 并行——本 ADR 定**机制与 Go 侧契约**，字段全集/操作符闭集/错误文案/compact 形态/virtual 语义等**字面契约以 aql.md 定案为准**，Accepted 前逐项对齐（见「软缝对齐清单」），分歧以勘误留痕回填——ADR-0041「定机制不定字面量」同款条款。**同日 Errata（T-409 回执 + Accepted 期对齐扫尾）**：$not 产生式作废/$msp 收录、checksums 改平名五字段、path/name 派生修正、type 默认 file、超时 408、官方截断通告 + 查询长度 6,000 门、virtual key 展开义务、非 admin 脱敏暂行——见文末「Errata」节，机制轴零翻动）
- 日期: 2026-09-01
- 背景: FR-133（M15 主轴）：AQL 语言子集（item+property 域）+ 执行引擎 + `POST /api/search/aql?compact`。取证基线 = JFrog 官方文档（PRD §1.4-1 webhook.md 先例：唯一行为基准）+ inv-1 §E（端点形态/九域/QRL→AqlTooManyRequests 反编译锚）+ inv-2 §1.C（14 老搜索端点）——官方文法锚已在案（docs.jfrog.com「AQL Query Structure and Syntax」：`<domain>.find(<criteria>).include().sort().offset().limit().distinct()` 方法链、`{"@license":"Apache-2.0"}` @ 属性键与 `@*` 通配、字段名大小写敏感、数字可引可不引、`$and/$or`、空 `{}` 合法、跨域点路径、**`.delete()`/`.update()` 写动作与 `.distinct()` 修饰存在**）。现役基座勾稽（读码定稿）：nodes 列面 = repo_key/path/sha256/size/mime/created_by/created_at/updated_at（**无 name/depth/type/updated_by 列**）；node_props（migration 013）PK(repo_key,path,name,value) + 预留索引 `idx_node_props_name(name,value)`（M10 承诺的兑现消费方）；blobs ledger（sha256 PK + sha1〔`idx_blobs_sha1`，003 迁移 T-73 缝〕+ md5〔无索引——ledger 行数=内容数，扫描有界，T-92 SearchByChecksum 同口径〕）；NodeSearcher 缝（metadata/search.go——独立接口 + `md.Nodes().(NodeSearcher)` 断言形态，nodeQuery「静态 SQL 片段 + 参数化 args」纪律）；T-92 ACL 链 = searchGate（匿名闭环 403）→ SQL 侧 repoFilter IN 谓词 → filterVisible 行级 allow()；**auth.Authorizer.Can 的 target 覆盖含 path 级 include/exclude 模式**（targetCovers/matchesAny）——repo 集合过滤不完备；allow() 私有（repo/service.go:210，admin 短路 + nil az fail-closed）；E-01 envelope = `{"errors":[{status,message}]}`（envelope.go，无 code/field 字段）；httpapi Server 仅 ReadHeaderTimeout 10s（**无 WriteTimeout**——T-392 登记归本 ADR 评审）；metrics = 自研 expvar Registry（ADR-0022，`binflow_*` 族 + label 对）。约束：零字符串拼接（NFR-S74 注入红线）；万节点 P95 item ≤500ms / property join ≤800ms（NFR-P67）；满载零 5xx 零 OOM（NFR-P68）；K63 暂行（上限 1,000 截断 / 并发 4 → 429+Retry-After / 超时 10s——与 Artifactory QRL 默认的冲突走 Q2 出口，C 层自有设计留痕）；双方言义务（ADR-0007）；零新配置键（ADR-0040 姿态——门参数 = 引擎内部常量，旋钮化等真实需求）；不逐行翻译 AqlOqlParser/SQL builder（ADR-0001——PRD「永久不做」行）；FR-133 四票拆分（T-409 语言/T-411 内核/T-413 门/T-415 端点）须接口可分可并行。
- 候选方案: （七轴）
  - 编译管线: A) **两段编译——search 包 AST→IR，metadata 包 IR→方言参数化 SQL**（IR = 方言中立的结构化谓词树；SQL 知识不越 metadata 层）；B) search 直产 SQL 字符串、metadata 透传执行（Query(ctx,sql,args)）；C) 不编译——AST 在 Go 内存过滤全表行。
  - name/depth 派生: A) **SQL 派生表达式**（双方言同式：`replace(path, rtrim(path, replace(path,'/','')), '')`——rtrim/replace/length 三函数 SQLite/PG 语义一致）；B) migration 019 生成列（`GENERATED ALWAYS AS`）物化 name/depth；C) Go 投影期派生（排序/过滤无法下推）。
  - ACL 织入: A) **两段——repo 集合 SQL 织入 + path 级行复核**：repo.Service 新只读缝 `SearchScope` 返回可读仓集合（全读仓 / path-scoped 仓两态）织成 `repo_key IN` 谓词，path-scoped 仓的行经同一 allow() 源的 `CanRead` 逐行复核；B) 全行级后滤（T-92 filterVisible 原样）；C) 权限模式 SQL 化（includes/excludes 模式翻译成 LIKE 谓词下推）。
  - 资源门载体: A) **引擎级三件套**（semaphore 并发 4 非阻塞 try-acquire + context deadline 10s + cap+1 截断探测），内部常量；B) httpapi 中间件层门；C) QRL 全量三态（PRD 明确 M16+）。
  - 错误面: A) **E-01 envelope 单形不动**（errors[]{status,message}——message 由 search 包结构化错误对象单点组装，含 domain/field 名）；B) 扩展 errorEntry 加 code/field 字段（改 E-01 形状——破坏全局契约）；C) AQL 专属错误 envelope（第二信封形态）。
  - WriteTimeout 归位（T-392）: A) **维持不设全局 WriteTimeout，查询时长治理归 K63 引擎 deadline**（响应体 ≤cap+1 行有界、编码毫秒级——无需传输层超时）；B) 设全局 http.Server.WriteTimeout；C) per-route SetWriteDeadline（http.ResponseController）。
  - sha1/md5 取数: A) **blobs join 惰性织入**（filter 或 include 引用 checksums.sha1/md5 才 `JOIN blobs b ON b.sha256 = nodes.sha256`）；B) nodes 表冗余 sha1/md5 列（migration + 写面三处接线）；C) 投影期逐行回查 ledger。
- 决策: **七轴全 A**。要点十条：
  1. **包边界与依赖方向（AC2 定案）**：新包 `internal/search`，**只 import metadata + repo 的公共 API 面**（零反向依赖、不摸内部结构）；httpapi 装配三者。文件布局 = 四票 area：`lexer.go/parser.go/ast.go/fields.go/queryerr.go`（T-409 语言前端，纯函数零 IO 零 DB）、`plan.go`（T-411 AST→IR 编译器）+ metadata 侧新缝文件 `aqlquery.go`（NodeQueryer——NodeSearcher 断言形态同款）、`engine.go/gate.go`（T-413 引擎入口 + K63 门 + 行复核）、`match.go`（通配→LIKE 转义内核——`$match`/`$nmatch` 与老搜索 pattern 端点〔T-417〕共享，防三个译者）、httpapi `search.go` 增 AQL handler（T-415）。`repo.Service` 接口 **+2 窄只读方法**（T-413 承载，文件 = api.go 声明 + search.go 实现〔T-92 缝同文件先例〕，与 T-412〔service.go〕同包不同文件，合入序归 tech-lead）。
  2. **文法形式 = EBNF 暂行骨架（软缝——字段/操作符闭集以 aql.md 定案）**：
     ```ebnf
     query       = domain , "." , "find" , "(" , criteria , ")" , { suffix } ;
     domain      = "items" ;                                  (* M15 唯一域入口；其余域 → 400 domain-not-supported *)
     suffix      = "." , ( "include" , "(" , [ field , { "," , field } ] , ")"
                        | "sort" , "(" , "{" , sort-dir , ":" , "[" , field , { "," , field } ] , "]" , "}" , ")"
                        | "offset" , "(" , integer , ")"
                        | "limit" , "(" , integer , ")" ) ;   (* .distinct()/.delete()/.update()：M15 不收 → 400（见点 6） *)
     criteria    = object ;                                    (* 空 {} 合法 = 全量（官方锚） *)
     object      = "{" , [ pair , { "," , pair } ] , "}" ;
     pair        = ( ( "$and" | "$or" ) , ":" , ( object | array ) )
                 | ( "$not" , ":" , object )
                 | ( field , ":" , ( scalar | comparator ) ) ; (* 裸 scalar = 隐式等值 *)
     array       = "[" , [ object , { "," , object } ] , "]" ;
     comparator  = "{" , op , ":" , scalar , "}" ;
     op          = "$eq" | "$ne" | "$lt" | "$lte" | "$gt" | "$gte" | "$match" | "$nmatch" ;  (* 终集照 aql.md *)
     field       = item-field | prop-field ;
     item-field  = ident , [ "." , ident ] ;                   (* 点路径形态（如 checksums 族）照 aql.md *)
     prop-field  = "@" , ( ident | "*" ) ;                     (* {"@license":"Apache-2.0"} / {"@*":"v"}（官方锚） *)
     scalar      = string | integer ;                          (* string = JSON 双引号；integer 可引可不引（官方锚） *)
     ```
     解析止步 AST（T-409 纯函数）；字段/操作符经**闭集注册表**（fields.go：field ID → {域, IR 表达式键, 可用操作符, 可排序, 可投影}）校验——注册表是「诚实拒绝」的单点：未注册域/字段/操作符 → 400 且 message 点名（含 M15 已知但无存储源的字段，如 `modified_by`——nodes 无 updated_by 列，**不加列不伪造**，注册表标 unsupported，后路 = migration 加列小票 + 勘误）。
  3. **编译映射（AST→IR→SQL 两段；AC1 核心）**——IR（`metadata.NodeQuery`）= 谓词树（And/Or/Not/Compare/PropMatch）+ 排序 + offset/limit + 惰性 join 标记，字段以**闭集枚举键**（FieldRepo/FieldPath/FieldName/FieldType/FieldDepth/FieldSize/FieldCreated/FieldModified/FieldCreatedBy/FieldSha256/FieldSha1/FieldMd5）引用，**用户输入只进 args 参数**——SQL 文本仅由常量片段拼装（nodeQuery G202 纪律同款，注入面结构性关闭）。字段→SQL 映射表：

     | AQL 字段 | SQL 表达式（除注明均双方言同式） | 说明 |
     |---|---|---|
     | repo / path | `nodes.repo_key` / `nodes.path` | path = repo 相对路径无前导 '/'（存储即如此） |
     | name | `replace(path, rtrim(path, replace(path,'/','')), '')` | 派生：末段名；folder 标记行（尾 '/'，ADR-0016）name='' |
     | type | file → `path NOT LIKE '%/'`；folder → `path LIKE '%/'` | 派生：nodes 无 type 列；仅收 $eq/$ne；值集照 aql.md |
     | depth | `length(path) - length(replace(path,'/',''))` | 派生：斜杠计数；口径照 aql.md 锚定 |
     | size / created / modified / created_by | `nodes.size` / `nodes.created_at` / `nodes.updated_at` / `nodes.created_by` | **时间比较走归一化表达式**（SQLite `julianday(x)` / PG `x::timestamptz` 双侧归一）——RFC3339 TEXT 裸比较在混时区偏移下不健全；排序同式 |
     | checksums.sha256 | `nodes.sha256` | 直接寻址 |
     | checksums.sha1 / .md5 | `JOIN blobs b ON b.sha256 = nodes.sha256` → `b.sha1` / `b.md5` | **惰性 join**（轴 7-A）：filter/include 引用才织入；ledger 侧取数（sha1 有索引/md5 扫描有界——T-92 同口径） |
     | `@key`（属性） | `EXISTS (SELECT 1 FROM node_props np WHERE np.repo_key=nodes.repo_key AND np.path=nodes.path AND np.name=? AND np.value <op> ?)` | **idx_node_props_name(name,value) 兑现消费**（M10 承诺）；多值语义 = 行级 any-of（EXISTS 天然）；`@*` 通配 → np.name 参数化 LIKE |

     `$match`/`$nmatch` → LIKE/NOT LIKE + `ESCAPE '\'`（通配 `*`→`%`、`?`→`_`，字面 `%`/`_`/`\` 转义——match.go 单一译者，K64 校准同源）；ORDER BY = 用户排序键 + `(repo_key, path)` 恒定 tiebreaker（翻页确定性）；无 `.sort()` 默认序即 `(repo_key, path)`（nodeQuery 既有稳定序）。
  4. **ACL 织入（AC1；不另建权限通道）**：`repo.Service` +2 窄只读方法——`SearchScope(ctx, p) ([]RepoScope, error)`（RepoScope{Repo string; PathScoped bool}：admin/匿名开实例 → 全部 local+remote 仓 PathScoped=false〔匿名读为全局语义，Can(nil) 同源〕；**virtual 仓不入 scope**——nodes 无 virtual 行，133.5 暂行「实际存储行」；匿名闭环 → 沿 searchGate 形态返 ErrForbidden；可读仓为空 → 引擎短路空结果，不落 SQL）与 `CanRead(ctx, p, repoKey, path) bool`（**allow(read) 的公开只读投影——同一私有 allow() 委托，同源由构造保证**，p=nil 匿名臂同判）。织入序：scope → `repo_key IN (?)…` 谓词（repoFilter 同款）→ 执行 → **仅 PathScoped 仓的行**经 CanRead 复核（path include/exclude 模式存在才触发——现网主体形态〔无模式 target〕零行级成本）。翻页语义在复核下的口径：offset 为 SQL 侧 offset，翻页遍历原始行序、逐页过复核——「offset/limit 分页可达全量」成立；截断标记语义 =「原始行序仍有余量」（诚实上界，见点 5）。否决 3-B（全行级后滤）：万节点 × 逐行 Can（每行 2 次 permission 查询）在 NFR-P67 预算内不可行，且 repo 集织入已消掉整仓面；否决 3-C（模式 SQL 化）：Go matchesAny 与 SQL LIKE 翻译成为**双真相源**，分叉即泄漏——搜索是泄漏面最宽的域，此轴只允许单源。
  5. **资源门 K63（引擎级三件套；AC1）**：执行序 = `Parse`（语法错 400 先于门——垃圾查询不占资源位）→ semaphore **非阻塞 try-acquire**（满 → 429 + `Retry-After: 1`，**不排队**——排队 = 无界等待面，AqlTooManyRequests 同语义）→ `context.WithTimeout(ctx, 10s)` 包执行段 → SQL `LIMIT effective+1`（effective = min(用户 .limit, 1000)；无 .limit = 1000）→ 多出一行即 `Truncated=true`，返回前 1000 行 → 释放位。截断标记走**响应头 `X-BinFlow-Search-Truncated: true`**（C 层自有扩展不动官方 envelope 形状——X-BinFlow-Resolved-From 头 §11.15 先例）。零 OOM 结构性保证 = LIMIT 下推 + 物化上界 1001 行（「流式装饰」的真身——PRD 133.4 的满载零 5xx 零 OOM 由界保证，非流式编码保证；httpapi 逐行 encode 可选归 T-415）。参数 = 引擎内部常量（`aqlMaxRows=1000 / aqlMaxConcurrent=4 / aqlQueryTimeout=10s / aqlRetryAfter=1s`）——零新配置键；**与 Artifactory QRL 默认的冲突走 Q2 出口**（PM 既定倾向：BinFlow 有门优于无门——单机 SQLite 形态防护必要性更高，冲突处 C 层留痕不硬仿）。
  6. **错误面 = E-01 单形（AC1）**：engine 错误分类 → httpapi 映射：`ErrSyntax`/`ErrUnsupportedDomain`（message 含 domain 名——「domain not supported」语义，零伪空集）/`ErrUnknownField`（含字段名与无源原因）/`ErrBadOperator` → **400**（文案逐字照 aql.md 核验样本——T-415 AC）；`ErrResourceBusy` → **429 + Retry-After**（E-01 body + 头）；`ErrQueryTimeout` → **暂行 503**（服务端资源语义，与 429 可区分；形态随 aql.md 锚定 + Q2 回写）；metadata 故障 → 500。`.delete()`/`.update()`（AQL 写动作面，官方存在）→ **400 诚实拒绝**（M15 AQL 是只读面——写操作走既有 REST/控制台通道；静默执行或伪空都不可接受）。结构化前信封形态 `QueryError{Kind, Domain, Field, Pos, Msg}` 由 search 包产出、message 单点组装——FE 内联呈现（FR-135）消费同一 message。不扩 errorEntry 字段（否决 5-B/C：E-01 是全局契约，单 AQL 域破形不成比例——域细节进 message）。
  7. **WriteTimeout 归位（T-392 收口）**：**不设全局 `http.Server.WriteTimeout`，由设计维持现状**。论据：同一 Server 承载内容面（`/binflow/**` 制品流 + `/v2/**` docker 层）——大 blob 下载/上传的响应写合法地以分钟计，任何全局 WriteTimeout 值都会杀内容面传输（net/http 的 WriteTimeout 覆盖整个响应写期，含 handler 业务耗时）；AQL 面的时长治理已由 K63 引擎 deadline 收口（超时发生在 SQL 执行期，响应体 ≤1001 行有界），传输层超时对此域无增量防护。T-392 登记项就此关闭：**超时治理统一在资源门（点 5），不另开面**；未来若引入 per-route deadline（http.ResponseController）属内容面/慢端点专项，另议不入本 ADR。ReadHeaderTimeout 10s 维持。
  8. **metrics 与慢查询日志（沿 ADR-0022 自研 Registry）**：search family 新组——`binflow_search_queries_total{plane="aql"}`（老搜索端点落牌时 plane 扩值）、`binflow_search_query_duration_seconds`（histogram）、`binflow_search_rejections_total{reason="concurrency"|"timeout"}`；慢查询（>5s = K63 阈值 1/2）单行 WARN 日志（query 摘要 + 时延——PRD §6.4 口径）。
  9. **老搜索并轨缝**：三端点（T-417）消费 match.go 通配内核 + search 包导出的 `ResultCap` 截断常量与截断头（「老搜索侧默认截断语义照 aql.md 锚」——形态同源单点）；数据路径维持 repo.Service 既有搜索方法（T-92 面零迁移）。virtual 仓 AQL 语义**暂行 = 实际存储行**（local + remote 缓存行；virtual 不作查询实体）——照 aql.md 活体锚定结论，翻转则勘误回写（133.5）。
  10. **四票拆分边界确认（AC3）**：T-409 交付 `Parse + 注册表 + QueryError`（纯函数，消费接口 = AST 类型）；T-411 交付 `Planner（AST→IR）+ NodeQueryer 缝 + AST→SQL 快照测试`（消费 T-409 类型与注册表）；T-413 交付 `Engine.Run + gate + SearchScope/CanRead 织入`（消费 T-411 IR）；T-415 交付 handler/envelope/metrics（消费 Engine）。每票产出可独立 table-driven 验证的纯函数或窄接口面——**四票粒度成立**，串行链上每票 review 面局部化（conductor 四分指令的落地形态）。
- 理由: 七轴共同判据 = 「SQL 知识不越层 + 单真相源 + 有界性由结构保证 + 先例复用」。管线轴 A 是唯一同时满足注入红线（参数化收口在 metadata——既有 nodeQuery 纪律原地延伸）、双方言义务（方言差异封在 store 实现内）与四票可分（IR 是 T-409/411 的交接契约）的选项——B 让 search 持表名/方言知识（schema 演进耦合进查询包），C 放弃 LIMIT/ORDER BY 下推、NFR-P67 无从谈起。派生轴 A 零迁移零写面改动且双方言同式（B 的生成列是为其唯一消费者 AQL 改热表写路径，YAGNI——真出现 name 排序热点再以索引化生成列升级，IR 键位不变）。ACL 轴 A 把 T-92 的行级语义精确抬升到集合语义：非 path-scoped 主体形态零行级成本、path-scoped 边角回到同一 allow() 源复核——B 不可行（性能）、C 不可接受（双源即泄漏面）。门轴 A 与错误轴 A 都是「域治理内聚 + 全局契约不动」的最小新面；WriteTimeout 轴 A 是内容面与查询面的时长语义差异的显式承认——传输层超时治不了查询慢，查询慢由 deadline 治。
- 后果: `internal/search` 新包（四票 area）；`repo.Service` +2 窄只读方法（零副作用契约：不写、不审计、不发 webhook）；`internal/metadata` +NodeQueryer 缝（新文件，nodeStore 实现，断言消费——NodeSearcher 先例）；httpapi search 面增 AQL 路由 + 截断头；**零新 migration、零新配置键、零新外部依赖、零既有端点行为变化**（T-92 面零回归由 T-415 AC5 钉）；技术债登记 architecture §11.46（path 复核行级成本 + memoization 缝）/§11.47（offset 无界深翻页）；Q2 出口（K63 参数与 429/超时形态随 aql.md 锚定回写）；PM K63 行随本 ADR 状态改「暂行定案（ADR-0043）」。
- clean-room 边界: 行为基准 = JFrog 官方文档（AQL Syntax/Criteria/Output 章节——2026-09-01 取证在案：方法链/JSON-like 规则/@ 属性键/大小写敏感/delete·update 动作面）+ docs/reverse/aql.md（T-407 落盘后为字面契约权威）+ inv-1 §E/inv-2 §1.C 补白；**EBNF/IR/SQL 编译/ACL 织入/资源门全 BinFlow 自有设计**；AqlOqlParser 与 SQL builder 的 Java 实现零参考零翻译（ADR-0001——PRD「永久不做」行）。
- **软缝对齐清单（AC3——aql.md〔T-407〕落盘后 Accepted 复核，分歧走本 ADR 勘误段，不翻机制只翻字面契约）**：① 属性键形态——本 ADR EBNF 取官方 `{"@license":"Apache-2.0"}` / `@*` 通配；**PRD 133.1 的 `{"@key":{"$eq":"license"}}` 写法与官方形态不一致**，以 aql.md 归一（若 Artifactory 旧版有 @key/@value 元字段形态，注册表双收）；② checksums 字段路径形态（点路径 `checksums.sha1` vs 嵌套对象）；③ name/depth/type 语义细节（folder 行 name 空值、depth 计数口径、type 值集 file/folder）；④ 操作符闭集终版（$match 通配语法 `*`/`?`、$nmatch 反义、隐式等值）；⑤ 截断标记走响应头（C 层自有）——若 aql.md 锚出官方 limit 截断形态（warning/字段）则对齐翻转；⑥ 超时错误形态（暂行 503）；⑦ virtual 仓语义（暂行实际存储行）；⑧ 无 type 谓词时 folder 标记行是否入结果（暂行 file+folder 均返）；⑨ compact 变体投影形态；⑩ 400 文案逐字样本（核验腿）。
- **四票验收锚点骨架**（FR-133 实现票消费）：T-409 = Parse 表驱动（合法/边角/拒绝三臂全覆盖，注册表闭集校验单测）；T-411 = AST→IR→SQL 快照（注入用例表驱动——字面 `%`/`_`/`\`/引号/`@*` 全转义断言）+ `idx_node_props_name` 消费核验（EXPLAIN QUERY PLAN）；T-413 = 双用户越权探针（t92 形态 + path-scoped target 夹具臂）+ 满载 8 路混合（零 5xx 零 OOM）+ 429/超时/截断三臂；T-415 = L21/L23 全链 + compact 对照 + 文案逐字 + metrics 族在场 + T-92 全量 spec 零回归。
- **Errata（2026-09-01 T-409 落地回执 + Accepted 期对齐扫尾——aql.md〔T-407〕落盘后的软缝复核，Status 维持 Accepted，机制条款零翻动；正文原行保留、以本段作废/替换为准——ADR-0041 勘误体例）**：
  - ① **$not 产生式作废（T-409 报告分歧 1）**：要点 2 EBNF 的 `| ( "$not" , ":" , object )` 产生式**作废**——T-407 定案（aql.md §0-1，双源：官方文档操作符面无 + t226 活体 `{"$not":{...}}` → 400 parse error〔v03-not-operator.txt〕）：**$not 不存在于 AQL**；PRD FR-133.1「$and·$or·$not 任意复合」前提随之校准为「$and·$or 任意复合」。同产生式位**补入 `$msp`**（官方 match-single-property 操作符：`{"$msp":[{条件},{条件}]}` 要求**同一属性实例**满足全部条件，普通多属性条件为任一命中；活体语法通过〔v12〕）——M15 取舍随本勘误定案：**收录**（aql.md §2.3 建议采纳；T-409 as-built 已收——parser 结构键，非字段 Op 集）。$contains 维持不支持（官方无此操作符，aql.md §2.4「不支持」行——本 ADR EBNF 原未收，无需作废）。
  - ② **checksums 点路径作废（T-409 报告分歧 2）**：要点 3 映射表 `checksums.sha256` / `checksums.sha1 / .md5` 两行**作废**，改为官方 item 域**平名**五字段：`sha256`（nodes.sha256 直取）、`actual_sha1` / `original_sha1` / `actual_md5` / `original_md5`（→ blobs.sha1 / blobs.md5，惰性 join 路径不变——轴 7-A 机制零变化）；**BinFlow 无 original/actual 双值语义——四字段同源两值**（aql.md §10「暂行同值」差异留痕）。IR 枚举键随 T-409 注册表实名（FieldSha256/FieldActualSHA1/FieldOriginalSHA1/FieldActualMD5/FieldOriginalMD5），要点 3 括号内 FieldSha1/FieldMd5 简写以实名为准。
  - ③ **path/name 映射修正（软缝清单③，aql.md §2.2/§10 定案）**：官方语义 = AQL `path` 为**父目录路径**、`name` 为**末段条目名**——两者均自 nodes.path（存储全路径）派生。要点 3 映射表「repo / path | `nodes.repo_key` / `nodes.path`」行中 path 半边**作废**：path → **去末段派生**（`substr(path, 1, length(path) - length(name 派生式))` 家族——双方言同式，name 派生式原文不变、为其前缀）；`$match`/`$eq` 对 path 的谓词语义照此编译（T-411 消费）。要点 3「path = repo 相对路径无前导 '/'（存储即如此）」注记改读为**存储层**事实（AQL wire 层 path 不含条目名）。**补强（2026-09-01 第三笔勘误指令，T-411 内核实现回执——conductor 确认为同一分歧，并入本条不另立）**：证据面补 t226 活体三探针（v01 基础方法链 / v05 / v09 全仓默认——输出行 path 均为父目录形态、不含条目名）；**T-411 as-built 已从 aql.md**（编译器按父目录/末段双派生实现，与本条作废-替换口径一致）；存储层 nodes.path 全路径语义（001_init.sql 注记）不受影响——仅 AQL wire 层投影与谓词编译按父目录形态。
  - ④ **type 默认语义翻转（软缝清单⑧，aql.md §2.2 活体 v09 定案）**：要点 9 及清单⑧的暂行「无 type 谓词 file+folder 均返」**作废**——官方默认**只搜 file**：查询不含 type 条件时查询侧补 `type=file` 谓词（= 既有 `path NOT LIKE '%/'` nodeQuery 默认式，零新面）；显式 `type:"folder"`/`"any"` 才含 folder 行（type 值集 file/folder/any，$eq/$ne 收）。
  - ⑤ **超时与 429 形态定案（软缝清单⑥，aql.md §5 K63 校准表）**：要点 6「ErrQueryTimeout 暂行 503」**作废**——超时码位对齐 **408**（A 层：官方 REST 900s 未设 override → 408；时长 10s 维持 C 层裁量）；429 body 对齐官方 `too many requests`（A 层码位与文案），`Retry-After` 头维持 C 层自有（官方未文档化——机读面补充）。并发 4 vs 官方默认 3、满载直接 429 不排队（官方 pending 10s 后 429）维持 C 层留痕（Q2 已有出口，不翻）。
  - ⑥ **截断通告与查询长度门（软缝清单⑤，aql.md §5）**：截断标记在原「`X-BinFlow-Search-Truncated` 响应头（C 层）」基础上**增补官方 A 层形态**——range.notification 文案逐字 `AQL query reached the search hard limit, results are trimmed.`（官方 OpenAPI 示例在册；envelope range 尾段形态照 aql.md §3.2）；C 层头并置保留（机读面）。**新增门维度：查询文本长度上限 6,000 字符 → 400**（官方 `aql.search.query.size.limit` 默认，A 层沿用）——检查点 = 引擎入口 Parse 前（T-413；T-409 parser 若已收上限则以其 as-built 为准），K63 常量族 +1（`aqlMaxQueryLen=6000`）。
  - ⑦ **virtual key 展开义务（软缝清单⑦定案，aql.md §7 双源）**：要点 9 暂行「实际存储行、virtual 不作查询实体」**确认成立**；补充定案 = **virtual key 是合法查询值**——编译期经 repo.Service 解析 virtual 成员展开（三分支：`$eq` 原条件替换为成员 OR 组；`$match`/范围原条件保留并与成员条件 OR；负向 `$ne`/`$nmatch` 成员组 AND 排除）；**结果行 `repo` 字段 = 实际存储仓 key**（非 virtual key）；`virtual_repos` 逻辑字段 **M15 收录**（查询目标为 virtual 时隐式输出——官方明文，aql.md §7-3/§10；运行时由 virtual_members 反查派生）——实现归 T-411 编译器（成员展开 + repo.Service 只读消费）与 T-415 投影，容量不足走 tech-lead 显式降级登记（不得静默丢弃）；`.transitive` 不实现（远期行）。**ACL 交互钉死**：展开后的成员仓仍过 SearchScope 织入——virtual 展开 ≠ 权限通道，成员不在调用者可读集即被谓词滤除（与内容面 pull 解析「成员级授权」同哲学）；不存在 repo key（含拼写的 virtual 名）→ 200 空集（官方活体 v06——无存在性校验）。
  - ⑧ **非 admin 身份脱敏暂行（aql.md §6 开放项交裁）**：M15 对齐官方默认——非 admin 调用者的输出行 `created_by` / `modified_by` 值投影为字面量 `unknown`（官方 ObfuscationUtils 默认脱敏；开关不建——零新配置键姿态）；实现归 T-413/T-415 投影装饰（admin 原值直出）；PM 可翻（脱敏 vs 全显的暴露面裁量，Q 项随收口窗）。
  - **软缝对齐清单就本 Errata 关闭**：①（属性形态——aql.md §2.3 确认 `@<key>` 主形态 + `property.key`/`property.value` 展开形态双合法；是否双收以 T-409 注册表 as-built 为准，不强制）、②③⑤⑥⑦⑧ 全闭；④（操作符闭集）收窄——$not/$contains 确认不支持、$msp 补入，其余（$last/$before 相对时间族、$eqic/$eqvic/$matchic 变体）按 EBNF 暂行标记照 aql.md §2.4 落地、无需再勘误；⑨⑩（compact/文案）照 aql.md §1/§3/§4 逐字锚落地，无分歧。机制轴（两段编译/派生表达式/ACL 两段织入/引擎级门/E-01/WriteTimeout/惰性 join）零翻动。

---

## ADR-0044: cron 调度域——统一 schedules 台账（表达式/next-run/last-run/状态/所属域）+ Quartz 六域子集自研解析与 next-run 纯函数 + 独立 1min ticker + 域注册面（三消费载体复用：TriggerFullSync / GC·cleanup RunOnce / export callable 化）+ 与事件驱动+outbox 并存语义（调度只触发全量类任务闭集、增量唯一引擎不迁、内容级零重复投递三层口径）+ 误触发防护三面；附 K68 会签（动作闭集 +annotate、wire 词 write→deploy-cache 翻新、零提权等价迁移——ADR-0026 增补评审）与 K69 会签（nodes 统计四列 + 三分计数口径 + last_downloaded_by 可见性门——详规 architecture §25）（FR-150 / T-436；下游 T-446/450/462 + T-444/438/440）

- 状态: Accepted（2026-09-03，M16 B0 T-436 交付；与 T-435〔cron 表达式子集锚——Artifactory/Quartz 对拍〕B0 并行在途——本 ADR 按 **Quartz 官方公开文档（quartz-scheduler.org CronExpression 章节）+ 仓内逆向锚**（console-ui.md §3.9/§3.10 三例表达式、replication.md §2.1 cronExp/§9.2-C-9 fake cron）先行定**机制与子集骨架**；字面契约（拒绝形态逐字/名月周缩写集/`?` 互斥细则/年域收否终版/next-run 呈现语义）以 T-435 锚落盘为准，Accepted 后差异以勘误回填——ADR-0041/0043「定机制不定字面量」同款条款，见「软缝对齐清单」）
- 日期: 2026-09-03
- 背景: FR-150（M16 Q1 用户终裁引入——**推翻 M15 Q5 终裁与 T-402a 勘误「事件驱动唯一引擎」**，PRD §5.4 断言反转⑦；ADR 本体承载 PRD §4.10 全项）。三消费面行为依据（clean-room 锚，全部在册）：console-ui.md §3.9（维护面三 cron 槽——Garbage Collection / Cleanup Unused Cached Artifacts / Cleanup Virtual Repositories 各带 Cron Expression + Next Run Time + Run Now；Compress Internal Database / Prune Unreferenced Data 为即时钮 + Quota 百分比）+ §3.10（备份实体 CRUD——列 Key/Repositories/Cron Expression/Next Schedule Backup/Enabled，表单含 Next Backup Time 可写位；**OSS 预置 backup-daily `0 0 2 ? * MON-FRI` / backup-weekly `0 0 2 ? * SAT`**）+ replication.md §2.1（ReplicationBaseDescriptor.`cronExp` string 高置信）+ §9.2-B-4（全局封锁对 cron 轨与事件轨**同时**生效——两轨并行独立的官方互证）+ §9.2-C-9（fake cron `0 0 12 1/1 * ? *`——**七域形态在案**，年域 `*` 收尾）。现役基座勾稽（读码定稿，2026-09-03）：replication Engine = 事件驱动（repo.Service.Put 链末 Enqueue 种 pending 行）+ `Run` 循环 wake + 1min sweep（`DefaultSweepInterval = time.Minute`，engine.go:67/427-441）单 worker——ADR-0021/0041 队列模式本体；**`Engine.TriggerFullSync`（M15 T-420）已是全量对账的既成载体**（种子 pending 行 + wake，包注释明文「deliberately no second executor, no new queue and no engine rework」）；`repo.CleanupEngine` / `repo.TrashEngine` 各有小时级 `Run` 循环 + `RunOnce` 载体（cmd main.go:925-943 生命周期装配先例——licenseMgr「无 drain 语义」家族）；GC = `POST /api/v1/system/gc` 同步执行（ADR-0015 勘误：`{apply, graceHours?}`）+ **data 目录维护锁族互斥**（gc/export/import/cleanup 共锁，ADR-0015 勘误③/ADR-0031 在途持有集）；export/import = CLI-only（ADR-0015 勘误二——`/api/export/**` REST 404 有意不兼容）；busy 二次机会预算（T-423 `RetryOnBusy`——metadata 写竞争统一缓解，15s BusyTimeoutMs 之上）；audit.Recorder 词表闭集（`Actions()`）+ replication push 域 audit 词在案（auditPush）；migration 水位 019（sqlite as-built 先行、PG 翻译随启用窗——018/019 同款现状）；`ActionDownload` 审计事件已是下载面的统一着落点（generic service.go / dockerremote / remoteexternal / virtual 各着陆点每 GET 一行）。约束：零新外部依赖（ADR-0005——解析器选型见轴 3）；零新配置键姿态（ADR-0040/0043——tick 间隔/表达式长度门/每域并发 = 引擎内部常量）；NFR-P74（next-run 与表达式对拍 + 到点精度 ±分钟级 + 零重复投递 + 误触发防护）/ NFR-S79（子集校验零注入 + 调度 CRUD 门 + audit 三事件）；**T-446 硬 AC「webhook/replication outbox 引擎文件 diff=0」**（PRD §1.3 派单纪律 + R7 风险缓解——调度域不得重造 replication 引擎/回收站 sweep/busy 预算三者的触发器）；FR-150 三票拆分（T-446 引擎 / T-450 三消费面 BE / T-462 FE）须接口可分可并行；三消费域 REST/wire 面互异（维护面设置 / 备份实体 CRUD / 复制配置内嵌字段）。
- 候选方案: （六轴）
  - 调度台账载体: A) **统一 `schedules` 台账表**（domain/key/表达式/next_run/last_run/last_status/enabled——PRD 150.1「schedule 实体五属性 + 所属域」的直接落形）+ **域注册面**（消费方注册 Runner）；B) 域内嵌列（maintenance 设置表三槽 + backups 表 + replications.cron_exp 列——三处各自维护 next_run/last_run 列）；C) 无台账（binflow.yaml cron 段或纯内存——配置文件承载调度）。
  - 触发器挂点: A) **`internal/scheduler` 独立 `Run` 循环**（1min ticker + wake——replication Run 同构新实例，cmd 生命周期段装配）；B) 挂接 replication Engine 的 1min sweep（顺带驱动调度）；C) 每域自跑（trash/cleanup 复刻形态——三域三循环各算 cron）。
  - cron 解析器: A) **自研 Quartz 子集解析 + `Next(expr, after)` next-run 纯函数**（闭集校验、参数化零注入、table-driven 对拍）；B) robfig/cron 外部库（MIT、维护中——但 **Vixie 方言非 Quartz**：`?`/`L`/年域语义不对位，Artifactory 表达式须预处理翻译，方言缝即缺陷缝）；C) 简化五域 crontab（分时日月周——与 Artifactory 六域秒级表达式不兼容，复制粘贴即拒）。
  - 复制全量载体: A) **复用 `Engine.TriggerFullSync` 同载体**（PRD 150.2 原文「与 Replicate Now 同任务载体幂等收敛」）；B) scheduler 自建全量执行器；C) 调度器回环 HTTP 调 `_trigger` REST 面。
  - 备份载体: A) **备份实体表（payload）+ schedules 行（domain=backup）+ export 引擎 callable 化**（CLI 命令内核函数化——单一载体两个入口〔CLI 与调度〕）；B) 调度器 exec CLI 子进程；C) 仅存 cron 配置不执行（呈现层调度）。
  - 补射与并发策略: A) **每域并发 1 + 错过窗坍缩**（missed > 1 tick 只补触发一次、next 自 now 重算——防停机补射风暴）；B) 补射全部错过点；C) 过期即跳过不补（静默丢调度）。
- 决策: **六轴全 A**。要点十三条：
  1. **包边界与依赖方向（internal/scheduler 定案——M16-SPLIT「新包定名从 ADR-0044」兑现）**：新包 `internal/scheduler`，**只 import metadata 的 ScheduleStore 公共面 + 各域导出的载体方法**（TriggerFullSync / RunOnce 族），零反向依赖、不摸内部结构、**不 import repo 的内容写路径与 webhook Bus**（并存语义点 2 的结构性保证）。文件布局 = 三票 area：`cron.go`（子集词法与解析——纯函数零 IO 零 DB，T-446）、`next.go`（next-run 计算器——纯函数）、`scheduler.go`（Run 循环 + 域注册面 + 防护 + audit/metrics facet + `Kick(domain,key)` 手动触发等价缝〔测试加速——与到点同 dispatch 路径〕，T-446）；metadata 侧新缝 `substores_schedules.go`（ScheduleStore——webhook store 先例）；httpapi `schedules.go`/`backups.go` + maintenance/replications 面扩字段（T-450）。域闭集 = 代码常量（`maintenance | backup | replication` 三域——新域 = 增量注册行，schema 零变化，ADR-0041 事件闭集同款纪律）。
  2. **schedules 台账 DDL（migration，sqlite 先行——编号先到先得惯例）**：
     ```sql
     CREATE TABLE IF NOT EXISTS schedules (
       domain      TEXT NOT NULL CHECK (domain IN ('maintenance','backup','replication')),
       key         TEXT NOT NULL,               -- 域内实体键：maintenance=作业槽名(gc|cleanup-unused-cache|cleanup-virtual)；backup=备份 key；replication=复制配置 id(TEXT 化)
       cron_expr   TEXT NOT NULL,
       enabled     INTEGER NOT NULL DEFAULT 1,
       next_run_at TEXT NOT NULL,               -- RFC3339 UTC；''=未排（disabled）
       last_run_at TEXT NOT NULL DEFAULT '',
       last_status TEXT NOT NULL DEFAULT '' CHECK (last_status IN ('','ok','failed')),
       last_error  TEXT NOT NULL DEFAULT '',    -- 失败摘要，截断上限沿用 errorTextLimit 族
       created_at TEXT NOT NULL, created_by TEXT NOT NULL,
       updated_at TEXT NOT NULL, updated_by TEXT NOT NULL,
     ...
     ```
     **「无行 = 不调度」单态语义**：cronExp 清空/实体删除 → 台账行同事务删除（复制配置 DELETE 联动删行；PUT 空 cronExp = 删行）——不引入「有行但永不触发」的歧形态。**不预置**：Artifactory OSS 预置 backup-daily/backup-weekly 是其发行默认，BinFlow 取零静默写盘纪律——调度默认全空，用户显式建（**有意差异登记**，architecture §10/§25.7——安全默认优先于发行形态对齐）。维护面三槽首配前 = 无行，GET 投影缺省态（enabled=false/cron 空）。
  3. **cron 表达式子集（Quartz 对拍骨架——字面细则随 T-435 锚）**：**六域全收（秒 分 时 日 月 周）+ 可选第七年域且仅收 `*`**（replication.md §9.2-C-9 七域形态直证——Artifactory 文档示例普遍带 `*` 年尾，拒收即复制粘贴碎）。收：`*`、`?`（日/周域互斥占位——互斥细则随锚）、`,` 列表、`-` 区间、`/` 步进（含 `1/1` 形态）、月名 JAN–DEC / 周名 SUN–SAT（大小写不敏感——细则随锚）、`L`（月末 / `nL` 该月最后周几——月末备份刚需，§3.10 预置形态邻域）。**拒**：`W`（最近工作日）、`#`（第 n 周几）、具体年域、非六/七域域数、闭集外任意字符 → **400 且 message 点名拒绝形态**（诚实子集——拒绝文案逐字随锚）。表达式长度门 = 引擎内部常量（零新配置键——NFR-S79 注入面：解析纯字符串→结构体，无 eval、无拼接，注入面结构性关闭）。
  4. **next-run 计算器契约**：`Next(expr string, after time.Time) (time.Time, error)`——纯函数、UTC、秒分辨率、确定性（同输入同输出——对拍断言可表驱动）；**PUT 期校验 = Parse + Next(now) 必须出值**（不可达形态 → 400）；触发后重算 `Next(now)` 落库。到点精度 = tick 量级 ±分钟级（NFR-P74——tick 1min 内部常量）。
  5. **触发器 = 独立 ticker（轴 2-A）**：`scheduler.Run(ctx)` 1min tick + wake（手动 Kick 唤醒）；每 tick 查询 `enabled=1 AND next_run_at <= now` → 按 domain 分组 → 域内串行 dispatch。**启动坍缩**：boot 扫描过期行 → 标记坍缩补触发一次（next 自 now 重算）——停机多窗只补一枪，防补射风暴（轴 6-A）。否决 2-B（挂 replication sweep）：跨域寄生耦合两域生命周期 + 违反「引擎文件 diff=0」红线（Run 循环加挂点即改引擎文件）；否决 2-C（每域自跑）：cron 解析/next-run/防护 ×3 重复，且维护面三槽无域实体可挂（cleanup 引擎是 period_hours 语义非 cron——T-324 as-built 语义不翻）。
  6. **域注册面（消费方注册——Go 契约）**：
     ```go
     // internal/scheduler
     ...
     // Register 加入域闭集（未知域/重复注册 → 装配期 error）；cmd 装配三域：
     // maintenance → gc/cleanup RunOnce 族；backup → export RunOnce；replication → TriggerFullSync。
     func (s *Scheduler) Register(domain string, r Runner) error
     ```
     每域并发上限 1（域内串行——维护域另由 data 目录维护锁结构性互斥 gc/export/import/cleanup 族，ADR-0015 勘误③；复制域的种子型 Run 秒回、实际推送由引擎既有单 worker drain）。
  7. **三消费面 REST 形态（T-450 消费——本 ADR 定归属与骨架，字段名细节票内与 FE 对齐）**：① 维护面 = `GET/PUT /api/v1/system/maintenance`（三 cron 槽 + Quota 百分比 + Compress/Prune 即时钮挂靠——cron 字段写台账、Run Now 各自既有面 `POST /api/v1/system/gc`/cleanup 维持并存）；② 备份 = `PUT/GET /api/v1/system/backups`（列表）+ `GET/PUT/DELETE /api/v1/system/backups/{key}`（**PUT upsert-by-key——Artifactory 官方备份面语义对齐**，路径走 BinFlow `/api/v1` 前缀惯例〔ADR-0008/§7.1〕；body 承载 payload + cronExp + nextBackupTime）；③ 复制 = replications 既有 CRUD body 增 `cronExp` 字段（§2.1 对位，空 = 不调度）+ GET 回显与 next-run 投影（台账读）；④ 只读投影 `GET /api/v1/system/schedules?domain=`（FE next-run 倒计时/失败态/禁用态——T-462 消费；system:read 门）。**ADR-0015 勘误二边界不破**：`/api/export/**` 维持 404——备份调度面是 `/api/v1/system/backups` 自有面，**import 维持 CLI-only**（写面高危带外——安全底线条款延续）。
  8. **并存语义（ADR 核心命题——断言反转⑦的边界补偿，五条）**：
     ① **调度触发面 = 全量类任务闭集**：{GC 全量（apply 载体 + grace 兜底不缩短——ADR-0015 在线触发安全边界延续）、Cleanup 两族全量 pass（RunOnce）、备份 export（RunOnce）、复制全量对账（TriggerFullSync）}。闭集外任务调度永不触发（增量推送/属性同步/webhook 投递/回收站清扫——均事件驱动或域内策略 sweep 的既有领地）。
     ② **事件驱动 + outbox = 增量唯一引擎不迁**：replication `Enqueue` 与 webhook `Bus.Emit` 织入面、引擎文件 **diff=0**（T-446 硬 AC——scheduler 只 import 复制包导出的 TriggerFullSync，不摸引擎内部；本 ADR 点 1 的 import 禁令是结构性保证）。
     ③ **复制全量调度 = 与 Replicate Now 同载体**：种子 pending 行 + wake——零第二执行器、零新队列；重试/退避/revive/协议推面/属性携带/ledger 可观测全沿用（T-420 trigger.go 契约原文）。
     ④ **零重复投递边界（三层口径——L48/T-450 断言据此写）**：职责层——调度只发全量、事件只发增量，无重叠职责面（Artifactory cron 轨与事件轨并行同构，replication.md §9.2-B-4「封锁对两轨同时生效」互证双轨独立）；任务层——**允许并存**（全量种子行与事件增量行可同窗共存——TriggerFullSync「重复触发不合并」既有豁免，ADR-0021 §11.28 同族）；内容层——**零重复**（目标侧 sha256 幂等命中：blob 已在即不重传字节——秒传语义）。**L48 断言口径 = 内容层**（目标实例 blob 集不增 + 无重复字节传输），非任务层（任务行允许并存）；调度窗内增量事件**不抑制不互斥**——双轨各走各的，收敛交给幂等。
     ⑤ **载体行为 diff=0**：调度触发的 GC/备份/cleanup/复制与手动触发走同一载体同参数面（载体的 audit 词/维护锁/webhook 行为零变化）——调度域只加自己的 audit 词（点 10），不改任何载体的对外行为。
  9. **误触发防护（三面——NFR-P74/S79）**：解析期 = ①闭集校验 400（子集外形态点名——零注入）②**过去时间拒配**：显式 next-time 字段（备份面 nextBackupTime——§3.10 表单可写位在案）在过去 → 400 ③next-run 不可达（永不触达形态）→ 400。运行期 = ①每域并发 1（域内串行 + 维护域锁结构性互斥）②错过窗坍缩（只补一枪）③时钟回拨防护（`now < last_run` 的行跳过 + WARN；next_run 在未来的行不触发）。管理面 = 调度 CRUD **同三消费面既有门**（GC/备份/维护 = system:write——readonly_admin 403，ADR-0026 GC 全路由先例；复制 cronExp 随复制配置面门）——**零新权限通道**。
  10. **audit 词表 +9 与 metrics**：`maintenance.schedule.{set,run,fail}` / `backup.schedule.{set,run,fail}` / `replication.schedule.{set,run,fail}`——set = 配置变更（detail 含 cron/next_run）、run = 到点执行成功（key/耗时）、fail = 执行失败（last_error 截断）；载体自身 audit 词（gc.trigger/replication.push 族）照常在场——三事件是调度层视角，不替代载体词。metrics：`binflow_scheduler_fires_total{domain}` / `binflow_scheduler_failures_total{domain}`（expvar 自研，ADR-0022 姿态）；启动 INFO 一行台账水位（enabled 行数 + 最近 next-run——ADR-0040/0041 同款）。
  11. **配置与生命周期**：零新配置键（tick 1min/表达式长度门/截断上限 = 引擎内部常量）；`scheduler.Run(ctx)` 由 cmd 装配（main.go 生命周期段——licenseMgr/cleanupEng/trashEng 先例）；优雅停机 = ctx cancel → 在途 Runner 完成或载体自身取消语义（复制种子型秒回；GC/备份 RunOnce 与维护锁/ctx 协作）；next_run 已落库，重启坍缩规则接管（点 5）。
  12. **K68/K69 双会签同场**：见下两节（K68 = ADR-0026 动作面增补评审；K69 = nodes 统计扩列会签——详规落 architecture §25.5，本 ADR 存决策记录）。
  13. **FR-150 三票拆分边界确认（AC3——可实现粒度成立）**：T-446 = `internal/scheduler`{cron.go,next.go,scheduler.go} + schedules migration + ScheduleStore + Runner/Registrar + 防护 + audit 九词 + Kick 缝——**纯新包零既有引擎文件改动**（cmd 装配行例外）；T-450 = 三消费面 REST（maintenance/backups/replications cronExp）+ 三载体接线（gc handler 内核 callable 化 / cleanup RunOnce 复用 / TriggerFullSync 复用 / export CLI 内核函数化）+ set 词接线；T-462 = FE 纯消费。交接缝 = Runner 注册 + schedules 台账 + `GET /api/v1/system/schedules` 投影——每票各自 table-driven 可验证，串行链 review 面局部化（ADR-0043 四票拆分论证同款）。
- 理由: 六轴共同判据 = 「载体复用非重构 + 台账单源 + 闭集可验证 + 与既有先例同构」。台账轴 A 是 PRD 实体五属性的直接落形，且一个 `(enabled, next_run_at)` 索引承担全部触发查询——B 把 next-run/last-run/状态三组列 ×3 域复制，维护面三槽还须凭空建设置实体；C 使调度不可 CRUD/不可观测（备份 CRUD 面无从谈起）。挂点轴 A 与 replication/trash/cleanup 的「每引擎自有循环」形态同构，生命周期独立可停机；B 的寄生耦合直接违反引擎 diff=0 红线。解析器轴 A：Quartz 子集即 ~300 行纯函数 + 闭集校验，方言精确对齐（robfig 的 Vixie 方言差是结构性的——`?`/`L`/年域三处不对位即三处缺陷缝，ADR-0005 依赖准入过不了「唯一实现且不可替代」的收益检验）；ADR-0043 自研 EBNF 先例（闭集可枚举验证、拒绝形态诚实点名）同族。复制载体轴 A 是 PRD 明文且载体已在产（T-420）——B 重造执行器即断言反转⑦警告的「双引擎」，C 的进程内 HTTP 回环引入认证/超时两个新故障面。备份载体 A 单载体双入口（CLI/调度）零协议面；B 的子进程形态引入 exec 依赖与部分失败不可观测；C 是呈现层假调度（违「无数据源不伪造」同源纪律）。补射轴 A 的坍缩是「停机后补一枪」与「补射风暴」之间唯一同时满足运维直觉与安全边界的取值；C 的静默丢调度在备份场景不可接受。
- 后果: `internal/scheduler` 新包（三票 area）+ migration（schedules 表；K68/K69 各一迁移——编号先到先得）；metadata +ScheduleStore 缝；httpapi maintenance/backups/schedules 三面 + replications cronExp 字段 + permissions 词表臂（K68）+ properties 判 a（K68）；audit +9 词；metrics +2；cmd 装配 + 优雅停机接线；architecture §6（nodes 增列）/§10（对齐表两行）/§11.48~50/§25（M16 增量节）修订；tech-writer cron 调度与定时备份指南（T-458 腿②）；技术债登记 §11.48（statisticsEnabled 通知本体缺位——smart-remote 域）/§11.49（StatsInfo 无源两字段）/§11.50（调度域 W/# 不收子集边界——翻转 = next.go 单点扩展）。
- clean-room 边界: 行为基准 = JFrog 官方文档（备份面 REST 语义/维护面 cron 形态/GC cron——T-435 锚与官方页为准）+ **Quartz 官方公开文档（quartz-scheduler.org CronExpression 章节——公开规范，非反编译产物）** + docs/reverse/console-ui.md §3.9/§3.10 + replication.md §2.1/§9.2（仓内高置信锚）；**QuartzTrigger/CronExpression 的 Java 实现零参考零翻译**（ADR-0001）——BinFlow 的台账 schema/解析器/next-run 计算器/域注册面/ticker 全自有设计；Artifactory OSS 预置 backup-daily/weekly 的发行默认**不对齐**（零静默写盘纪律，有意差异登记）。
- **K68 会签（ADR-0026 增补评审——Q7 终裁「加 Annotate」的架构落法；LC-88 归 A）**：
  - 命题三件：①动作闭集扩列（annotate）②wire 动词翻新（write→deploy-cache——Artifactory 权限矩阵五列 Read/Deploy-Cache/Delete/Annotate/Manage 的对位）③存量权限数据迁移（零提权）。
  - 候选: A) **`ActionAnnotate = "a"` 入 Can 闭集 + wire 词表翻新 + 存量 write 行回填 annotate（等价迁移）**；B) annotate 不入闭集、属性写门走独立第二判定通道（properties handler 专用）；C) 仅翻 wire 呈现词不拆语义（annotate = write 别名——伪拆分）。
  - 决策: **A**。要点七条：
    1. **动作闭集 r/w/d/m → r/w/d/m/a**（auth/api.go 常量 +1）。语义 = 属性写权限：M10 属性系统三动词对位——GET properties（读，随 item-info 的 r 门**不变**）/ PUT properties（→ a）/ DELETE properties（→ a）（§15.3.3 的 allowPropsWrite 从 ActionWrite 改判 ActionAnnotate——单点 properties.go:327，翻转面隔离在 handler 单点）。annotate 不开任何内容字节面（上传/落地仍 w、删除与覆盖检查族仍 d——「属性是元数据非内容」的 §15.3.3 分界延续）。否决 B：第二判定通道 = 双真相源（ADR-0043 ACL 轴同款论证——分叉即泄漏面）；否决 C：伪拆分骗不过 403 探针（无 a 位用户必须真被拒）。
    2. **wire 词表翻新**：PUT/GET `/api/v1/permissions` 动作词 `{read,write,delete,manage}` → `{read,deploy-cache,annotate,delete,manage}`。**PUT 双词收一轮**（`"write"` 收为 deploy-cache 别名——存量脚本/迁移工具不破；M17 移除评估）；**GET 回显正名 deploy-cache 单形**（canonical 单形——兼容臂只在收不在发）。**DB 列 can_write 不改名、内部码 `w` 不变**——deploy/cache 是合并列语义（Artifactory Deploy/Cache 一列同构）；「拆分」拆的是**属性写（annotate 出走）**，不是部署与缓存分离。`?permissions` 视图（SE-08）字母集 +a（w 字母维持——内部 compact 码与 wire 词分层，quotaBytes→quota_bytes 先例同族）。
    3. **迁移映射（零提权等价迁移）**：migration `+can_annotate` 列（默认 0）+ **存量 `can_write=1` 行回填 `can_annotate=1`**——write 原语义覆盖属性写（§15.3.3 as-built：属性写门 = w），拆分时存量授权跟随两半。效果 = 迁移前后全量 (principal × repo × path × 动作) 有效权限矩阵**逐位相等**——零提权 ⊇ 零回归（NFR-S77 与 T-444「既有动词行为零回归」同时成立的唯一形态）。新建 target 可只授 deploy-cache 不授 annotate（新表达力——Artifactory 五列同构）。
    4. **dry-run 报告口径**：枚举 permission_principals 全行 → 映射表 {行 → deploy-cache（can_write 原值不动）+ annotate（回填 = can_write）}；断言 = **映射率 100%**（每行确定映射、零无主行）+ 回填集恒等于 write 集（`annotate_after ⇔ write_before`——双向包含）。回滚 = can_annotate 列弃用（DROP 或保留零消费）+ wire 词翻回 write 别名位——**可回滚**。
    5. **零提权断言口径（机器形态——L39/NFR-S77）**：M7 RBAC 全量 spec 复跑（角色 × 动作 × 端点门矩阵）+ 迁移前后矩阵 diff=0 + 新增探针三臂：无 a 位用户 PUT properties → 403；有 w 无 a 用户（新建态）PUT properties → 403（拆分语义在位——w 不再隐含属性写）；w+d 用户上传/删除行为零回归。
    6. **角色与门位**：admin 旁路放行 a（Can 短路——与 r/w/d 同）；readonly_admin 对 a 恒拒（内容面 w/d/m 恒拒闭集延续）；**m ↛ a**（无提权链不变量延续——m 是仓库配置权非内容权，ADR-0026 决策 4）；docker token scope 词表**不收 a**（ADR-0026 决策 5「m 永不出 scope」同款——/v2 无属性面）。
    7. **ADR-0026 增补评审留痕 + Property Set 裁定**：本节构成 ADR-0026 动作面的增补评审——闭集纪律（新增 = 架构变更）经本 ADR 记录在案；ADR-0026 决策 1（角色闭集）/3（m 语义）/5（两臂一致性）零翻动；architecture §10 对齐表「annotate 不做」行随本 ADR 翻转为收录。**Property|Property Set 分段（T-447 候裁臂）：裁不做**——BinFlow 无属性集域（M10 属性系统无 set 概念），裁做须 BE 属性集配置小域另立票（PM FR + ADR），M16 缺位登记（「无域不伪造」纪律）。
- **K69 会签（per-node 下载统计基建——LC-87 归 C〔自有基建、字段呈现 A 对位〕；详规 architecture §25.5）**：
  1. **nodes 四列**（migration ALTER ×4）：`download_count` / `last_downloaded_at`（RFC3339，空串 = 从未）/ `last_downloaded_by`（空串 = 从未；匿名下载 = `"anonymous"`）/ `remote_download_count`——folder 行恒零值不计。**四列 = 批次③字段族与 FR-148 usage 域的唯一计数源（单源契约——T-438 票内声明、T-440 复核、QA 中期断言；零第二通道）**。
  2. **三分计数口径**：①直连 = 节点所在仓的 GET；②经 virtual = virtual 解析命中成员——**计数落成员节点行**（virtual 无节点行，ADR-0043 口径）；③remote 缓存命中 = remote 仓节点的 GET（含未缓存先回源后服务）。三臂皆 `download_count+1` 且刷 `last_downloaded_at/by`；臂③另计 `remote_download_count+1`（remote_downloads 语义 = 该 remote 仓作为服务点的下载次数——local 仓该列恒 0）。**计数判定与 audit `ActionDownload` 事件同源同址**：凡记 audit download 的下载面即计数（generic/docker/remote/virtual 各着陆点——HEAD/探测不计）；audit 行与四列**同批写**（单源纪律的结构性保证）。
  3. **埋点单点与性能**：repo 服务下载路径各着陆点单条 `UPDATE nodes SET download_count = download_count + 1, ...`（SQL 自增原子——SQLite 单写者/PG 行锁）+ T-423 busy 预算包裹；下载面已在每 GET 一次 audit 写的量级，列增同批零第二事务（NFR-P71 零可感知尾延的结构性保证 + 前后对比兜底）。
  4. **stats wire 面 = `GET /api/storage/{repo}/{path}?stats`（StatsInfo）**——rest-api.md §（storage 段 ?stats 行）高置信锚：官方七字段 `downloadCount/lastDownloaded/lastDownloadedBy/remoteDownloadCount/remoteLastDownloaded/remoteLastDownloadedBy`(+uri)；**BinFlow 投影前四 + uri；无源两字段（remoteLastDownloaded/remoteLastDownloadedBy）缺位不投影**（加列后路 = 小迁移票，§11.49）。基础 FileInfo 零变化——**T-438 AC1 的「FileInfo 投影扩字段」按此锚正位为 ?stats 面**（差异留痕：tech-lead 派单附注 + T-438 票内核对）。
  5. **`last_downloaded_by` 可见性门（K69 定案）**：默认档 = **CapSystemRead 同源（admin ∨ readonly_admin）——与审计日志 GET `/api/v1/audit` 同档**（PRD K69「与 audit 同档」口径的读码兑现：router.go 该面 = manage: CapSystemRead）；非档位调用者 ?stats 投影**省略** lastDownloadedBy 字段（omitempty）；downloads/lastDownloaded 计数面无门（计数非身份信息）。AQL statistics 域的 downloaded_by 同门（非档位 → `"unknown"`——ADR-0043 Errata ⑧ created_by/modified_by 脱敏同款）；m-holder（CanManageRepo）加档 = **放宽项候 Q 裁**（K69 留缝不动——放宽须裁的 PRD 原文边界）。
  6. **statisticsEnabled / sourceOrigin 行为化裁定**：官方语义取证（JFrog 官方 smart-remote 文档 + provider 文档）：`contentSynchronisation.statisticsEnabled` = **向上游 Artifactory 实例通知下载统计**（smart remote——上游须为 Artifactory 系且开放同步，默认 false）；`sourceOrigin` 官方族 = sourceOriginAbsenceDetection（上游缺位检测）——**两者均系 smart-remote 域**。BinFlow M16 行为化 = **本地最小诚实**：①四列计数**无条件**（本地统计是本地事实，不受同步键门控——与官方本地统计口径对位）；②statisticsEnabled 行为化 = `enabled && statisticsEnabled` 时 remote 服务臂的 audit download 行 detail 附 statsSync 资格标记（双 BinFlow 链路未来通知端点的数据面预备）；**通知本体不做**（上游通知端点属新 API 面——§5.3 零私加 + PM FR 流程，§11.48 缺位登记）；③sourceOrigin「落库」= audit download 事件 detail 增 `origin` 维度（`direct | virtual:<vkey> | remote`——三分口径审计轨迹；nodes 零新列）。
  7. **AQL 字段族对位**（aql.md statistics 预锚——增量段 T-435 定案）：四列承载 `downloads/downloaded/downloaded_by/remote_downloads`；`remote_downloaded`（时间戳）/`remote_downloaded_by`/`remote_origin`/`remote_path` 无源 → 注册表 unsupported 诚实拒绝（后路 = 加列小票 + 勘误——ADR-0043 注册表「无源不伪造」条款同款）。
- **软缝对齐清单（T-435 cron 锚落盘后 Accepted 复核，分歧走勘误段——机制轴零翻动；T-435 在途，本清单即差异核对协议）**：① 域数/年域终版（本 ADR：六域 + 年域仅 `*`——七域 `0 0 12 1/1 * ? *` 在案支撑）；② `?` 互斥细则（双 `?`/双具体值的拒绝形态）；③ 名月周缩写集与大小写策略；④ `L`/`nL` 语义边界与支持域；⑤ `W`/`#` 拒绝文案逐字 + 400 envelope 形态；⑥ next-run 呈现语义（服务端台账权威——若锚定客户端计算则呈现层差异留痕）；⑦ 备份 wire 字段名（backupKey/cronExp/nextBackupTime——官方备份面锚归 T-450 消费时核对）；⑧ statistics 字段族定名（aql.md 增量段 vs 本 ADR K69 点 7 对位表）。效力序：用户裁决（BOARD）> T-435 锚（docs/reverse/）> 本 ADR > PRD 暂行值。
- **验收锚点骨架（T-446/T-450——PRD FR-150 AC 对位）**：T-446 = 子集解析表驱动（合法/边角/拒绝三臂——`1/1`/`MON-FRI`/`L`/七域 `*` 年尾全覆盖）+ next-run 对拍（表达式手工展开对拍表——NFR-P74）+ Kick 等价腿 + 坍缩/回拨/域并发三防护臂 + **replication/webhook 引擎文件 diff=0 审计**；T-450 = 备份 CRUD 全链（PUT upsert/GET 列表/DELETE——next-run 回显）+ GC cron 保存-回显-到点腿（手动 Kick 等价）+ 双实例复制 cron 腿（T-420 形态复用——目标逐路径 sha256 一致 + **内容层零重复**〔目标 blob 集不增〕+ Replicate Now 并存幂等）+ audit 九词在场 + 过去时间/畸形表达式 400 臂 + readonly_admin 调度 CRUD 403。

---

## ADR-0045: Build-info domain — internal/build new package, normalized builds table family with nodes linkage, repo-level allow() same-source ACL (path plane carries build_name; no standalone build permission face), promote gate = w(targetRepo) ∧ r(buildRepo), Emit/search weaving via import-ban boundary; community floor (no addon slot)

- 状态: Accepted（2026-09-06，M17 W0 T-489 交付；与 T-488〔build-info.md 规格票〕W0 并行在途——本 ADR 定**机制与 Go 侧契约**，REST 端点字面形态（路径/方法/请求响应体/错误文案/build JSON 字段集/promotion 状态机字面/retention 参数形态）以 build-info.md 冻结为准，Accepted 后差异以勘误回填——ADR-0041/0043/0044「定机制不定字面量」同款条款，见「软缝对齐清单」。K74 的 ACL 模型项由本 ADR 关闭）
- 日期: 2026-09-06
- 背景: FR-152（M17 主轴 P0）：Build-info 域全链（模型→REST→promote→webhook wired→AQL 域→FE 页，六票 T-507~T-512 串行链 = 本程决定性路径）。行为依据（clean-room 锚，全部在册）：**JFrog 官方 REST 文档为唯一行为基准**（Build Info 域——webhook.md/aql.md 先例）+ inv-2 §103（Build CRUD 端点族——随产品发布 openapi.yaml 996 行：GET 全量/PUT 上传/DELETE 批删/多删/rename/append/retention/promote/diff；promotion 支持 copy/move+scope+properties+dryRun+failFast；Docker 镜像提升独立端点）+ inv-4 D1~D5（数据模型：BuildRun〔name/number/started〕、Module〔artifact/dependency 列表带 sha1/md5〕、promotion 历史；`PUT /api/build` 全量/增量保存 + `append` 增量合并；promotion 参数族 targetRepo/status/ciUser/properties/dependencies/failsOnMissingArtifacts；retention 按数量/天数/buildNumbers/minimum retained days——高置信）+ webhook.md §3.4（build 域三事件休眠中：uploaded/deleted/promoted，data = build_name/build_number/build_started/build_repo——**envelope 字段已冻结**）+ aql.md §2.1（build 系五入口现 400「远期 dep Build-info 域立项」行——T-511 翻转点）+ console-ui.md（Builds 页形态）。现役基座勾稽（读码定稿，2026-09-06）：`auth.Authorizer.Can(ctx, p, repoKey, path, action)` 公开单决策点（authorizer.go:32）；`repo.Service.allow` 私有镜像（service.go:218——admin 短路 + nil fail-closed + az.Can 委托）+ CanRead/SearchScope 只读投影（ADR-0043）；动作闭集 r/w/d/m/a（K68 后五动词）；`repo.Service` CopyOrMove 载体族（M12 操作域——§15.4.2，跨仓迁移的既有用户面管线，T-339/T-343/T-345 已验）；webhook 66 型注册表（eventtypes.go——build 域 3 型 dormant，dormant→wired = 常量行翻转 + 织入行，schema 零变化〔ADR-0041 决策 7〕）；search 字段注册表 + unsupportedDomains 闭集（fields.go:195——builds/build.properties/build.promotions/modules 等在列，翻转 = 移除 + 域字段注册）；migration 水位 023（annotate）；audit Actions() 词表闭集；expvar 自研 metrics（ADR-0022）。约束：PRD §1.4-5 ACL 零泄漏（T-92 血统——build 数据系新泄漏面最宽处，R5）；NFR-S80/S81（越权探针逐票硬 AC + promote 权限门）；NFR-P80（写入/查询 P95 ≤300ms 量级——单 build 十模块百依赖；promotion 大 build 迁移零 5xx）；ADR-0026 闭集纪律（新增权限概念 = 架构变更）；双方言义务（ADR-0007）；零新配置键姿态（ADR-0040/0043/0044）；六票拆分（T-507 模型/T-508 REST/T-509 promote/T-510 webhook/T-511 AQL/T-512 FE）须接口可分可串行。
- 候选方案: （五轴）
  - ACL 模型（K74 核心命题——「仓库级 allow() 同源 vs 独立 build 权限面」）: A) **仓库级 allow() 同源**——build 行携 `build_repo` 逻辑键，全部动作用既有五动词对该键经同一 auth.Authorizer 求值，**path 位传 build_name**（target 的 includes/excludes 模式对 build 名生效）；B) 独立 build 权限面——permission target 增 builds[] include/exclude 段（Artifactory 权限面形态对位：其 target 模型确有独立 builds 段）；C) 管理面能力门全收（CapSystemRead/Write 承载 build 域读写）。
  - 数据载体: A) **metadata 规范化表族 + payload 归档**（build 数据是记录不是制品——不进 storage/nodes 内容面）；B) blob-backed 隐藏仓（build JSON 落 storage + nodes 行——Artifactory artifactory-build-info 隐藏仓存储形态对位）。
  - promote 权限门: A) **w(targetRepo) ∧ r(buildRepo)**（既有动词零新增——deploy-cache 语义）；B) 新动词 p（promotion 专属）；C) m(targetRepo)（manage 语义）。
  - 织入面: A) **import 禁令式包边界 + 注入 facet**（build 禁 import webhook/search/scheduler/bundle/insights/license/replication/httpapi；webhook 经装配层注入 Emit facet；search 单向 import build 公开只读面）；B) build 直接 import webhook.Bus 与 search 引擎（编译期耦合——织入面 = build 包内调用）；C) webhook/search 反向查询 build（依赖倒置溢出）。
  - promote 载体: A) **复用 repo.Service CopyOrMove 族 + docker 面 manifest 重放**（T-509 明文「复用非重构」）；B) build 包自建跨仓迁移器（第二执行器）。
- 决策: **五轴全 A**。要点十二条：
  1. **包边界与依赖方向（internal/build 定案——M17-SPLIT「新包定名归 ADR-0045」兑现）**：新包 `internal/build`，只 import metadata（BuildStore sub-store 公共面——webhook store/ScheduleStore 先例）、repo（公开 Service：CopyOrMove 载体 + 节点查找 + CanRead 投影）、auth（Authorizer + Principal 类型）、audit（facet）、config（只读）。**import 禁令（review 依据，import 图即审计证据——ADR-0044 点 1 同款结构性保证）**：禁止 import `internal/webhook`、`internal/search`、`internal/httpapi`、`internal/scheduler`、`internal/bundle`、`internal/insights`、`internal/license`、`internal/replication`。文件布局 = 票 area：`model.go`（类型）+ `store.go`（metadata sub-store 缝）+ `service.go`（上传/append/查询/promote/retention 编排 + ACL 镜像）+ `promote.go`（T-509）+ `emit.go`（facet 定义——T-510 接线）+ metadata 侧新缝文件 `substores_builds.go`（T-507）；httpapi `build.go` 新 router 文件（T-508——dispatchAPI 显式路由族，ADR-0034 管理面惯例）。
  2. **DDL 表族（migration 024 起——sqlite 先行、PG 翻译随启用窗，021~023 惯例；实际编号先到先得，与并行票冲突顺延）**：
     ```sql
     builds(          build_name TEXT, build_number TEXT, started TEXT, build_repo TEXT NOT NULL,
                      created_by TEXT, created_at TEXT, updated_at TEXT,
                      PRIMARY KEY (build_name, build_number));
     build_modules(   build_name, build_number, module_id TEXT, module_type TEXT,   -- module_id = Module ID 字段（T-512 消费）
                      PRIMARY KEY (build_name, build_number, module_id));
     build_artifacts( build_name, build_number, module_id, repo_key TEXT, path TEXT,
                      name TEXT, sha256 TEXT,
                      PRIMARY KEY (build_name, build_number, module_id, repo_key, path),
                      FOREIGN KEY (repo_key, path) REFERENCES nodes(repo_key, path));  -- build ↔ 制品/镜像工件关联（nodes 真引用——镜像工件 = docker 仓 manifest 节点路径）
     build_dependencies( build_name, build_number, module_id, seq INTEGER,
                      dep_type TEXT, dep_name TEXT, dep_version TEXT, scopes TEXT, sha1 TEXT, md5 TEXT,
                      PRIMARY KEY (build_name, build_number, module_id, seq));      -- 依赖非制品——不要求 nodes 可解析（sha1/md5 对位 inv-4 D1）
     build_promotions( id TEXT PRIMARY KEY, build_name, build_number,
                      status TEXT, target_repo TEXT, ci_user TEXT, dry_run INTEGER,
                      params_json TEXT, promoted_by TEXT, promoted_at TEXT);         -- append-only 历史；现势状态 = 最新行（状态机字面照规格票）
     build_properties( build_name, build_number, name TEXT, value TEXT,
                      PRIMARY KEY (build_name, build_number, name));
     -- 索引：builds(build_name)（列表/最新）、builds(build_repo)（ACL 可见集过滤）、
     --       build_artifacts(repo_key, path)（制品反查 build——Module ID/AQL include 用）、
     --       build_promotions(build_name, build_number)
     ```
     **列集字面与 wire JSON 字段集以 build-info.md 冻结为准**（本骨架钉主键/外键/关联形态——软缝①）。build 原始 JSON payload 归档进 `builds` 表 payload 列或伴表（T-507 定形）——**不进 storage/nodes**（轴 2-A 论证：build JSON 是 KB 级记录，落 nodes 将把内容面〔搜索/浏览/复制/配额/GC〕全部卷入；备份语义随 metadata.db 完整——差异留痕 architecture §11.51）。
  3. **build_repo 逻辑键**：build 行归属的授权域键。缺省常量 `artifactory-build-info`（webhook.md §3.4 示例值对位；官方缺省核验归规格票——软缝②）；客户端可指定（参数形态照规格票）。**逻辑键不要求 repositories 行存在、不自动建仓**（零静默写盘纪律——ADR-0044 决策 2 同姿）；同 key 真仓存在亦无干扰（build 数据不经 nodes）。ACL target 匹配按键（permission target 的 repos[] 模式对该键求值——无需仓实体）。
  4. **ACL = allow() 仓库级同源（K74 定案；轴 1-A）**。求值形态：build 服务内嵌 allow 镜像（`p.Admin` 短路 + nil az fail-closed + az.Can 委托——与 repo/service.go:218 逐字同构）+ **同一装配注入的 auth.Service 实例**（同源由构造保证——单决策点纪律，ADR-0026 决策 5/ADR-0030 E9/ADR-0043 决策 4 一脉）。**path 位 = build_name**：target 的 includes/excludes Ant 模式对 build 名生效——「按 build 名模式授权」（Artifactory build 权限的核心语义）零新面获得；无模式 target = 整 build_repo 域。动作映射：上传/append = **w**(buildRepo)；查询/列表 = **r**（列表零泄漏 = **服务端过滤可见集**——按 (buildRepo, build_name) 逐行求值后返回，非客户端过滤；aql.md §6 锚「builds 域要求 name/number/repo 字段行级过滤」同口径）；删除/retention = **d**(buildRepo)；promote 见点 5。**否决轴 1-B**（独立 build 权限面）：B 的全部表达力（按 build 名模式授权）在 A 里由 path 位零新面可得，B 却需四处新增（target wire 加段 + schema 列 + FE 编辑器 + 存量迁移）并制造**第二条权限通道**——「双真相源分叉即泄漏面」（ADR-0043 ACL 轴否决 C 同款论证）；且新增权限概念触发 ADR-0026 闭集纪律（架构变更级）。**否决轴 1-C**：build 是 CI 数据面非管理面——PRD 场景 A 要求 CI 工程师（user 角色）自助上传，CapSystemWrite 门使主场景落空；readonly_admin 全读 build 域亦非必要。readonly_admin 内容面语义延续：r 恒放行、w/d 硬拒（角色短路，target 不参与——ADR-0026 决策 1）。
  5. **promote 权限门 = Can(targetRepo, "", w) ∧ Can(buildRepo, build_name, r)**（轴 3-A；NFR-S81 承载）。理由：promote 本质 = 跨仓部署（copy/move 载体执行），部署权即 deploy-cache（w）——与 CI 直接 PUT 目标仓同一权限位（「能 push 的才能 promote」——语义等价、零提权面）；**m 不承载**（manage 是仓库配置权非内容迁移权——ADR-0026 决策 3 分界）；**无新动词**（r/w/d/m/a 闭集维持——新增即架构变更）。targetRepo 校验：必须存在、必须 local（virtual/remote 不收——promotion 目标是实体仓；400/404 形态照规格票）；`dryRun` = 全链校验零落变更（inv-4 D3 参数族——targetRepo/status/ciUser/properties/dependencies/failFast/failsOnMissingArtifacts 字面照规格票）；缺制品语义（build_artifacts 悬空引用）= failFast 时 4xx 拒绝、否则跳过（细则照规格票）。docker 腿：targetRepo 为 docker 仓时经 repo.Service docker 面迁移 manifest + 关联 blob（docker_refs 对账——sha256 全量核对，删缓存复拉仍成功的结构性保证），细归 T-509。
  6. **REST 子集对位（M17 面——机制级骨架，字面照 build-info.md 冻结）**：
     | 官方族（inv-2 §103/inv-4） | BinFlow M17 形态（骨架） | 门 |
     |---|---|---|
     | Put Build Info（上传） | `PUT /api/build/{name}/{number}`（全量保存语义；重复上传覆盖/合并细则照规格票） | w(buildRepo) |
     | Build Append（增量合并） | `PUT /api/build/append/{name}/{number}`（**合并不覆盖**——模块/依赖段按 module_id 合并、依赖按 (id,seq) 去重追加以规格票冻结为准；方法/路径形态照规格票——inv-4 D1 与官方文档方法面有分歧迹象） | w(buildRepo) |
     | Get Build Info | `GET /api/build/{name}/{number}` | r |
     | Builds 列表/最新 | `GET /api/build/{name}`（numbers 族）/ `GET /api/build`（名清单）；「最新」= 最大 number 排序取首（分页/过滤参数照规格票） | r |
     | Delete Builds | `DELETE /api/build/{name}?builds=&dateRange=&artifacts=`（批删参数族照规格票） | d(buildRepo) |
     | Promotion | `POST /api/build/promote/{name}/{number}` | 点 5 |
     | Retention | `POST /api/build/retention/{name}`（保留窗参数族照规格票） | d(buildRepo) |
     **不进 M17（远期行登记）**：rename、diff、docker promote 独立端点（`docker/{repoKey}/v2/promote`——镜像晋升经 promote body targetRepo 直达 docker 仓承载，独立端点不另建）、projectKey 过滤族（BinFlow 无 projects）。错误面 = E-01 envelope 单形（400 畸形 body/404 不存在/403 越权——文案逐字照规格票核验样本）。
  7. **webhook 织入（T-510——断言反转④）**：build 域三事件 uploaded/deleted/promoted **dormant→wired**：① eventtypes.go 三常量行 Source 翻转（webhook 包内——schema 零变化，envelope 七字段与 data 四字段〔build_name/build_number/build_started/build_repo〕已冻结于 webhook.md §3.4，零发明）；② 发射点 = build.Service 成功路径与 audit 同址（上传/append 成功 → uploaded；删除/retention 成功 → deleted；promote 成功 → promoted）经**装配层注入的 Emit facet**（`emit.go` 定义 func 类型，cmd 装配 webhook.Bus.Emit——ADR-0041 决策 8 gate func 同款分层：internal/build 不 import webhook）；③ criteria 面 build scope（anyBuild/selectedBuilds/include-excludePatterns）= Bus 既有过滤器求值（domain 匹配事件类型 + criteria 对 build 名/仓过滤——机制既有，形态照 webhook.md）。
  8. **AQL/search 织入（T-511——断言反转⑥）**：`internal/search` **单向 import** internal/build 公开只读面（`BuildSearcher`——store 级查询接口，repo.Service 只读缝同族；禁止摸 service 内部）。承接面：fields.go 的 unsupportedDomains 移除 **builds / modules / dependencies 三入口** + build 域字段注册（builds.find 域字段集照 aql.md 增量段——T-488 冻结）；**promotion / releasebundle / sensitive 入口维持 unsupported 400 诚实拒绝**（promotion 数据面已备但查询面 M17 不开——M18+ 翻转点留痕；releasebundle dep ADR-0046 域本体；sensitive 无源）；`/api/search/buildArtifacts` + `/api/search/dependency` 端点随域落地。**ACL 织入**：builds 域查询过 BuildScope 服务端可见集（点 4 同源——SearchScope 的 build 域变体，返回可见 (buildRepo, buildName) 集）+ path-scoped 行复核（CanRead 同族——build.CanRead 投影）；越权 build 行零出现为 T-511 硬 AC。
  9. **promote 载体复用（轴 5-A）**：generic 制品经 repo.Service CopyOrMove 族（用户面管线既有——但 promote 是系统编排动作：SystemPrincipal 内部身份 + 照 trash/observer 先例豁免 license 门？**不豁免**——promote 由真实用户发起，principal = 调用者，目标仓 w 门已在点 5 把关；载体调用照 CopyOrMove 既有语义含 trash/observer/audit/webhook Emit 全链——制品搬动的事件〔artifact.moved/copied〕随载体照常发射，与 build.promoted 事件并存两域各表）。大 build 迁移零 5xx = 载体既有分批/busy 预算承载（NFR-P80）。
  10. **audit 词表 +5 与 metrics**：`build.{upload,append,delete,promote,retention}` 入 Actions() 闭集（actor/build 标识/targetRepo/结果——detail 截断惯例）；retention 删除逐 build 记 deleted 明细行（PRD「删除 audit」）。metrics：`binflow_builds_put_total` / `binflow_builds_get_total` / `binflow_builds_promote_total` + promote 耗时 histogram（expvar 自研——ADR-0022 姿态；命名族照 T-508 AC 口径）。
  11. **档位映射（ADR-0033 联动）**：**无槽——community 地板核心能力**。依据：inv-2 §103（OSS 发行 openapi.yaml 含 build CRUD 全族）+ inv-4 D1~D5（reverse-src = OSS 发行集合——build-info REST 在场，高置信）；与 AQL 同位（核心查询/记录能力无门控）。Builds 页 OSS 档可用性核验归 T-488 活体腿（若翻案 → 本 ADR 勘误 + 以预留 ID `build-info`（Kind=KindFeature）建槽——翻转面单行常量）。AQL build 系入口 OSS 活体 400（aql.md v14）系 Artifactory 引擎档位差异——BinFlow AQL 为自有核心域无槽（M15 定案），build 域入口随之无槽（子集注记：超 OSS 档对位，形态对齐商业 AQL）。
  12. **六票拆分边界确认（FR-152——可实现粒度成立）**：T-507 = model.go + store.go（BuildStore）+ DDL migration + ACL 镜像与 CanRead 投影（纯新包零既有引擎文件改动——cmd 装配行例外）；T-508 = service 上传/append/查询编排 + httpapi/build.go 上传查询族（消费 T-507 store）；T-509 = promote.go + retention + repo CopyOrMove/docker 面消费（消费 T-508 service）；T-510 = eventtypes 翻转 + cmd 装配 Emit facet 接线 + criteria 面（消费 build.emit 缝——织入窗口独占，M13 织入大票先例）；T-511 = search 注册表 + BuildSearcher 消费 + 两老搜索端点（消费 T-507 store——**不 dep T-508/T-509**，波序可提前的接口依据）；T-512 = FE（消费 T-508 REST + T-507 Module ID 数据）。每票各自 table-driven 可验证，交接缝 = BuildStore 接口 + build.Service 方法族 + Emit facet——串行链 review 面局部化（ADR-0043 决策 10/ADR-0044 决策 13 同款论证）。
- 理由: 五轴共同判据 = 「单决策点 + 既有载体复用 + 闭集可验证 + 织入面降级为 import 图」。ACL 轴 A 是「T-92 血统延续」与「Artifactory build 权限语义」的唯一交集——path 位传 build_name 使按名模式授权零新面可得，B 为已可表达的东西付出第二条权限通道（分叉即泄漏面）；C 把 CI 数据面误判为管理面（场景 A 落空）。载体轴 A 守住「记录 vs 制品」的分界（KB 级 JSON 进 blobs 是把元数据问题误投给存储层）；promote 门轴 A 与「CI 直接 push 目标仓」同权位——语义等价零提权，B 触闭集纪律，C 语义错位（配置权 ≠ 迁移权）。织入轴 A 的 import 禁令让「六票串行链 + 织入票窗口独占」的排波纪律获得编译期证据（build 不会静默长出对 webhook/search 的依赖）。载体轴 A（promote）兑现 T-509「复用非重构」——第二执行器即断言反转⑦警告的双引擎同型错误。
- 后果: 新增 `internal/build` 包（六票 area）；migration（表族六表，双方言）；metadata +BuildStore 缝（substores_builds.go）；httpapi +build.go router（新 router 文件——波内并行零共享文件）；`repo.Service` 零改动（CopyOrMove 既有消费）；webhook eventtypes 三行翻转 + cmd 装配 Emit facet（T-510）；search 注册表 build 域三入口 + BuildSearcher（T-511）；audit +5 词；metrics +4；architecture §2 包树 + §26 [M17] 增补 + §11.51 债登记；K74 的 ACL 模型项回填 PM（「已定案 ADR-0045」）；tech-writer Build-info 接入指南（T-518 腿①）。零新配置键、零新外部依赖、零既有端点行为变化。
- clean-room 边界: 行为基准 = JFrog 官方 REST 文档（Build Info 域——T-488 逐条锚定出处）+ inv-2 §103/inv-4 D1~D5（OSS 反编译集合的端点族与数据模型**行为规格**——inv-4 明示档位/缺失对照）+ webhook.md §3.4 + aql.md §2.1/§6（仓内冻结锚）；**表族 schema/ACL 求值/promote 编排/Emit facet/搜索织入全 BinFlow 自有设计**；BuildRunImpl/Module 等 Java 实现零参考零翻译（ADR-0001）；build JSON 为官方公开数据契约（非代码）。
- **软缝对齐清单（T-488 build-info.md 落盘后 Accepted 期复核，分歧走勘误段——机制轴零翻动；T-488 在途，本清单即差异核对协议）**：① 表族列集与 wire JSON 字段集（builds/modules/artifacts/dependencies/promotions/properties 字段名与形态——本 ADR 骨架钉主键/外键/关联，列名以规格票冻结）；② build_repo 缺省值与指定参数形态（本 ADR 暂行 `artifactory-build-info`）；③ 上传/append 端点方法与路径形态（PUT vs POST 分歧——inv-4 D1 与官方文档迹象不一致，以官方文档优先、规格票定案）；④ append 合并细则（module 合并键/依赖去重/重复上传覆盖语义）；⑤ promotion 状态机字面（状态值集与现势推导）与参数族（failFast/failsOnMissingArtifacts/properties/dependencies 行为细则）；⑥ retention 参数形态（count/days/buildNumbers/minimum retained days）与删除审计明细；⑦ 「最新」端点形态（排序取首 vs 专用端点）与分页/过滤参数；⑧ 错误文案逐字样本（400/404/403）；⑨ delete 端点参数族（builds/dateRange/artifacts）形态；⑩ 档位核验结论（Builds 页 OSS 档——若翻案触发点 11 勘误）。效力序：用户裁决（BOARD）> build-info.md（含 as-built 段）> 本 ADR > PRD 暂行值；分歧按 M15 T-407/T-408 先例上 BOARD。
- **验收锚点骨架（T-507~T-511——PRD FR-152 AC 对位，M17-SPLIT 票面 AC 全文为准）**：T-507 = 表族关系/级联/幂等迁移 table-driven + **越权探针三臂**（无权限用户 build 查询 403 / 详情零出现 / 列表过滤零泄漏——NFR-S80）+ 纯新包零既有引擎文件 diff；T-508 = curl 上传/append 合并/查询族全链 + 错误面诚实（畸形 400 逐字/404/403）+ metrics 在场 + jf CLI 条件腿；T-509 = promote 状态翻转 + audit 行 + **promote 动词权限门 403 探针** + docker 删缓存复拉（sha256 对账）+ retention 保留窗外删除 + 大 build 零 5xx；T-510 = 三事件 wired（local receiver HMAC 验签逐字）+ ADR-0041 schema 零变化断言 + 注册表零伪造审计 + criteria build scope 生效；T-511 = builds.find 三入口绿（远期 400 行翻转）+ dependency include + promotion/releasebundle/sensitive 维持 400 注记 + **AQL 越权零出现**（BuildScope 行级过滤）+ K63 门沿用 + P95 ≤300ms 量级。
- **Errata（2026-09-06 软缝回填轮——T-488 build-info.md 落盘后 Accepted 期复核，conductor 指令〔E1~E4 + 证实项〕；Status 维持 Accepted，机制轴（ACL 同源/表族载体/import 禁令/promote 载体复用）零翻动；正文原行保留、以本段作废/替换为准——ADR-0041 勘误体例）**：
  - ① **决策 6 REST 对位表三处字面作废-替换（E1/E2/E3）**：
     - **上传行**：原「`PUT /api/build/{name}/{number}`（全量保存语义…）」**作废** → **`PUT /api/build`（name/number 在 body，路径不带坐标）**；body = 完整 build info JSON；重复上传同名同号 = 覆盖且**需 delete 权**（官方权限注记——覆盖臂 = w(buildRepo) ∧ d(buildRepo)，首次创建臂仅 w）；modules 的 artifacts 携正确 sha1/md5 方可与制品关联（无 checksum 只入记录不建关联）。**PRD FR-152 AC1 与 M17-SPLIT 票面 AC 的 `PUT /api/build/myapp/1` 骨架形态随本条作废**（L50 命令行同步——PM/conductor 回写）。
     - **append 行**：方法修正为 **POST** `POST /api/build/append/{buildName}/{buildNumber}`（原骨架 PUT 作废）；body = **BuildModule 数组**；成功 **204 空体**；父 build 不存在 → 404 `Build-Info not found`（逐字）；权限 = Deploy ∧ Delete（官方——对位 BinFlow w ∧ d）。
     - **批删行**：原「`DELETE /api/build/{name}?builds=&dateRange=&artifacts=`」参数族**作废** → 实际 = **`DELETE /api/build/{buildName}?buildNumbers=<csv>&artifacts=0|1&deleteAll=0|1`（无 dateRange）** + **POST `/api/build/delete`** body `{project, buildName, buildNumbers[], deleteArtifacts, deleteAll}`（6.13+，支持 build 号特殊字符——该端点独立存在的理由）；成功 200 text `The following builds has been deleted successfully: 'name#51', 'name#52'.`（官方例文含历史语法瑕疵「has」——逐字对齐与否归 T-508 票内裁留痕）。
     - **retention 行**：「count/days/buildNumbers/minimum retained days」中 **days 作废** → 实为 **`minimumBuildDate`（ISO8601 时间戳，早于此可删——非天数）**；body 全集 = `{deleteBuildArtifacts, count, minimumBuildDate, buildNumbersNotToBeDiscarded[]}`；query `async`（缺省 **true**）；**设定保留 ≠ 立即删除**（官方明文——保留参数挂 build 名，删除由保留窗驱动；T-509 的「retention 删除保留窗外 build」腿 = 设定后的执行窗语义，票面 AC 措辞随此校准）。
  - ② **证实项回填（原暂行值获双源证实——正文相应句读作已定案）**：软缝② build_repo 缺省 **`artifactory-build-info`**（产品 DDL DEFAULT + webhook §3.4 示例 + `getPreferredBuildRepo` 回落三证，高）——指定参数 = `buildRepo`（append/dependency）/ `project` 族不承接（BinFlow 无 projects 域）；软缝④ append 合并键 = **module id**（同 id = 同模块追加 artifacts/dependencies、不传则新增；「引用子 build」形态 module id = `<子build名>/<子build号>`——聚合 build 经引用模块收纳子 build）；软缝⑤ promotion status = **自由字符串无闭集**（UI 惯例值是约定非协议）——promotions 六元组 append-only、现势 = **按 timestamp 取最新**（正文「状态机字面照规格票」待答项关闭）；软缝⑦「最新」= **无专用端点**，GET /api/build 每名回 lastStarted、按 started 排序取首；软缝① 列集字面 = build-info.md §3 字段集表（builds 9+id / modules 2+id / artifacts 5 / dependencies 6 / promotions 6 / properties 3——产品 DDL 逐字在 §3.2，含 `statuses[]` 回显 schema 缺项注记）。
  - ③ **E4 档位推断修正（决策 11 依据句作废-替换，决策不翻案）**：原「inv-2 §103（OSS 发行 openapi.yaml 含 build CRUD 全族）…build-info REST 在场」推断**作废** → **「openapi.yaml 随 OSS 代码库发行 ≠ 端点在 OSS 档可用」**——Artifactory 侧行为证据整体指向 **JCR/Pro 商业档门控**（官方描述三处 Pro/JCR + t226 旧活体 build 搜索族 400 Pro 门 + Builds nav 在 OSS 在场但内容行为未实证）；**community 地板决策维持不翻案**，依据改立为「BinFlow 自有超集实现裁量」（无许可门 = 超集实现不违 parity——aql.md §8.2 档位口径先例）；Builds 页 OSS 内容行为 = build-info.md §9 待验证 #1（活体恢复后补腿，勘误终局锚）。
  - ④ **新增证据面登记（ADR 未及项，build-info.md 已留档）**：㋐ 出口 B 的 Artifactory 实态三证（permission target 可选 `builds` 段 + 预置 `artifactory-system-default-build-permission` + t226 UI `Edit Builds`）= 真值留档**不构成翻案**（出口 A 已裁，表达力等价面论证在案）；㋑ buildinfo 仓被**排除于 ANY LOCAL/REMOTE/DISTRIBUTION/Anything 预置回退链**（授权序精确仓 ACL → buildinfo/release-bundle 仓**短路拒绝** → ANY 族）——印证 BinFlow 伪键通道显式承载的方向（ANY 家族不隐式覆盖 build/bundle 域）；㋒ buildinfo 仓直传/文件面/copy-move 拒绝族（§2.3）= Artifactory 形态记录，**BinFlow 不承接**（载体轴 A 重申——REST 面 `PUT /api/build` 为唯一入口）；㋓ **builds 唯一性四元** (name, number, started, repo)——同名同号不同 started 是不同 run、`?started=` 为消歧键：决策 2 DDL 骨架的 PK (build_name, build_number) 建议扩为**含 started 的复合唯一键**（对位官方 run 语义），定形归 T-507 照 §3.2；㋔ docker promote 权限链三证 = **目标 w ∧ 源 r ∧ 目标 annotate**（属性随迁）——决策 5 的 promote 门维持 w(targetRepo) ∧ r(buildRepo)，**properties 随迁臂（若 T-509 承载 body.properties——挂被促销制品上）增判 a(targetRepo)** 对位官方语义；㋕ promote body 补充字面（copy 缺省 false = move / artifacts 缺省 true / dependencies 缺省 false / scopes[] additive / sourceRepo 缺省自动解析 / targetRepo 5.7+ 可为 virtual——BinFlow 裁决「必须 local」**维持**〔M17 无 virtual 促进用例，virtual 直写既有解析〕，若票内翻案留痕）；㋖ 老搜索两端点逐字文案已冻结（buildArtifacts 三条 400 含「your」拼写原文 + 404 形态；dependency 键名 uri vs buildArtifacts 键名 downloadUri）——T-511 断言面照 aql.md §15.4。
  - **软缝对齐清单就本 Errata 关闭**：①②④⑤⑦ 证实/定案，③⑥⑨⑩ 勘误回填（上列①③）；⑧（错误文案逐字）部分关闭——上传/promotion/retention 逐字文案不可得（REST 实现类不在反编译集合），build-info.md §9 待验证 7 条如实降档，活体恢复后补腿（零静默升格）。验收锚点联动：T-508 断言面照修正后端点形态；机制条款不动清单（ACL 同源五动词/promote 门/import 禁令/表族六表骨架/Emit facet/BuildSearcher/K63）全部维持。
- **Errata 二（2026-09-15，L023-2E 实现期复核包——L023-1 规格核对〔build-info.md §11：勘误 E1~E13 + 活体 26 探针，OSS 7.161.16 源码 + 活体 7.161.15 双源〕+ L023-2A/L023-2B 实现对位双源复核触发；Status 维持 Accepted，机制轴（ACL 同源五动词/表族载体不进 nodes/import 禁令/promote 载体复用/单决策点）零翻动；正文与前段 Errata（下称 Errata 一）原行保留，以本段作废/替换为准——ADR-0041 体例）**：
  - ① **软缝④ append 合并键证伪（Errata 一 ② 的软缝④句 + 正文点 6 append 行「按 module_id 合并」半句作废）**：原「append 合并键 = module id（同 id = 同模块追加 artifacts/dependencies、不传则新增）」→ 修正 = **append 为模块列表纯拼接**（既有在前、新数组尾接后整体重存）：同 id 模块**不合并、不覆盖、不去重**——同文档重复 id 与跨请求重复发布均产生重复模块条目并存（各持自己的 artifacts/dependencies，模块内行亦无去重）；「聚合 build 引用子 build」= 客户端把子 build 坐标编码进 module id 的约定（`<子build名>/<子build号>`），服务端不识别不解析。**DDL 联动**：决策 2 骨架 build_modules PK (build_name, build_number, module_id) 字面作废 → **ord 键**（modules PK=(坐标, ord)、artifacts/dependencies PK=(坐标, module_ord, seq)——migration 026 双方言，ROW_NUMBER 按 module_id 定序回填旧行）。证据：build-info.md §11.1-E5 + §11.4-3 + §2.2；reports/agents/L023-1.md（活体 append 同 id → GET 两行 mod-a 各持 artifacts）；实现 commit 9716d500（L023-2B：mergeModules 删除改纯拼接 + 026_build_module_ord.sql + REST 级同 id ×3 各持己物断言钉死「不合并」）。
  - ② **软缝⑥ retention 时机修正（Errata 一 ① retention 行两处字面作废）**：原「query async（缺省 true）」→ **缺省 false = 同步**（源码 JAX-RS 原语直读：无 @DefaultValue 的 primitive boolean 缺参即 false → 同步分支删完才回 204——E9；对外可观察差异面仅删除完成与响应返回的时序，功能面零差异）；原「设定保留 ≠ 立即删除（官方明文）」→ **POST 即触发删除**（缺省同步删完再回 204、async=true 后台；官方表述过时——E7）。**新增面入册**：build JSON 上传携 buildRetention 块时当次发布尾部同步执行同一删除流程（§11.6-8——正文原未载）；补正字面：count=0/缺 body → 400 `Max count retention needs to be a positive number`（逐字）、删序两遍（日期维度先行、count 维度后行——候选按 started 倒序跳过前 count 个）、豁免 = promoted（promotion 历史非空永不被 retention 删）∪ buildNumbersNotToBeDiscarded[] 且不占 count 名额。正文点 6 retention 行门 d(buildRepo) 维持（REST 资源层断言；执行层无权 = 宽容跳过零删除 + warn 非报错——§11.6-6 两层分层）。证据：build-info.md §11.1-E7/E9 + §11.6；reports/agents/L023-1.md（活体 count=0 400 / count=1 即删留 promoted 三探针）；**as-built 已收口：retention 对位与发布尾部触发随 commit dbc52c66 落地（开放项 ① 关闭——2026-09-15 conductor 回填）**。
  - ③ **promote build 侧门动词 r→w（正文点 5 字面修正，本 Errata 即裁定 authority）**：原「Can(targetRepo, "", w) ∧ Can(buildRepo, build_name, r)」的 build 侧 r 作废 → **w(buildRepo)（upload 权断言）**：promote 落一条 append-only promotion 历史 = 对 build 的写操作，规格 §11.5-1 前置「build 上传权断言」与官方「promote 需 build 的 Deploy 权」（§2.4，高置信）同源；status-only 臂 drop w(targetRepo) 分肢维持 → 全门 = **w(targetRepo) ∧ w(buildRepo)**（status-only = 仅 w(buildRepo)）；403 文案随实际门动词走 §7 upload 句。机制面维持：allow() 同源、r/w/d/m/a 闭集、单决策点、「拒绝先于存在查找」无 oracle 序不动——仅 build 侧动词字面翻正（Errata 一收口段「promote 门……维持」按此读：机制维持、字面翻正）。**as-built 现状 = 仍按原 ADR 字面跑 r**（L023-2B 明示语义门未动候 architect 裁——internal/build/promote.go:239-244 CanRead + read 句文案 + divergence 注释在码登记）——翻正实现 = 本段开放项 ②（后续小票，门序位置不变）。证据：build-info.md §11.5-1 + §2.4 + §7；reports/agents/L023-2B.md Compatibility ①（上报原文）；commit 9716d500（r 门现状）。
  - ④ **annotate upfront 403 门作废（Errata 一 ④㋔「增判 a(targetRepo)」句作废——仅 ④㋔，④㋑ 及其余子项维持）**：原 properties 随迁臂的请求级 upfront a(targetRepo) 403 门 → **无 upfront 门**：body properties 非空时对（目标仓或源仓的）收集项**逐项** annotate 检查——无 annotate 权 → warning `User doesn't have permissions to annotate '<path>'`（逐字）+ failFast=true 时中止标注；lenient 时 warning 与 200 并存（§11.5-7）。安全注记：权限判定仍在 allow() 单点、逐项检查 = 无权项跳过——越权标注结构性不可能，变的是拒绝形态（403 门 → warning 行 + failFast 中止）。证据：build-info.md §11.5-7；实现 commit 9716d500（L023-2B：upfront a() 门删除、逐项 warning、annotate 逐项 warning 测试臂——promote.go:250-252「No upfront annotate gate」）。
  - ⑤ **顺带漂移清偿（四项之外，以规格为准）**：㋐ append 404 文案：Errata 一 ① append 行「404 `Build-Info not found`（逐字）」作废 → `The build <name>:<number> is not found`（openapi 描述字段非实现文案——E4；实现 commit c42fb255）。㋑ append 门断言序补字面：**先断言 delete 权、再断言 upload 权**（§11.4-2 无 oracle 姿势的序要求；commit 9716d500）。㋒ 批删成功文案待裁项裁结 = **have**（实现为准——E6；Errata 一 ①「逐字对齐与否归 T-508 票内裁留痕」就此关闭）：`The following builds have been deleted successfully: '<name>#<n>', ….\n`（分隔 `, `、段尾 `.\n`）+ 部分失败续 `Warning - the following builds could not be removed: '<n>'.\n`、整体 200；deleteAll 独立文案 `All builds '<name>' under '<repo>' have been deleted successfully`（无尾句点）；**POST /build/delete body 六字段**（Errata 一 ① 的五字段缺 buildRepo——E10；DELETE 族文案已落 commit c42fb255，POST body 面 = 票 C 在途）。㋓ targetRepo virtual 收取：正文点 5「必须 local（virtual/remote 不收）」与 Errata 一 ④㋕「BinFlow 裁决『必须 local』维持」中的「virtual 不收」半句作废 → **virtual targetRepo 收**：解析为其 default deployment local 仓后继续（statuses[].repository = 解析后仓——§11.5-3）；不可解析 virtual / 非 local 族 / 缺失 → 统一 404 `Cannot find target repository by the key '<caller key>'`（§11.5-4——正文「400/404 形态照规格票」待答项关闭为 404）；门求值与审计对解析后键（commit 9716d500：virtual 三别名解析 + 统一 404 + routed-virtual 落历史测试臂）。
  - **维持项复核确认（不翻案）**：软缝② 缺省 `artifactory-build-info`；软缝⑤ status 自由字符串 + promotions append-only + 现势 = 最新 timestamp；软缝⑦ 无专用 latest 端点（细化：号单 started 严格倒序最新在前、同号多 run 并列——§11.8，Errata 一 ② 表述成立且获双源落定）；点 4 ACL 同源五动词；点 9 promote 载体复用；点 11 community 地板（Errata 一 ③ 已裁）；点 5「缺制品语义 failFast 拒 / 否则跳过」骨架获实证（E12：failFast → 400 `Unable to find artifacts of build '<name>' #<number> from <buildRepo> repo: aborting promotion.` 逐字；lenient → 单条 warning 带名单）。
  - **Errata 一收口段状态修订**：其「①②④⑤⑦ 证实/定案」清单中的 **④ 重开并证伪**（本段 ①）；「⑧ 错误文案逐字部分关闭（REST 实现类不在反编译集合）」的依据句过时——L023-1 实现源解锁（OSS 7.161.16 树 build-handler/build-handler-service——E13），REST 全族文案已逐字在案（§11），残余 = promote timestamp 非法路径 body 形态与 patternArtifacts 响应（§9 #5/#9）。
  - **开放项（复核输入，未定案——零猜测入册）**：① 本段 ② 的 retention 对位 commit 号（票 C 收口回填）；② 本段 ③ 的 build 侧门 r→w 翻正实现票；③ PUT 覆盖臂 promotions 保留（BinFlow 整体替换 upsert）vs 参照「先删后建」连历史删——观察面等价、UNPROBED，候契约探针定案（L023-2A Risks ③）；④ E12 lenient 名单连接符 ", " 为推断（活体样本单名）；⑤ 名清单序向（SQL 升序字面）中置信待多活体；⑥ retention async 缺省的 7.161.15/.16 版本漂移（§9 #10，低风险时序差异）；⑦ promote 413 配额面不承接（build JSON 走 DB 无配额面——L023-2B 登记）。
  - 效力序重申：用户裁决（BOARD）> build-info.md（含 §11 as-built 段）> 本 ADR > PRD 暂行值；分歧按 M15 T-407/T-408 先例上 BOARD。交叉对账：ADR-0046 所引「ADR-0045 Errata ④㋑」（buildinfo 仓 ANY 族排除）不受本 Errata 影响（作废仅 ④㋔）；机制条款不动清单（ACL 同源/import 禁令/表族六表骨架/Emit facet/BuildSearcher/K63）全部维持。

---

## ADR-0046: Release Bundle domain (minimal face) — internal/bundle new package with feature slot `release-bundle` (MinTier=pro) and Any-Distribution preset as an allow()-sourced pseudo-repo channel; with K75 co-sign (insights aggregation layer: internal/insights new package, scheduler 4th domain, four metric families on the K69 single source) and B-1.7 supplement review (Administer Platform/Manage Resources dual booleans vs three-value enum — keep the enum, register the tripwire trigger; ADR-0026 supplement-review record)

- 状态: Accepted（2026-09-06，M17 W0 T-489 交付；与 T-488〔release-bundle.md 规格票〕W0 并行在途——端点族路径/冲突语义/状态机字面/模型字段集以 release-bundle.md 冻结为准（软缝清单同 ADR-0045 条款）；**Q2 深度终裁为 conductor 派单前小裁——本 ADR 两出口并列作其输入，M17 实现面按出口①（最小面）定案**；K75/K76 与 B-1.7（Q11）三项会签/评审由本 ADR 关闭）
- 日期: 2026-09-06
- 背景: FR-153（Release Bundle 最小面，P1）+ FR-154（洞察报表，P1——K75 会签为 FR-154 拆票前置）+ FR-156.3（B-1.7 增补评审腿归本票）。行为依据：JFrog 官方 REST 文档（Release Bundle v1/v2 域）+ inv-4 distribution 族（RBv2 = Distribution 服务对接——Enterprise+/Edge 档）+ webhook.md §3.5~3.7（release_bundle 三域 9 型休眠——RBv1 域不建）+ aql.md §2.1（releases/release_artifacts 入口「远期 dep Release Bundle 域」——**M17 维持 400**，随最小面不开查询入口）+ console-ui.md §3.3（权限两步弹窗预置 Any Local/Any Remote/Any Distribution）+ T-453 as-built（7.161.20 用户表单探针——管理位双布尔形态在案）+ rbac-model.md #1/#2（Artifactory 实例级无角色层、无 read-only admin——高置信）。现役基座勾稽：permission target 模型 repos[] + includes/excludes + r/w/d/m/a（ADR-0026 + K68）；auth.Authorizer.Can 单决策点；license 三档 + addons Registry + 三缝门控（ADR-0032/0033——features 槽现有 6 个〔properties/ha/repo-operations/xray-integration/trashcan/webhook〕，第 19 槽 = webhook）；scheduler 域闭集 ('maintenance','backup','replication') + CHECK 扩列预留（021 注释明示「new domain is an incremental registration (a CHECK list edit in a later migration)」）+ Runner 注册面 + Kick 缝（ADR-0044）；K69 nodes 统计四列（migration 020——唯一计数源契约）+ repo_usage 既有聚合面（M9 E1）；M16 per-node 统计四列已落（T-438/T-440）。约束：Q2 PM 倾向最小面（已载 PRD）；NFR-S81（bundle 管理 admin/manager 门 + audit）；NFR-P81（聚合查询 P95 ≤800ms 万节点 + 快照任务不阻塞在线路径）；R4（洞察单源纪律——零第二计数通道）；K69 单源契约不可破（ADR-0044）；LC-104 C 层自有（Artifactory Insights 系独立商业产品无 OSS 对位）；PRD §1.4-4 低置信不进断言（B-1.7 wire 证据未核验）。
- 候选方案: （七轴）
  - 深度边界（Q2）: A) **最小面**（本地 bundle 记录：创建/查询/列表 + Any Distribution 预置——v2 signing 与 Distribution 服务对接不做）；B) 全量面（v2 signing + Distribution 对接——跨实例分发编排）。
  - bundle 载体: A) **metadata 记录表族**（bundle 是版本化发布记录非制品）；B) bundle 清单落隐藏仓制品（存储形态）。
  - bundle 门与 Any Distribution: A) **管理面系统能力门 + Any Distribution 预置 = allow() 同源伪键通道**（Can 的 repoKey 位承载通配桶值——r/w 对伪键求值）；B) 纯系统能力门（Any Distribution 仅 UI 装饰——伪门）；C) 独立 bundle 权限面。
  - 档位: A) **新槽 ID=`release-bundle`、Kind=KindFeature、MinTier=pro（暂行）**；B) 无槽 community 地板；C) MinTier=enterprise。
  - 洞察触发机制（K75）: A) **scheduler 域扩列**（第 4 域 insights——schedules CHECK 扩列迁移 + Runner 注册 + Kick 加速 + 域并发 1/坍缩/回拨三防护免费复用）；B) 独立 1day ticker（insights 包内自建循环）；C) 惰性计算无快照（查询时现算——无时序历史）。
  - 快照默认: A) **boot 物化默认行**（domain=insights, key=daily-snapshot, cron `0 0 2 * * ?`——台账面可见可改可删，启动 INFO 一行）；B) 零预置（出场空图表——用户显式建行才起）；C) config 键开关。
  - B-1.7（Q11）: A) **维持三值枚举 + tripwire 触发器机制登记**；B) 双布尔落地（users wire 增 adminPlatform/manageResources + 求值 + 迁移）；C) 双布尔与枚举并存（双通道）。
- 决策: **七轴全 A（Q2 轴 A 为 M17 实现位，出口②并列登记为终裁输入）**。要点十二条：
  1. **包边界（internal/bundle 定名——M17-SPLIT「新包定名归 ADR-0046」兑现）**：新包 `internal/bundle`，只 import metadata（BundleStore sub-store）/ auth（Authorizer + 能力门）/ audit（facet）/ config（只读）；**import 禁令**：禁止 import webhook、search、httpapi、repo（bundle 记录不触内容面——与 build 域分界的结构性保证：两新包互不 import）、scheduler、insights、license、replication。文件布局：`model.go` + `store.go` + `service.go` + metadata `substores_bundles.go` + httpapi `bundle.go` 新 router（dispatchAPI 族）。DDL 骨架（migration 编号先到先得，与 build 表族分迁移）：
     ```sql
     bundles(   bundle_name TEXT, bundle_version TEXT, state TEXT NOT NULL CHECK (state IN (...)),  -- 状态闭集照 release-bundle.md 冻结
                created_by TEXT, created_at TEXT, updated_at TEXT, description TEXT DEFAULT '',
                PRIMARY KEY (bundle_name, bundle_version));
     bundle_items( id TEXT PRIMARY KEY, bundle_name, bundle_version,
                repo_key TEXT, path TEXT, sha256 TEXT,            -- 清单行 = nodes 时点快照（sha256/size 固化——制品后续变化不追溯，bundle 是发布时点记录）
                size INTEGER, added_at TEXT, added_by TEXT );
     -- 索引：bundle_items(bundle_name, bundle_version)
     ```
     模型字段集（名字/版本/制品清单/创建者/状态）以 release-bundle.md 冻结（软缝①）；**清单不可变**（版本一旦创建不追加——重复版本冲突 4xx，形态照规格票）。
  2. **端点族最小面（M17 实现 = Q2 出口①；两出口并列作 Q2 终裁输入）**：最小面端点族 = 创建（名+版本+清单）/ 单查 / 列表 三族（路径与请求响应体照 release-bundle.md 冻结——倾向官方 v1 族对位 A 层；骨架示意 `PUT/GET /api/release_bundle/{name}/{version}` 族，不猜字面）。**出口②（全量面）登记**：v2 signing（GPG 签名链）/ Distribution 服务对接（跨实例分发、release_bundle_v2 三域事件、distributed 状态机）= 越单仓产品边界风险类 Xray 集成面（PRD §2.2 明示）——翻转条件 = 用户明示推翻 Q2 + Distribution 服务可行性评估票 + 本 ADR 增补（届时 MinTier 升 enterprise、端点族全量重裁）。webhook release_bundle 域 9 型**维持 dormant**（最小面无 signing/distribution 语义——created 事件是否随创建端点 wired 归 T-513 票内与规格票对拍，倾向 wired 一型〔created〕其余维持——软缝⑤）。AQL releases/release_artifacts 入口**维持 400**（查询面 M17 不开——最小面子集注记）。
  3. **bundle 门 = 管理面能力门 + Any Distribution 预置伪键通道（轴 3-A）**：创建/删除 = **CapSystemWrite**；列表/单查 = **CapSystemRead ∨ Any Distribution 通道**。Any Distribution 通道 = permission target repos[] 预置通配桶值（T-491 三预置同场——Any Local/Any Remote/Any Distribution）作用于 **Can(p, <any-distribution 桶值>, bundle_name, r) 的 repoKey 位**——allow() 同源（auth.Service.Can 单决策点，与 build 域点 4 同构；path 位传 bundle_name 使 includes/excludes 按名生效）；桶值拼写与三预置机制同族（T-491 票内/architect 小评终裁——本 ADR 钉死通道形态：**通配桶值进 Can 的 repoKey 参数位**）。授予面覆盖探针（T-514）：被授权用户可见/可操作。**否决 B**（UI 装饰伪门）：违「无数据源不伪造」同源纪律——勾选不生效即伪门；**否决 C**：双真相源论证同 ADR-0045 轴 1-B。
  4. **档位 = 新槽 `release-bundle`、Kind=KindFeature、MinTier=pro（暂行）**（第 20 槽——slots.go 装配清单 + cmd 一行）。依据：Release Bundle 系 Artifactory **商业档能力**（RBv1 历史档 Enterprise；RBv2 = Distribution 类）——行为模式对位（ADR-0033「包型/功能 = addon 门控单元」+ 用户 ADR-0032 指令「拥有和 Artifactory 的 license 控制」）；pro 而非 enterprise：最小面是记录域（enterprise 级真身 signing/Distribution 已被出口①排除）。三缝门控（ADR-0032/0041 惯例）：REST 面 403 + `X-Binflow-License-Required: release-bundle`；数据面缝（service 首行 gate func 注入）；`addons.disabled` 熔断（写 403 无头 + 读 200 + **暂停不丢**）。**勘误路径**：T-488 商业档位核验若证 OSS 档可用（预期低）→ MinTier 降 community = 常量一行；Q2 出口② → 升 enterprise + 新 ADR。Any Distribution 预置在 community 实例 = 授权面在场、能力面被槽位门（不伪造——gated 入口带徽章可见同哲学，ADR-0032 D5/§11.41）。
  5. **洞察包定名 = `internal/insights`**（K75 定案项——M17-SPLIT「定名从会签」兑现；否决 analytics〔与 ADR-0022 metrics 语义混淆〕/reports〔与 audit 报告面混淆〕）。包边界：只 import metadata（快照 store + 只读统计查询）/ repo（只读聚合消费——repo_usage 面）/ auth / audit / config；**import 禁令**：禁止 import httpapi、scheduler（**反向**——scheduler 经 Runner 注册面消费 insights 导出的 RunOnce 载体，依赖方向 scheduler → insights，与 ADR-0044 点 1「scheduler import 各域导出载体方法」一致）、webhook、search、build、bundle、license、replication。文件布局：`snapshot.go`（快照任务 RunOnce 载体——幂等：同日重跑 = 当日行重算 upsert）+ `model.go` + `store.go`（metadata `substores_insights.go`）+ `query.go`（四指标族查询）+ httpapi `insights.go` 新 router。
  6. **触发机制 = scheduler 域扩列（轴 5-A；ADR-0044 并存语义扩列留痕）**：① schedules 表 domain CHECK 扩列加 `'insights'`（重建式迁移——schedules 行量小，sqlite/PG 双方言；021 注释预留的既定路径）；② 域注册面 +1：cmd 装配 `scheduler.Register("insights", insightsRunner)`——Runner.Run(ctx, "daily-snapshot") 调快照载体；③ **ADR-0044 决策 8-①「全量类任务闭集」扩列 +1 = insights 日快照**（快照是全量聚合任务——非增量、不与事件驱动重叠、闭集纪律保持；该 ADR 勘误留痕由本条承载，机制条款零翻动）；④ 域并发 1/错过窗坍缩/时钟回拨三防护 + Kick(domain,key) 手动触发等价缝（测试加速——T-515「手动触发等价腿」的承载）免费复用；⑤ audit 词 `insights.schedule.{set,run,fail}` 三词（域闭集扩列自然延伸——ADR-0044 决策 10 九词 +3）。**否决 B**（独立 ticker）：第 4 套调度循环违「台账单源」（next-run 不可见、无 CRUD 面、无坍缩防护——B 的全部成本恰是 ADR-0044 立项要消灭的形态）；**否决 C**（无快照）：时序趋势无载体（FR-154 立项动因即历史曲线，现算无昨日）。
  7. **快照默认 = boot 物化一行（轴 6-A；对 ADR-0044 决策 2「不预置」的边界注记）**：boot 序检测 domain=insights 无任何行 → 物化默认行（key=daily-snapshot、cron `0 0 2 * * ?` 每日 02:00、enabled=1）+ 启动 INFO 一行。**非静默**：台账面（`GET /api/v1/system/schedules?domain=insights`）可见、可改 cron、**删行即关**（「无行 = 不调度」单态语义保持——默认不是第二机制，是首配）。与 ADR-0044 决策 2 的分界论证：该裁决针对备份域（重 I/O + Artifactory 发行默认不对齐——安全默认优先）；insights 日快照是轻聚合（秒级 SELECT 聚合）且系 BinFlow 自有域（无 Artifactory 默认可对齐/错齐）——出场即有图表数据（FR-154 用户体验）与零静默写盘纪律的平衡点 = 可见可删的首配行。备域（backup/maintenance）不预置维持不变。
  8. **指标集（K75 定案——四指标族）与快照 DDL 骨架**：
     | 指标族 | 定义 | 数据源（白名单） |
     |---|---|---|
     | download_trend 下载趋势 | 每日下载次数（全局 + per-repo 差分序列） | nodes.download_count **累积计数器的日差分**（K69 四列） |
     | storage_growth 存储增长 | 每日已用字节序列（per-repo） | **repo_usage 既有聚合面（dedup 后字节）**——禁 nodes.size 裸和（共享 blob 双计） |
     | top_repos Top 仓 | 窗口内按下载次数/字节排序 | per-repo 快照行 |
     | top_artifacts Top 制品 | 窗口内按下载次数排序（Top N 物化/日） | nodes.download_count + 节点身份 |
     ```sql
     insights_repo_snapshots( snapshot_date TEXT, repo_key TEXT,           -- 一日一行/仓
                downloads_delta INTEGER, downloads_total INTEGER,
                used_bytes INTEGER, node_count INTEGER,
                PRIMARY KEY (snapshot_date, repo_key) );
     insights_top_items( snapshot_date TEXT, rank INTEGER, repo_key TEXT, path TEXT,
                name TEXT, downloads_delta INTEGER,                       -- 每日 Top N（N=100 内部常量）物化
                PRIMARY KEY (snapshot_date, rank) );
     -- 全局趋势 = per-repo 行聚合（无 '*' 汇总行——单一聚合路径）；保留窗不自动清（行量有界：仓数×365/年 + 100×365/年；裁剪缝留 maintenance 家族）
     ```
     快照**只读业务表、只写快照表**——零第二计数通道的结构性保证。查询 REST = 自有面 `/api/v1/insights/*`（C 层——四指标族查询 + 窗口参数；手动重算触发端点 = CapSystemWrite、Kick 等价）；查询门 = **CapSystemRead**（readonly_admin 可见——审计/GC 面同档惯例，architecture §25.5 K69 门同族）。
  9. **K69 单源对账面（R4 承载）**：① 数据源白名单闭集 = nodes 四列（下载族）+ repo_usage/blobs 尺寸聚合（存储族）——快照任务**不写任何业务表**；② 对账断言口径 = 夹具下载 N 次 → 快照 downloads_delta 与 FileInfo `downloads`（?stats 面）**同值同源**（Σ日差分 ≤ 现值计数器，等号达于最后快照）——T-515 AC3 table-driven；③ 今日活性值 = 查询面可由「最后快照 + 现值计数器差值」拼接（现值唯一源 = K69 四列，快照只存历史——两面板不漂移）；④ **与 T-505 storageinfo 快照不共享存储载体**：storageinfo = 现值缓存快照（on-demand calculate + 冷缓存 503 语义，gap-endpoints §5）vs insights = 时序日快照——两语义并行，唯二共享 = 底层事实源（nodes/blobs/repo_usage）与 scheduler 基建；合体窗口留 T-505 票内核定（本 ADR 预裁倾向不合——时序表被现值端点复用会引入清缓存语义耦合）。
  10. **B-1.7 增补评审（Q11——维持现行 + tripwire 触发器机制登记；本节构成 ADR-0026 角色面的增补评审留痕，K68 先例同款）**：
      - 命题：7.161.20 用户表单**管理位双布尔**（Administer Platform + Manage Resources，另有 Platform Auditor / Manage Webhook 位）vs BinFlow **Role 三值枚举**（admin/readonly_admin/user，ADR-0026 决策 1 + T-214③ wire `adminRole`）。
      - 决策：**维持三值枚举（轴 7-A）**。论证五条：① **表达力不对称**——双布尔表达不了 readonly_admin（Artifactory 实例级无 read-only admin〔rbac-model #1/#2 高置信〕，BinFlow 自有角色无对位出口），而双布尔的 (adminPlatform=false, manageResources=true) 组合（「资源管理员」）在 BinFlow 无载体——对位需第 4 角色或语义扩列 = ADR-0026 级闭集变更；② **闭集可枚举验证是既有 QA 资产**——Role(3)×能力(6) 全表自 M7 起为回归矩阵锚，布尔组合（2² 组合 × 与 readonly 语义交互）使矩阵无界化；③ **第二条提权路径风险**——ADR-0030 E8 已拒组级 adminPrivileges（同型论证：正交布尔位 = 可组合的提权面）；④ **wire 证据未核验**——7.161 呈双布尔系表单呈现，access API 对位字段族未活体核验（Tokens 字段核验候活体源同族登记）——低置信不进断言、无核验不翻案（PRD §1.4-4）；⑤ **readonly_admin 语义自洽依赖角色短路**（ADR-0026 决策 1）——双布尔与角色短路叠加即「只读管理员还能删」矛盾配置的复活。
      - **tripwire 触发器机制登记（转正路径钉死）**：① FE tripwire 已备（T-453 as-built）：`user-form-role-parity-note` 差异登记附注 + 能力位三旗 API 漂移钉（t453 spec：PUT 携带三域 201 而 GET 回显不动——BE 承接落地日翻红）；② **触发器定义**：任一未来票在 users wire（PUT/POST body 或 GET 回显）承接 `adminPlatform` / `manageResources`（或同义字段族）→ 漂移钉翻红 → **转正路径 = PM FR + architect 新 ADR**（角色闭集变更 = 架构变更，ADR-0026 决策 1 纪律）+ 零提权等价迁移论证（K68 点 3 同款——迁移前后 (principal×能力) 有效权限矩阵逐位相等）——**实现票内禁止顺手承接**（私加即 tripwire 红线）；③ 登记位：parity 册 §12 B-1.7 行维持候裁臂已裁态（「维持枚举——ADR-0046〔2026-09-06〕」+ tripwire 指针）；ADR-0026 决策 1/3/5 零翻动。
  11. **FR-153/154 四票拆分边界确认（可实现粒度成立）**：T-513 = bundle 模型 + DDL + store + service + 端点族 + ACL 双门（纯新包——消费本 ADR 点 1/3/4 + release-bundle.md）；T-514 = FE bundle 面 + Any Distribution 勾选腿（消费 T-513 REST + T-491 预置语义——dep T-491 已在 W2 化解）；T-515 = insights 快照层 + scheduler 注册 + 查询 REST（消费本 ADR 点 5~9——**新 router 文件与 build 面零共享文件注记**，W5 并行的接口依据）；T-516 = FE 图表族（消费 T-515 REST）。每票交接缝 = BundleStore/快照 store 接口 + service 方法族 + Runner 载体——table-driven 可验证、review 面局部化。
  12. **audit 与 metrics**：bundle 域 +3 词 `bundle.{create,delete,query?}`——定稿 `bundle.{create,delete}` 两词（query 不审计——读面不记的既有惯例）+ 门控审计随 license 三缝惯例；insights 三词（点 6⑤）。metrics：`binflow_bundle_creates_total` / `binflow_insights_snapshot_duration_seconds` + `binflow_insights_snapshots_total{result=ok|failed}`（expvar 自研）。
- 理由: 七轴共同判据 = 「Artifactory 商业边界的行为模式对位 + 单决策点 + 台账单源 + 闭集可验证」。深度轴 A：v2/Distribution 是跨实例编排服务（BinFlow 单仓产品边界外——PRD §2.2 明示），最小面承载「版本化发布记录可查询可授权」的完整用户价值（场景 B）而把签名/分发的翻转面登记为 Q2 出口②——终裁权留 conductor/用户，实现面定案不留 open。载体轴 A 同 ADR-0045 论证（记录 vs 制品分界）。门轴 A 是「权限两步弹窗预置有真实语义」（parity 要求）与「不建第二条权限通道」（安全要求）的唯一交集——伪键进 Can 的 repoKey 位使 Any Distribution 与 Any Local/Remote 同机制同场（T-491「同场同语义」的架构承载）。档位轴 A 对位 Artifactory 商业层（用户建 license 体系的明示目的），pro/enterprise 取 pro 因 enterprise 级真身已被出口①排除。洞察触发轴 A 是「台账单源」纪律的必然推论——scheduler 建域即为此类全量任务（021 注释预留扩列路径）；B 是 ADR-0044 刚消灭的形态复活。快照默认轴 A 在「出场即有价值」与「零静默写盘」间取可见可删首配行——两纪律各得其所。B-1.7 轴 A 的五条论证以「闭集资产保护 + 证据未核验不翻案」为主轴，tripwire 使「维持」成为可触发的活决策而非永久搁置。
- 后果: 新增 `internal/bundle` + `internal/insights` 两包（四票 area）；migration ×3（bundle 表族 / insights 表族 / schedules CHECK 扩列——编号先到先得）；metadata +BundleStore/+InsightsStore 缝；httpapi +bundle.go/+insights.go 新 router；addons slots.go 第 20 槽 + cmd 装配两行；scheduler 域闭集 +1（'insights'）+ Runner 注册 + audit 三词；audit +5 词（bundle 2 + insights 3）；metrics +4；architecture §2 包树 + §26 增补 + §10 对齐表行 + §11.52~53 债登记；K75/K76 与 Q11 由本 ADR 回填 PM（K76 深度边界项 = 出口①定案 + 出口②登记）；T-488 软缝清单两份待对齐；tech-writer Release Bundle 与洞察指南（T-518）。零新配置键、零新外部依赖、零既有端点行为变化。
- clean-room 边界: 行为基准 = JFrog 官方 REST 文档（Release Bundle v1/v2 域——T-488 锚定）+ inv-4 distribution 族（档位与缺失对照）+ webhook.md §3.5~3.7 + console-ui.md §3.3 + T-453 探针报告（reports/agents/t453-probe/——表单形态行为观察）；**bundle 记录模型/伪键通道/槽位/快照表族/指标族/B-1.7 论证全 BinFlow 自有设计**；RBv1/RBv2 的 Java 实现与 Distribution 服务协议零参考零翻译；insights 为 C 层自有（Artifactory Insights 系独立商业产品——无对位规格可引，LC-104 留痕）。
- **软缝对齐清单（T-488 release-bundle.md 落盘后 Accepted 期复核——机制轴零翻动）**：① bundle 模型字段集与 wire 命名（名字/版本/清单/创建者/状态字段名与形态）；② 端点族路径与请求响应体（官方 v1 族 vs 自有 /api/v1——倾向 A 层官方对位，规格票定案）；③ 重复版本冲突语义（409 vs 400 + 文案）；④ 状态机字面（状态闭集值集与流转）；⑤ release_bundle 域 webhook 事件处置（created 是否随创建端点 wired、signed/deleted 维持 dormant——本 ADR 倾向 created 单型 wired）；⑥ 清单行来源形态（显式路径清单 vs AQL 查询引用——M17 倾向显式清单，AQL 引用随 T-511 联动评估）；⑦ 商业档位核验结论（对点 4 MinTier 的勘误输入）；⑧ Any Distribution 桶值拼写（T-491 小评联动）。效力序同 ADR-0045。
- **验收锚点骨架（T-513/T-515——票面 AC 全文为准）**：T-513 = curl 创建/查询/冲突全链 + **越权用户零出现探针**（NFR-S81——CapSystemRead ∨ 伪键通道两臂）+ v2 signing/Distribution 不做留痕（出口①）+ pro 门三缝（community 403+头 → 装证 200 → addons.disabled 熔断）；T-515 = 快照任务幂等（同日重跑 upsert）+ **单源对账 table-driven**（下载 N 次 → 快照 delta == ?stats downloads == FileInfo 口径）+ Kick 等价腿 + 查询 REST 四指标族 + NFR-P81（万节点 P95 ≤800ms + 在线路径零阻塞——快照只读业务表断言）+ readonly_admin 只读/非授权 403。
- **B-1.7 评审收口行**：Q11 出口①（增补闭集）/**②维持现行 + 触发器**——本 ADR 取②并登记 tripwire 触发器机制（点 10）；PM 收口笔（T-519）Q11 行据此归位；parity 册 §12 B-1.7 行标注更新归 T-492/T-519 顺腿（本票不写 parity 册——area 纪律）。
- **Errata（2026-09-06 软缝回填轮——T-488 release-bundle.md 落盘后 Accepted 期复核，conductor 指令〔E5~E8 + 证实项〕；Status 维持 Accepted，机制轴（最小面/记录载体/伪键通道/槽位三缝/insights 全项/B-1.7）零翻动；正文原行保留、以本段作废/替换为准——ADR-0041 勘误体例）**：
  - ① **E5 端点骨架作废-替换**：决策 2 的骨架示意「`PUT/GET /api/release_bundle/{name}/{version}` 族」**作废**——与两侧官方字面均不符。定案锚（release-bundle.md §1，Artifactory 源侧 18 端点全表）：源侧 = **`/api/release/*` 族**（创建对位 = `POST /api/release/bundle`〔AQL 装配语义——BinFlow 降级为显式清单子集，软缝⑥证实〕/ 查询族 `GET /release/bundles`〔名清单〕+ `GET /release/bundles/{name}`〔版本清单〕+ `GET/HEAD /release/bundles/{name}/{version}`〔描述符/校验和——HEAD 恒 SOURCE 带 X-Checksum-Sha256〕+ `?format=jws` 形态/ `…/status` / DELETE 两行〔主端点恒 TARGET、`/source/` 恒 SOURCE〕）；Distribution 侧（独立服务，M17 面外）= **`/api/v1/distribution/*`**（约 60+ 操作——Q2 出口②启用时再锚）。**BinFlow 最小面路径归 T-513 按 §1 对位**（A 层源侧查询族双源高；transaction/store/jose/config/fat_manifest 族系签名事务链——面外）。
  - ② **E6 冲突语义定案（决策 1「重复版本冲突 4xx」细化）**：**三态 202/200/409**——新建 → **202**；既有且**签名相同**且状态 ≠ COMPLETE → **200**（续传/重放）；签名不同或已 COMPLETE → **409**；409 体 = 产品通用 ErrorResponse **`{"status":409,"message":"Bundle already exists"}`**（原因枚举不出 wire）。**BinFlow 签名缺席适配（C 层裁量留痕）**：出口① 无签名链，「同签名」臂以**内容摘要等价**（清单行集 sha256）承载——同清单未完结 → 200 续传、异清单或已完结 → 409（语义对位，字面自定）；冲突根 = `UNIQUE(name, version, type)`（产品 DDL 一手约束）。
  - ③ **E7 档位真身登记（决策 4 注记，MinTier=pro 暂行维持）**：官方明文 **Enterprise+**（「available to Enterprise+ customers using JFrog Distribution」；RBv1 历史 Enterprise、RBv2 = Distribution/Release Lifecycle）——MinTier=pro 系 BinFlow 自有裁减（最小面 < enterprise 真身），**不翻案、登记注记**：翻转点两处既有——T-488 核验勘误路径（已行使——真身 enterprise 在案，若 conductor/用户裁定对齐真身则常量一行升 enterprise）与 Q2 出口②启用（既定升 enterprise 路径）。
...
- Errata ①（2026-09-11，LOOP 004 L004-2 探针实证——**触发条件已触发，本 ADR 依自带 Errata 条款自动作废**；探针报告指针: reports/compatibility/L004-negative-cache-probe.md §0 结论速览/§2.1 manifest 臂矩阵/§2.4）: 裁定依据的前提「Artifactory 运行时未见负缓存生效」（L000-B 时延判读）**不成立**——探针以上游 access 行计数 + BinFlow sqlite 行直证双判据实证参照侧**双面兑现** missedRetrievalCachePeriodSecs：M-a（已知 image 随机 tag miss）1/3 回源 + **60s 到期复核通过**（负缓存条目过期后回源+1）；M-b（未知 image 名）0/3 确定性本地 404；L002-2「3/3 回源」不复现（差异归因存疑，报告 Risks）。**C14 转按对齐收**：BinFlow manifest 冷 miss 负缓存自 T-363（2026-08-30）起从未存在（非回归的 BUG 缺口）→ 按 BUG + 实现票补齐，known-divergence C14 已翻（INTENTIONAL→BUG，authority 链 = 本 Errata + 探针报告）。**blob 臂不受影响**：B1（ADR-0047 链门）/B2（链内负缓存）双端一致现状维持，本 Errata 对其无行为要求。M-b 相邻新分歧（未知 image 名 0/3 vs 3/3）不属 C14 本体，裁定权在 conductor，此处仅作证据引用。正文原样保留（只追加纪律），本 Errata 节作废声明优先于正文裁定；M-b 语义若后续定谳另立 ADR/Errata。

## ADR-0049: 存储链兼容终案——KEEP 主体落册（§1 表 16 行）、INTENTIONAL 5+1 定谳、REFACTOR 两小面 as-built 收账、prune 族契约 authority 锚（Pruner optional facet + gcDeleteGate 共享删除纪律；L001-4 设计稿收编）

- 状态: Accepted（2026-09-11，LOOP 004 L004-4 正式化——收编 L001-4 设计稿 docs/design/storage-v2.md〔状态行同日 Proposed→Accepted 并注本 ADR〕；as-built 证据 = LOOP 003 双审 APPROVE×2 + 差分对拍 reports/compatibility/L003-storage-admin-diff.md；行为规格冻结源 = docs/reverse/storage/prune-gc-admin.md + binary-provider-chain.md〔U-STG-01 partial〕；known-divergence 存储域 INTENTIONAL 登记的 authority 本体——authority: { type: adr, ref: 本条 }）
- 日期: 2026-09-11
- 背景: L001-4 设计稿给出 storage 域 25 行裁定总表与终案建议，但按「裁定性内容走 DECISIONS.md 流程」纪律以 Proposed 状态留白编号待正式化。其后 LOOP 002 规格票（L002-3 → prune-gc-admin.md，E1-E10 端点表 + PruneStatusReport 字段级 schema + §6 差异清单 D1-D10）与 LOOP 003 实现票（storage 管理面 10 端点全交付 + Pruner 能力面，差分对拍 SAME×9/PARTIAL×1）把两 REFACTOR 小面落地；LOOP 004 Review B 上报③——设计稿仍 Proposed、prune 族新契约面缺 authority 锚、INTENTIONAL 5 项挂账「待 ADR authority」未清偿。本 ADR 正式化收编：终案裁定入册 + as-built 两处注记 + prune 族契约 authority 锚定。clean-room：行为依据 = docs/reverse/ 规格（B 活体优先 / A 反编译 / C 模板与官方文档）与官方 REST 文档，不以反编译结构为设计依据（ADR-0001）。
- 候选方案（设计稿 §6 四轴 + 本票新增 as-built 轴）:
  - ① GC 形态: A) 事件驱动管道对齐（node_events 分区表 → 批次供给 MinorGcCollector，batch/workers 参数族随之引入）；B) mark-sweep + hold-set + 删除前 Live 复核 + grace 维持（ADR-0031 as-built）。
  - ② 云链: A) 逐层复刻 `cache-fs(eventual(retry(s3-storage-v3)))` provider 包装（含 cache-fs 本地读缓存层）；B) `[filestore, s3] + dual-write` + fail-open 磁盘重试队列（ADR-0036/0040）等价能力维持。
  - ③ blob 寻址: A) 迁 sha1 前缀寻址对齐 `filestore/<sha1[:2]>/<sha1>`；B) `blobs/<sha256[:2]>/<sha256>` 维持（ADR-0006 布局承诺）。
  - ④ provider 广度: A) 24 模板矩阵全量对齐（模板速记展开器 + 20+ provider 类型）；B) 链形闭集（filestore/s3/双员迁移链）+ 保留名诚实拒绝维持。
  - ⑤ prune 进度面机制（本票新增轴——设计草案与 as-built 的分歧正式裁定）: A) 设计草案 §4-1 的「GCSweep 加可选 progress sink」（prune 语义寄居 GC 引擎，扫描进度回调缝）；B) 独立 `Pruner` optional facet——walk 语义（枚举 00..ff、逐分片套用 GCSweep 候选规则），删除纪律经共享 gcDeleteGate 与 GC 单源。
- 决策: **①B ②B ③B ④B ⑤B**。要点落册：
  1. **KEEP 主体（设计稿 §1 表 25 行判词分布，按行号精确入册）**：判词含 KEEP 共 **16 行**——纯 KEEP 11（#2 上传中断回收/#3 崩溃残留清理/#4 checksum 409/#5 checksum-deploy 400/#7 去重/#8 回收站/#14 -direct/#20 MPU〔ADR-0039 已裁〕/#22 空 blob 协议面/#23 binstore.yaml/#24 tmp 角色），KEEP+INTENTIONAL 复合 5（#1/#6/#10/#11/#13，定谳见下）；§2 数据完整性 P0 面 8 条（原子落盘/中断回滚/digest 强校验/去重不变量/GC 零误删/回收站引用阻断/备份保 mtime/checksum-deploy 409 面）全 KEEP——**任何后续存储改造不得回退**。**REWRITE = 无**：Go-native 等价物已覆盖 7.161 默认链（file-system）与企业 S3 链全部可观测行为面；剩余差距是产品范围（云厂商广度/HA/归档）不是实现质量。
  2. **INTENTIONAL 定谳 5+1 项（authority = 本 ADR；known-divergence INTENTIONAL_DIFFERENCE 登记引本条）**：
     ① **sha256 寻址**（#6）：`blobs/<sha256[:2]>/<sha256>` 维持；协议面兼容由附属摘要承载（sha1/md5 入 blobs 表 + X-Checksum 头族已 ✅）；跨产品磁盘级互换不在目标（迁移走 `bf migrate`，ADR-0024）。
     ② **uploads/ 暂存形态**（#1）：`uploads/<uuid>/data` 追加写→fsync→rename 维持；`_pre/dbRecord*.bin` 命名不迁移（ADR-0006 布局即兼容承诺；原子性语义等价——同分区 rename、崩溃无半写暴露；命名成分生成源未取证）。
     ③ **GC 非事件驱动**（#10）：mark-sweep + hold + Live + grace 维持（ADR-0031）；事件管道不进目标，批次/worker 参数族（batch=trashcanMaxSearchResults、workers=3）随之不进。
     ④ **不复刻 UNKNOWN 回收资格窗**（#11）：Artifactory 零引用 blob（含回收站节点全删）短窗内不回收的现象**原因 UNKNOWN**（binary-provider-chain §8-1 挂账）——不可能也不应该复刻原因未知的延迟行为（方向缺陷向）；BinFlow 资格 = 引用集判定 + 显式 grace 窗契约。§8-1 取证票若揭示机制，仅作记录不翻案。
     ⑤ **云链 Go-native 合并形态**（#13）：不逐层复刻 cache-fs/eventual/retry——dual-write + fail-open 磁盘重试队列（ADR-0036/0040）即 eventual+retry 的 Go-native 合并形态；cache-fs 本地读缓存层不做（filestore 形态即缓存，S3 形态本地腿已由 dual-write 磁盘腿覆盖）。
     ⑥ **存储面空 blob 不特化**（#22 后半，设计稿 §3 亦列，随本条一并定谳）：空 blob 在 checksum 寻址下即普通 blob；`empty` provider 存储面特化是客户端不可观测的内部优化——不做（§41 不为不可观测细节买单）；docker 空层 digest 协议面合成照旧（C17 SAME）。
  3. **REFACTOR 两小面——已落地收账（LOOP 002/003），本 ADR 追认 as-built 形态**：
     ① **prune/gc 管理面**（#9，原 P2）：`/api/system/storage/*` 10 端点族全交付（prune start/stop/status + gc + compress/optimize/backup/size/info/exportds）；差分对拍 SAME×9 / PARTIAL×1（L003-1 E1-E10；E7 backup 成功臂 cmd 接线欠账转派 conductor）；D1-D10 差异清单清偿见 L003-1 §4（D8 minor/FULL 节奏差维持 known-divergence）。
     ② **包型检索窗默认 21600s**（设计稿 §3-2，原 P1，remote 域）：docker/helmoci 内容类 TTL 默认 21600s 已端到端落地——authority 本体 = **ADR-0012 勘误三**（本 ADR 引用不重复展开；internal/remote/ttldefaults.go + repo facts/engine loadRepo 双消费同源解析）。
     **as-built 注记两处（实现与设计草案的字面分歧，按 as-built 落册——ADR-0041 体例显式声明，草案正文不回改）**：
     - **注记 A（prune 机制形态）**：实现为独立 `Pruner` optional facet（internal/storage/prune.go——MultipartUploads/SessionSweeper 的 capability-by-type-assertion 先例；缺席引擎对管理面诚实 503），**取代**设计草案 §4-1 的「GCSweep 加可选 progress sink」。walk 语义 = 枚举 00..ff 全 256 分片目录、逐分片套用 GCSweep 候选规则（mark 命中→hold 门→grace 窗→删除门）、stop 标记在目录边界消费（对齐活体「stop 后 ~1 目录内停下」粒度）。**删除纪律单源**：prune 与 GC 共享 gcDeleteGate 序列（gc.go「load-bearing sequence」：hold 集排除 + 删除前 Live 单点复核——ADR-0031 A+B 组合），无第二删除路径（LOOP 003 双审确认；iteration-003.md「GCSweep 四门同构」；prune_walk_test.go 钉死）。
     - **注记 B（TTL 已落地）**：见上 ②——设计稿落笔时该面尚在 REFACTOR 清单，现为 as-built 事实；authority 链 = ADR-0012 勘误三 → 本 ADR 追认。
  4. **prune 族契约 authority 锚（本 ADR 定谳；字面量以冻结源为准，见软缝协议）**：
     - **PruneStatusReport 26 字段/4 对象 schema**（顶层 6 键 + timing 6〔两处复用〕+ report 3 + lastHandledDirectory 5+timing；`progress:"N of 256"`、`yyyy-MM-dd'T'HH:mm:ss` 无时区、`HH:mm:ss.SSS` duration）：活体三态全捕获，BinFlow 侧逐键对照 SAME（L003-1 E3）；status 枚举 running|stopped|finished|error（error 态完整形态 V-4 待故障注入取证）。
     - **D6 dry-run 极性并存**：新 prune 面 Artifactory 极性（dryRun 缺省 false=真删；dryRun:true 只统计回显 cleaned=0）；旧 `/api/v1/system/gc` 的 apply 极性（缺省 false=dry-run）**不动——两族并存**（L003-1 §2）。
     - **gc 并发 409 信封**：BinFlow 统一 errors JSON 信封 + 持锁者诊断；Artifactory 侧 409+status 消息体仅反编译单源（**V-7 无活体，维持挂账**——活体取证若翻案走 Errata 回填，不静默改）。
     - 并发互斥码族：prune 重复启动 412（底层 409 资源层映射）；gc 409。报告持久化载体 = `<data>/prune_report.json` 原子写（temp+rename、终态 fsync、跨重启实测保持）= Artifactory configs 表 STORAGE_PRUNE_REPORT 的等价能力（BinFlow 无 configs KV 面——**载体差异随本条定谳 INTENTIONAL**）。
     - **冻结字面量源 = docs/reverse/storage/prune-gc-admin.md（§1 端点表/§2.2 schema/§6 D1-D10）+ reports/compatibility/L003-storage-admin-diff.md（E1-E10 对拍矩阵）**；storage-admin 契约化（LOOP 004 L004-3，compatibility-engineer 域）落 contracts/ 后，条目引本 ADR 为 authority。
  5. **UNSUPPORTED 族（本 ADR 裁定范围）**：GC 策略枚举/cluster singleton 调度（#12——单实例无 singleton 语义需求，HA 面随 #17）、归档/sharding/double-shards/redundant-shards（#16——企业冷存储/分片，无部署矩阵承载 ADR-0004）、full-db（#21——blob 不入 DB，与 ADR-0007 三层模型互补）、模板速记层（#23 附带——ADR-0036 模板条款维持，且被 U-STG-01 的 24 模板矩阵证据强化）。**用户待裁项不入本 ADR（维持 storage-v2.md §3 留缝状态，裁定权在用户——效力序：用户裁决 > 规格票 > ADR > PRD）**：GCS/Azure（#15——Backend 缝 ADR-0019 + binstore.yaml 保留名拒启已埋，是否进目标 = 产品裁定）、HA cluster 链（#17——wire 协议 UNKNOWN〔H1〕，gap 总账 NEW-BUILD 候选 ha）、云重定向 200KB/预签名 302（#18 UNKNOWN——MinIO 抓包取证挂账 §8-4，盲实现 = 猜测兼容行为，ADR-0001 红线）。
  6. **接口面**：storage.Engine/Backend/Session/GCMarker 接口**零改动**（裁定即「不动」）；Pruner 为 optional capability（type assertion 发现、缺席诚实 503），不破坏 Engine 接口闭集；UNKNOWN 挂账（云重定向/S3 exists-check/资格窗机制）维持取证后再裁，不猜测。
- 理由: 逐行证据链（binary-provider-chain B 活体优先、UNKNOWN 不猜测——ADR-0001）；57 节 §41 等价能力原则——24 模板矩阵是 Java 类层次表达，已有 Go-native 等价物的面不复刻形态，未有能力按部署矩阵与产品目标裁剪。**⑤B 的裁定理由**：prune 与 GC 的候选规则本就同构（分片目录扫描 + mark/hold/grace/Live 四门），独立 facet 让 prune 的 walk 生命周期状态（startFrom 续跑、stop 目录边界消费、逐目录报告累计）自持于单一组件——progress sink 方案会把这套状态机塞进 GC 引擎，删除纪律与生命周期状态双重所有权错位；共享 gcDeleteGate 保证删除纪律物理单源（双审确认无第二删除路径），「等价行为」与「机制不复制」同时成立。
- 后果: known-divergence 存储域 INTENTIONAL 条目（上列 5+1 + 报告持久化载体差异）可引本 ADR 为 authority；contracts/ storage-admin 条目（L004-3 产出）authority 链 = 本 ADR + 冻结源两文档。docs/design/storage-v2.md 状态行 Proposed→Accepted（注本 ADR）；其 §4-1 progress sink 措辞由本 ADR 注记 A 取代（设计稿正文不回改——效力 = ADR > 设计稿）。architecture.md 回写建议（§4 存储引擎节一行引用 + §11 技术债三行：云重定向 UNKNOWN、tempFolder 配额 P3、S3 exists-check 核对）维持 design §7-4 移交 conductor 派票——本票不写 architecture.md。E7 成功臂 cmd 接线（3 行）与 V-1/V-3/V-4/V-5/V-7 挂账移交后续差分票。**验证载体**：管理面 = REST 对拍 curl（参照腿 :8082 只读面 + BinFlow 腿全端点，L003-1 已立回归锚）；内容面 = 真实客户端（docker push/pull）；存储票硬门 = corruption/concurrency/recovery 三门；prune 删除纪律 = `go test -race ./internal/storage/...` + gcDeleteGate 共享断言（prune_walk_test.go）。HA/GCS/Azure/云重定向进用户裁定清单（conductor 在案），本 ADR 不预支。
- 交叉对账（显式列 Errata 候选，不静默改）:
  - **ADR-0031**: 无冲突且被强化——prune 经共享 gcDeleteGate 复用 A+B 组合（hold 门 + Live 复核），as-built 即其「删除纪律」单源延伸；gcDeleteGate 抽取自 GCSweep apply 序列，语义零漂移（双审确认）。
  - **ADR-0036**: 无冲突——链形闭集/保留名拒启/模板条款全部维持；GCS/Azure 保留名条款的「是否进目标」不在本 ADR 裁定范围（用户待裁）。
  - **ADR-0012**: 无冲突——TTL 21600s 的 authority 本体即其**勘误三**；本 ADR 仅追认落地（注记 B），无新增勘误。
  - **ADR-0047/0048**: 无冲突——remote 域（docker digest 门控/负缓存）与存储域正交；ADR-0048 的 Errata 触发条件（跨 missedTTL 窗复测）不受本 ADR 影响。
...
## ADR-0051: virtual 解析序两桶升四桶——cache 投影逻辑视图 + `<key>-cache` 派生注册表（ADR-0013 联动记录① 勘误；BIN-M1 / T-520）
- 状态: Accepted
- 日期: 2026-09-25
- 背景: T-516 逆向规格 `docs/reverse/virtual-resolution.md`（7.161.24 反编译单源；§1 成员展开/去重、§2 四桶序精确装配均高置信度）把 repo-semantics §8.1 的概览四桶精确化：成员声明序展开（嵌套 virtual DFS + visitedKeys 环剪断 + key 首现去重 + 缺失静默丢弃）→ 按类型分桶收集（locals 恒前于 remotes）→ priorityResolution 四段拼接；一个 remote 成员在序中贡献**两个实体**（cache 投影进 local 桶、本体进 remote 桶），cache 投影的优先标记**继承**其 remote。现行实现（internal/repo/virtual.go virtualMemberOrder）是 ADR-0013 联动记录① 依「ADR-0012 无 `<key>-cache` 影子仓投影」做的**有意两桶简化**。该简化在 BIN-M1（Maven 试点业务闭环）不再成立：两桶已表达 priority 越位，但表达不了 ① 同优先级内 local 类恒先于 remote 本体、② cache 副本命中与回源命中分离——而 §5.1 metadata 合并「按四段序逐成员且跳过全部 cache 仓」、§3.6 快照解析「跳过所有 cache 仓」必须以 ② 为前提（合并步无法区分缓存副本与上游文档，同一 remote 双读且跳过规则无处挂）；③ 嵌套 virtual 聚合拓扑（§1）为配置期拒绝，Maven 试点聚合分层仓需要它。勘误定位澄清（票面写「ADR-0012 勘误」）：两桶决策的**正文载体**是 ADR-0013 联动记录①，ADR-0012 是其引用前提（无影子投影）——故勘误行落 ADR-0013 条目，ADR-0012 条目追加引用面注记（机制零翻动）。
- 候选方案:
  - A) 维持两桶 + 优先标记补丁：不改 seam 与存储，但 D1/D2 语义（见设计文档 §2.4）无法表达——cache/本体不分离则 Maven 跳过规则无处挂，Maven 试点差分腿必败。
  - B) 四桶逻辑投影：节点存储零迁移（缓存行仍在 remote 仓自身命名空间，ADR-0012 落盘/再验证/负缓存协议全部不动），解析层引入 cache facet（零上游 standing-copy 命中步）+ internal/remote 提供 `<K>-cache` 派生投影注册表（继承 priorityResolution/handle* 等，remote-cache-projection.md §1.2 单源）。
  - C) 四桶物理投影：`<K>-cache` 建持久仓行 + 节点 repo_key 迁移。
- 决策: **B**。四段序 = [优先 local + 优先 cache 投影]（段内 real-local 先、cache 投影后，各按声明/遇到序）→ [优先 remote 本体] → [非优先 local + 非优先 cache 投影] → [非优先 remote 本体]；cache facet = 零上游 standing-copy 命中（过期副本亦服务——§2 结构读法，预登记 Errata 触发条件 O2）；remote 本体 facet = 既有 FR-20 全链原样；嵌套 virtual 解禁（DFS 展开 + 运行时环剪断，配置级不做全图环检测）。投影**不建持久仓行、不改任何存储面**（对齐参照自身「cache 仓非持久实体、重载派生」语义）。不设 feature flag（对齐 ADR-0050 决策 6 单窗口先例）。实现拆分：dev-go-core（internal/repo 序装配/walk/seam 扩展 + internal/repo/validate.go `-cache` 后缀建仓防护；internal/metadata 零 schema 变化）+ dev-go-storage（internal/remote 投影注册表，fetcher 零改动）+ dev-go-adapter 消费面（maven metadata 合并跳 cache 步、npm packument 缓存仓去重——加字段向后兼容，未处理方退回成员级语义无放大）。详细设计 = docs/design/virtual-four-bucket.md。
- 理由: A 在 BIN-M1 验收面前直接失效（差分必败项）；C 的节点迁移成本与收益不成比例，且参照的 cache 仓本就是内存派生视图非持久实体——B 以最小物理变更取得全语义，ADR-0012 的存储机制轴零翻动（其勘误一~四的 fetch 侧语义全部维持）。嵌套解禁顺带消除一处配置期人为不兼容（参照支持混排嵌套）。
- 后果: `VirtualMemberOrder` seam 增 Facet 维度（结构体加字段，15 个 adapter 消费方零破坏编译）；walk/browse/copy-move 源解析序全部换四段（同路径归属成员可能变化——browse display 归属差分关注 O5）；两桶断言测试翻新为不变量表（设计文档 §8 I1-I12）；`-cache` 后缀 key 进建仓校验保留字。验证载体：mvn CLI（快照解析 + metadata 合并）+ curl 双发差分腿，重点臂 O1/O2/O3。暂缓面（登记设计文档 §9 F1-F8）：`<K>-cache` 直访、REST 存储面 locals→caches、搜索域 cache key 映射、统计路径重写、zap、virtual patterns 三层过滤、远端抑制 header、§3 深水语义。
- 软缝协议（Errata 触发条件预登记，效力序 = 用户裁决 > 规格票 > ADR > PRD）: ① **O1** 段内 real-local 与 cache 投影相对次序（§9.1 推演序被差分推翻时仅动段内两类次序，机制轴不动）；② **O2** cache 步过期副本无条件服务（参照若实测为「过期降级到本体步再验证」则 cache 步命中判定加检索窗检查）；③ **O3** 新鲜负缓存 + 过期 standing copy 并存经 virtual 的表现（若参照 404 则 cache 步先查负缓存）；④ **O4** 嵌套 + 混布局路径翻译；⑤ **O5** browse display 归属。任一触发走 Errata 回填，不静默改册。规格源为反编译单源（参照实例凭据 pending，规格 §9.5 自记高风险）——本 ADR 的语义断言以差分腿为准绳。
