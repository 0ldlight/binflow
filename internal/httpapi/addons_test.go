...
func TestAddonsEndpointRoles(t *testing.T) {
	...
	} else if rows, _ := addonRows(t, body); len(rows) != 12 {
		t.Fatalf("admin GET carries %d slots, want 12: %s", len(rows), body)
	}
...
func TestAddonsEndpointCommunityFloor(t *testing.T) {
	...
	wantOrder := []string{"generic", "docker", "maven", "npm", "pypi", "go", "nuget", "cargo", "properties", "repo-operations", "ha", "xray-integration"}
