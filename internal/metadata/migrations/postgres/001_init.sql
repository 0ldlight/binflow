-- 001_init.sql (postgres dialect) — initial BinFlow metadata schema.
--
-- Contract: docs/design/architecture.md section 6 (M1 revision), mirroring the
-- sqlite dialect file 001 one-to-one (ADR-0007 lockstep). Conventions follow
-- the shipped postgres files (009/012/021): timestamps are RFC3339 UTC text,
-- booleans are INTEGER 0/1 (the Go store layer writes ints via boolToInt),
-- and auto-increment ids are SERIAL. The schema_migrations bookkeeping table
-- is created by the migrator itself (see internal/metadata/migrate.go), not
-- by this file.
...
CREATE TABLE remote_configs (
  ...
  unreachable_mask INTEGER NOT NULL DEFAULT 0
);
...
CREATE TABLE tokens (
  id           SERIAL PRIMARY KEY,         -- sqlite: ROWID alias; postgres: SERIAL (dialect-local, ADR-0007 note)
