-- 002_docker.sql (postgres dialect) — docker domain tables (M2, ADR-0010).
--
-- Contract: docs/design/architecture.md section 6, "002_docker.sql" block,
-- mirroring the sqlite dialect file 002 one-to-one (ADR-0007 lockstep). The
-- statements are inside the SQLite/Postgres common subset, so they carry
-- over verbatim; only this header differs. No transaction statements in the
-- file body — the migrator wraps each migration in one transaction.
--
