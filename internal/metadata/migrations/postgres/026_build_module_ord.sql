-- 026_build_module_ord.sql (postgres dialect) — module identity becomes the
...
-- carries the sqlite dialect one verbatim (ADR-0007 lockstep rule; the
-- postgres migrator is not embedded yet — see ../migrations/postgres/
-- README.md). Window functions, ALTER ... RENAME and DROP ... IF EXISTS are
-- common-subset spellings; no transaction statements in the body — the
-- migrator wraps each migration in one transaction. The rebuild is version-gated (applied versions are skipped);
