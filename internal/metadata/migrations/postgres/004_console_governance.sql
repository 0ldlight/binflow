-- 004_console_governance.sql (postgres dialect) — console session, groups
-- and governance schema (M4, ADR-0014/0015 + the T-108 errata).
--
-- Contract: docs/design/architecture.md section 6, "004_console_governance.sql"
-- block, mirroring the sqlite dialect file 004 one-to-one (ADR-0007
-- lockstep). Apart from groups.id (SERIAL per the 001 id convention) the
-- statements are inside the SQLite/Postgres common subset and carry over
-- verbatim; only this header differs. Conventions: RFC3339 UTC text
-- timestamps, booleans as INTEGER 0/1, no transaction statements in the
-- file body.
