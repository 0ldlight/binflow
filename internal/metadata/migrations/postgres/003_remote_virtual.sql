-- 003_remote_virtual.sql (postgres dialect) — remote proxy + virtual domain
-- schema (M3, ADR-0012/0013).
--
-- Contract: docs/design/architecture.md section 6, "003_remote_virtual.sql"
-- block, mirroring the sqlite dialect file 003 one-to-one (ADR-0007
-- lockstep). The statements are inside the SQLite/Postgres common subset
-- (ADD COLUMN ... NOT NULL DEFAULT, RENAME COLUMN, CREATE INDEX), so they
-- carry over verbatim; only this header differs.
--
