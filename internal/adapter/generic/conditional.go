...
// Range and conditional-request semantics for downloads (rest-api.md 1.4,
// high confidence; PRD FR-4-AC14/AC15).
//
// BinFlow's contract, decided by this ticket:
//
...
//     Content-Range: bytes */<total>. A syntactically invalid byte-range-set
//     has no meaningful first-pos, so RFC 9110's unsatisfied-range form is
//     the only Content-Range a 416 here can carry; the Artifactory shape
//     (bytes */<total>) is kept for every 416.
//   - Non-"bytes" units and multi-range sets are NOT implemented -> the
//     header is ignored and the full 200 body is served (must never 5xx).
//
// The If-None-Match comparison is weak (RFC 9110 13.1.2): BinFlow's ETag is
// an unquoted sha1, but clients quote it or prefix W/ with abandon, so both
// spellings compare equal after stripping. If-None-Match wins over
// If-Modified-Since when both are present (RFC 9110 13.2.2 evaluation
// order).
...
func (p httpRangeParser) parseRange(header string) (r httpRange, malformed bool, ignore bool) {
	...
	if !strings.EqualFold(strings.TrimSpace(spec[:eq]), "bytes") {
		return httpRange{}, false, true // other units (items=...) are ignored
	}
	set := strings.TrimSpace(spec[eq+1:])
	// An empty byte-range-set (bytes=) is an invalid ranges-specifier; a
	// server MAY ignore it (RFC 9110 14.2) — ignoring keeps the client's
	// download working. Multi-range sets are explicitly out of scope.
...
func (p httpRangeParser) parseOne(spec string) (httpRange, bool) {
	...
	case first == "": // suffix: bytes=-N, the final N bytes
		...
		if n == 0 || p.total == 0 {
			return httpRange{}, true // zero-length suffix, or an empty entity: unsatisfiable
		}
		if n > p.total {
	...
	case last == "": // open: bytes=N-, from N to EOF (bounded by total)
		...
		if p.total == 0 || start >= p.total {
			return httpRange{}, true // empty entity, or bytes=total- and beyond
		}
...
// If-None-Match is evaluated and If-Modified-Since is ignored (regardless of
// the match outcome — a failing If-None-Match is still authoritative).
// Returns true when the store is fresh for this client: answer 304.
func evalConditional(r *http.Request, etag string, lastModified time.Time) bool {
	if inm := r.Header.Get("If-None-Match"); inm != "" {
		// Present but not matching: the precondition fails, If-Modified-Since
		// is not consulted, and the store serves 200.
		return etagMatch(inm, etag)
	}
	if ims := r.Header.Get("If-Modified-Since"); ims != "" && !lastModified.IsZero() {
		t, err := http.ParseTime(ims)
		if err != nil {
			return false // unparseable date: treat as not modified-since, serve 200
		}
		// Truncation to whole seconds mirrors Last-Modified's granularity
		// (http.ServeContent does the same before comparing).
...
// list. BinFlow's stored form is an unquoted sha1, so every client spelling
// — bare, "quoted", W/"weak" — normalizes to the bare digest before
// comparing; "*" matches any existing representation.
...
// remainder: the value BinFlow stores is an opaque hex digest, so this is
// token shaping, not case folding of a meaningful field.
