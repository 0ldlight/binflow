...
type Options struct {
	...
	Signer RepomdSigner
	// SpoolDir roots the PUT spool files (T-476, the T-474 family): the
	// .rpm body stages to disk — the header parse and the landing both
	// re-read it, and packages reach tens of megabytes. Hardened
	// deployments — read-only rootfs, the kubernetes norm — mount no
	// writable /tmp, which the OS-temp spool of the pre-T-476 code
	// assumed (the same family as the UAT helm/nuget push incidents).
	// The cmd assembly passes <storage data_dir>/staging — the SAME
...
func (h *Handler) serveUploadRpm(ctx context.Context, w http.ResponseWriter, r *http.Request, repoKey, rel string) {
	...
	if err != nil {
		...
		slog.ErrorContext(ctx, "rpm: package upload spool failed",
	...
	f, err := os.Open(spoolPath) //nolint:gosec // G304: our own staged spool path, never client input
	if err != nil {
		slog.ErrorContext(ctx, "rpm: package upload spool reopen failed",
			slog.String("repo", repoKey), slog.String("path", rel), slog.String("error", err.Error()))
...
func spoolBody(dir string, body io.Reader) (string, error) {
	f, err := adapter.StageFile(dir, "binflow-rpm-*.rpm")
	if err != nil {
		return "", err
	}
...
// msgSpoolUnavailable is the PUT staging refusal (T-476, the T-474
