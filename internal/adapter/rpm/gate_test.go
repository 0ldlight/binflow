...
// The license-gating integration legs (the T-285..T-309 verification form,
// through the rpm adapter's own surface):
//
//   - D3: community tier (no document) refuses the rpm repository create
...
//     X-Binflow-License-Required: rpm) while reads keep serving (D1).
...
func newLicenseKeys(t *testing.T) *licenseKeys {
	...
	return &licenseKeys{kid: "t311-test", pub: pub, priv: priv}
}
...
func newAddonsSeam() *addonsRegistrySeam {
	return &addonsRegistrySeam{reg: addons.New(addons.Generic(), addons.Rpm())}
}
...
func newLicensedStack(t *testing.T) (*stack, *licenseKeys) {
	...
	s := newStackOpt(t, stackOptions{addons: newAddonsSeam(), keys: keys, dataDir: t.TempDir()})
	if s.license == nil {
...
// createRpmRepo issues the REST repository create (the D3 surface).
func (s *stack) createRpmRepo(t *testing.T, key, rclass string) (int, string) {
	t.Helper()
	body := fmt.Sprintf(`{"key":%q,"rclass":%q,"packageType":"rpm"}`, key, rclass)
...
// TestGateD3CommunityRefusesCreate: no document installed → the rpm create
// answers the D3 400 naming the addon and the tier gap.
func TestGateD3CommunityRefusesCreate(t *testing.T) {
	s, _ := newLicensedStack(t)
	status, body := s.createRpmRepo(t, "rpm-local", "local")
	if status != http.StatusBadRequest {
		t.Fatalf("community rpm create status = %d, want 400 (body %s)", status, body)
	}
	for _, token := range []string{"rpm", "pro"} {
...
// TestGateProLifecycle: install pro → create 200 + package PUT 201;
// uninstall → PUT 403 with the X-Binflow-License-Required marker while
// reads keep serving (D1 + D2), create returns to the D3 400.
func TestGateProLifecycle(t *testing.T) {
	s, keys := newLicensedStack(t)
	ctx := context.Background()

	if status, _ := s.createRpmRepo(t, "rpm-local", "local"); status != http.StatusBadRequest {
	...
	if status, body := s.createRpmRepo(t, "rpm-local", "local"); status != http.StatusOK {
		t.Fatalf("pro create status = %d, want 200 (body %s)", status, body)
	}

	pkg := fixturePackage("gated-1.0-1.noarch.rpm", gatedHeader("gated"))
	if status, body, _ := s.put("/binflow/rpm-local/gated-1.0-1.noarch.rpm", pkg, nil); status != http.StatusCreated {
		t.Fatalf("pro package PUT status = %d, want 201 (body %s)", status, body)
	}
	...
	pkg2 := fixturePackage("gated-2.0-1.noarch.rpm", gatedHeader2("gated"))
	status, body, hdr := s.put("/binflow/rpm-local/gated-2.0-1.noarch.rpm", pkg2, nil)
	if status != http.StatusForbidden {
		t.Fatalf("post-uninstall package PUT status = %d, want 403 (body %s)", status, body)
	}
	if got := hdr.Get("X-Binflow-License-Required"); got != "rpm" {
		t.Errorf("X-Binflow-License-Required = %q, want rpm", got)
	}
	status, body, _ = s.get("/binflow/rpm-local/gated-1.0-1.noarch.rpm")
	...
	if status, b2 := s.createRpmRepo(t, "rpm-other", "local"); status != http.StatusBadRequest {
		t.Errorf("post-uninstall create status = %d, want 400 (body %s)", status, b2)
	}
}

// gatedHeader is a minimal parseable header for the gate legs.
func gatedHeader(name string) *headerBuilder {
	b := &headerBuilder{}
	b.str(tagName, name)
	b.str(tagVersion, "1.0")
	b.str(tagRelease, "1")
	b.str(tagArch, "noarch")
	return b
}

// gatedHeader2 is the second version's shape.
func gatedHeader2(name string) *headerBuilder {
	b := gatedHeader(name)
	b.str(tagVersion, "2.0")
	return b
}
