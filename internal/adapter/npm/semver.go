...
// Semver validation and precedence, node-semver's strict rules (the npm
// registry rejects leading zeros — "1.02.3" is not a version; publish chain
// step 5, spec section 2.3). Hand-rolled: ADR-0005 keeps go.mod stdlib-only.

// semver is one parsed npm version.
type semver struct {
	major, minor, patch uint64
	pre                 string // prerelease without the leading '-', "" when none
	build               string // build metadata without '+', "" when none (ignored in ordering)
}
...
// zeros ("0" itself legal, "01" not — the npm registry rule).
...
// alphanumerics and hyphens, numeric identifiers additionally free of
// leading zeros in the PRERELEASE (build metadata exempts them, semver
// section 10).
...
// isNumericIdentifier reports whether id is all digits (used by ordering).
...
// compareSemver orders two version strings by semver precedence:
// -1 when a < b, 0 when equal (build metadata ignored), +1 when a > b.
// Invalid spellings order by their raw string (a stable total order for map
// keys; validation is a separate concern the publish chain owns).
...
// latestVersion picks the greatest key of versions by semver precedence
// (ties broken lexically for determinism). "" when the map is empty.
func latestVersion(versions map[string]any) string {
	best := ""
