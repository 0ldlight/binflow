...
// engine, real repo.Service (remote pull-through engine included), the real
// auth chain and the real httpapi router, with the goproxy adapter under
// test mounted beside the generic one — the pypi harness posture, so every
// assertion crosses the full middleware chain (auth gates, the addon write
// gate when a registry is mounted, prefix stripping).
type stack struct {
	t       *testing.T
	srv     *httptest.Server
	st      storage.Engine
	md      metadata.Store
	svc     repo.Service
	license *license.Manager // non-nil on the licensed assembly (gate legs)
}

// newStack builds the default stack: anonymous reads on, no addon registry
// (the pre-M10 posture — the gate legs in gate_test.go build their own).
func newStack(t *testing.T) *stack {
	t.Helper()
	return newStackOpt(t, stackOptions{})
}

// stackOptions tunes the assembly: a non-nil keys builds a REAL
// license.Manager over the stack's own licenses store (test keypair through
// the constructor seam), attaches the D3 gate onto repo.Service and mounts
// the addon registry — the licensed posture cmd assembly serves.
type stackOptions struct {
	addons *addonsRegistrySeam
	keys   *licenseKeys
}

// newStackOpt assembles the stack.
func newStackOpt(t *testing.T, opt stackOptions) *stack {
	...
	// The provider registration feeds classifyPath/upstreamPathFor (the
	// remote legs depend on it); the duplicate guard makes repeated stacks
	// safe. The handler itself stays OUT of the global adapter registry —
	// httpapi mounts Deps.Adapters explicitly.
	RegisterMetadata()
	handler := New(svc, md.Repos(), md.Blobs())
...
// seedRepo writes a repository row directly through the metadata store (the
// pypi harness posture: the validation matrix is not under test here; the
// adapter only reads the row's class and package type).
...
// seedRemoteConfig attaches one remote_configs row (loopback upstreams need
// the SSRF exemption — the admin-set flag ADR-0012 defines).
...
// local-first resolution, goproxy.md section 6.3).
...
// adminPrincipal is the service-face admin caller (repo fakes' admin()
// shape, for the few legs that exercise svc directly).
func adminPrincipal() *repo.Principal { return &repo.Principal{Name: adminUser, Admin: true} }
