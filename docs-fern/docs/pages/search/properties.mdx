...
挂在既有 `/api/storage/{repo}/{path}` 路由上；**读**走制品读门，**写**要求该路径的 `w`（属性是元数据不是内容——不要求 `d`）。
...
| 403 `permission denied: writing properties requires write access on the item` | 有读无写 | 给 principal 授该路径 `w`（不需要 `d`） |
