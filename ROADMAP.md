...
- [ ] 「M13 未纳入项」对账：收口时建「M14 未纳入项」段（DoD#7 字面；M15+ 主轴候选第一顺位 = AQL + 老搜索专程——UI-parity 让位留痕 + Replay/D1 busy/成员同型滚程登记；**备稿已就绪——T-395 v1.1，见下段，收口窗启用**）

### M14 未纳入项（**备稿**——滚入 M15+ 候选池；T-395 2026-08-31 起草，收口窗由 conductor 启用并按届时 as-built 修订勾稽；DoD#7 对账）

> **状态：备稿未启用**——PM 预备文本（沿 M13 R5 先例），启用与内容定稿归 conductor 收口动作；「票级遗留」以下以 2026-08-31 23:3x 票据状态（M14 12/22 + T-402 增补）为基线，启用时逐条复核仍成立/已消化。
