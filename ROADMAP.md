...
### M15 — 搜索基建专程（AQL 首程）：AQL 查询语言与执行引擎 + 老搜索首批端点 + virtual 聚合浏览收口 + 复制包 B 首批（**`m15-done` 2026-09-02**；PRD v1.2 终版——25 票全落〔T-434 树栈系 M16 先行票不计〕+ 条件票 T-403 未触发留痕；T-430 终验 PASS：L20~L34 全过〔L21 PRD 原样命令活体——D-T421-1 修复终核〕+ §5.7 十七族活体对账 + DoD 八条 + 144 提交归属 100% + E1~E7 零倒退 + 断言反转四处终核；**Q1 终裁：维持分阶段切分，M16 statistics 域优先**）
...
### M15 未纳入项（滚入 M16+ 候选池；T-427 起草，**m15-done 收口窗已启用 2026-09-02**——与全量审计产出双源对账 + T-430 §5 三处增补；DoD#7 对账）

> **收口窗增补（T-430 §5）**：① server 裸二进制 40.59MB 首破 40MB 原始线（WARN-only 面——M16 观察项，与 F1 聚合门 104.52MB PASS 并行登记）；② offline 三处预存在（make 注释断链 / SHA256SUMS 自指 / install 30s 硬编码窗——候小票）；③ T-431 已执行（docker×virtual 开禁落 as-built——清理条件票出口行）；④ t422:79 spec 的 `.invalid` DNS 形态在本机 clash fake-IP 下不可达（环境定性——spec 改用非 DNS 不可达表达，候小票）。
