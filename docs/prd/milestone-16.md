...
> **v1.2 收口笔（2026-09-06——T-460）：Q8~Q12 终裁归位（候裁层清零：Q8/Q9 已裁落章核对 / Q10 条件窗关闭〔PM 裁不转正〕/ Q11 裁点未至滚 M17+ / Q12 移交 conductor）+ K67~K73 回填定案 + B 偏差 47 项〔表列 48 行 + 后补 B-3.20 = 49 行〕四态收口审计核对〔零无主——B-2.8/B-3.2 两无主候选本笔核定〕+ 勘误五条集中登记（§7 v1.2 收口总账）+ ROADMAP「M16 未纳入项」备稿（沿 T-395/T-427 先例）+ M17 衔接核对（PRODUCT.md 修订未发生——M17 维持预立项不启动）**。
...
| v1.2 | 2026-09-06 | **收口笔（T-460）**：① §7 Q8~Q12 终裁归位（表内标注 + v1.2 收口总账段——「待裁」层清零）；② §5.6 K67~K72 回填定案 + 新增 K73（repoLayoutRef 布局联动评估——B-1.5 承接票前置，T-439 漂移登记归位）；③ B 47 项收口审计四态核对结论（§7 总账——49 行终态详表归 T-460 日志，T-466 对账输入；B-2.8/B-3.2 两无主候选核定）；④ 勘误五条集中登记（§7 总账——正文不改写，验收命令以勘误为准）；⑤ ROADMAP「M16 未纳入项」备稿 + M17 预立项段衔接核对（PRODUCT.md「明确不做」未修订——门槛未发生不启动） |
...
| # | 项 | v1.0 暂行值 / 校准路径 → **v1.2 定案（收口笔回填，T-460）** |
|---|---|---|
| K67 | 树头工具带断言锚（facet 复选组形态 / Sort-by 选项集 / Compacted 呈现差异 / My Favorites 持久化载体） | V 式复核（t226 树页）+ reverse §3.2 回填后冻结 → **已冻结**：parity 册 §11（树栈四项 as-built K67-1~4——T-437 冻结/T-434 as-built：目录前文件后 / select≠expand / TAB 省略规范形 / 工具带三行带 + rclass 三态无第四态 + facet 动态实有型）+ §11.2 分页控件（T-451 落地——档位 [20/50/100/200/1000] 缺省 100）；7.161 ag-grid 翻页形态分歧留痕候 ux 复核（7.84 锚维持） |
| K68 | Annotate 语义映射与 Property Set 域（annotate ↔ M10 属性三动词对位 / write→deploy-cache 迁移映射与呈现粒度 / Property\|Property Set 分段的 BE 属性集配置小域去留） | Q7 终裁 + architect 会签（ADR-0026 增补评审）→ **已定案**：动词域 wire 五词（GET 正名单 read/annotate/deploy-cache/delete/manage；PUT 收 write 别名——M17 别名移除评估登记）+ 迁移 023 双库 + dry-run 公共 API 零提权（NFR-S77）+ FE 独立位列零联动（T-444 拆分语义下 UI 联动 = 越权——唯一正确形态）；**Property Set 不建不登记**（缺位——BE 属性集小域另立票候选）；低置信两登记：动作字母 a vs rest-api.md 勘误行 n（reverse 勘误候选）/ migration 011 满载 flake（QA 在案） |
| K69 | 统计基建计数口径（直连/经 virtual/remote 缓存命中三分——remote_download_count 区分边界；last_downloaded_by 可见性权限面） | architect 会签 schema 时定案 → **已定案**：nodes 四列 + 三分口径（直连 / 经 virtual〔virtual 命中给**实际存储成员行**计数〕/ remote 缓存命中）+ **FileInfo 非敏感投影与 `?stats` 敏感面分治**（CapSystemRead 门——ADR-0044 K69.4 正位，as-built T-438、消费 T-445）+ `remote_*` 族与下游 smart remote 回拉统计**正交**（T-435 防错条）+ statisticsEnabled 行为化 / sourceOrigin 落库顺车；单源契约声明（零第二计数通道） |
| K70 | cron 调度域形态（表达式子集 / next-run 计算 / 与事件驱动 sweep 引擎边界 / 消费方注册面） | Q1 终裁引入后独立 ADR 承载 → **已定案**：ADR-0044 Accepted + cron-scheduling.md（Quartz 六域子集**自研解析器**——robfig/cron 方言判断否决）+ next-run 纯函数 + schedules 台账（ScheduleStore 五方法）+ 独立 1min ticker（domain 回调）+ 防护三面（过去时间拒配/每域并发上限〔域内串行〕/误触发）+ **只触发全量类任务**（事件驱动仍是增量唯一引擎——零重复投递）；三消费面 as-built（GC 三槽 + 备份定时 CRUD + import/export + 复制 cron 字段）；gc-cron-gap（Quota/Compress/Prune 无载体）诚实缺位登记 |
| K71 | 远端浏览可选档语义细节（`listRemoteFolderItems` 对位 wire / 三型枚举形态 / 上游故障降级 / virtual 口径扩面） | remote-browsing.md（T-425 §1/§2 成稿）→ **已定案**：批 1 三型（helm classic index 全树 / deb / rpm 元数据枚举）；wire = `configuration.listRemoteFolderItems` **指针布尔**（显式 false 往返——D-T456-1 修复；PUT 全量 remote config 语义）+ 默认 false off diff=0；on 态派生行 display-only + 点击回源 + `?stats` 单源计数；上游降级 = `FolderInfo.remoteDegraded` 上 wire（D-T461-1）+ 已缓存行不整树塌；§8.5 扩面（on 时 virtual 树含远端成员行）；generic/maven HTML 抓取与五型无根 API 物理不做面维持；枚举快照 TTL 600s 非 wire 可调（登记） |
| K72 | QRL 全量参数域（三态语义 / 指标 job 周期 / REST wire / 可配置化边界——默认值维持 K63 定案 1000/4/10s） | aql.md 增量段（inv-1 §E QRL 锚 + 官方文档）→ **已定案**：`v1/system/query_rate_limiter` 三态 REST（admin 门——非 admin 403）+ 指标 job 采样 + **与 K63 门读数一致**（默认值维持 1000/4/10s）；QRL 与 AQL 429 并发闸**正交**（QRL 限流 = 延迟放行非 429——T-435 发现③）；QRL audit 词补登记（T-452 遗留） |
| **K73**（v1.2 新增） | repoLayoutRef 布局联动评估（B-1.5 承接票前置——T-439 契约漂移登记归位） | BinFlow 布局由协议 adapter 固定、**无 repoLayoutRef 消费方**——BE 承接票（configJSON 转发 + round-trip）落地时须接线布局引擎或钉协议默认值；候「M16 未纳入项」BE 承接票立项定案 → **已落定（M17 T-490，2026-09-06）**：出口②**钉协议默认值**——字段 local 臂全量承接（转发+落库+回显 verbatim）、不接布局引擎、值域不设门（scenario D 迁移容忍，field-only 先例），ADR-0003 勘误行留痕；Stage 域 wire 键同票定案（`environments` canonical + `stages` 别名、分歧 400）；FE 预留位转正与 t439 漂移钉 spec 翻绿归 **T-519** 对账 |
...
| Q8（C8） | 表单域豁免缺口：maxUniqueSnapshots/repoLayoutRef（M4 延期裁定）+ blackedOut/archiveBrowsingEnabled（无豁免条目）——补齐范围 | ① 全补（表单域 + 行为语义随票——FR-143.2）② 只补 UI 域（行为化另票） | **①全补**——API 已收全，行为联动随票评估（repoLayoutRef 留 K 项） | 批次②断言冻结前；FR-143 → **【已裁——出口①（PM 倾向随 T-437 册冻结落章），as-built 修正：全补限 FE 面】**：T-439 八域表驱动全落（预留位 + API 漂移钉 tripwire）；活体对账证「PUT 全收」仅到**解码层**（configJSON 四域静默丢弃——契约漂移登记）→ **BE round-trip 承接缺口滚「M16 未纳入项」**（承接票 + 行为联动 + repoLayoutRef = K73）；Force Auth 域注勘误见 §7 v1.2 总账④ |
| Q9（C9） | 自有增强层去留：仓库详情中间页 / children 表复合 / E4 测试器·diff / 快搜范围页签缺位 / 重置钮 / mimeType·校验块 | 逐项三态（维持自有登记 / 收敛对齐 / 条件保留） | children 表**收窄**（主路径走树叶子）/ 详情中间页**维持** / E4 **维持**（A8）/ 重置钮**移除** / 快搜范围页签**维持缺位**（R2 既定）/ 校验块收进伴随形态 | 批次①③断言冻结前；FR-142/144 → **【已裁——PM 倾向九项全数随票兑现落章（v1.2 核对）】**：children 表收窄（批次①）/ 重置钮移除（T-439——B-3.11）/ 校验块·mimeType 收伴随菜单（T-447——B-2.3/B-2.12）/「类型」列收敛（T-443——B-3.9）/ B-2.4 多出列维持自有（T-445 对账）/ 详情中间页维持（§9A-S4）/ E4 维持（§9A-S1）/ 快搜范围页签维持缺位（§9A-S7）；候裁残留 B-3.19 行内编辑钮随本笔**维持 L2/E1 豁免登记**（V4 实证行尾仅 trash——超集自有增强） |
| Q10（C10） | 条件票机制 vs 直接排期：NuGet symbol（五承未触发）/ docker virtual（Q6 已裁开禁）/ Tokens 字段核验（候活体源） | M16 转正排期 or 续滚 | docker virtual **滚入 M16 首票**（已终裁）/ NuGet symbol **六承转正排期**（P2 尾波——mini as-built 规格随票）/ Tokens 核验**维持候源不排期** | 尾波条件窗 → **【已裁——PM（T-460）：条件窗关闭，NuGet symbol 不转正】**：材料 = BOARD 全程零用户触发信号（intake ⑤~㉑ 无一涉及）+ FE/BE lane 收官（i18n 独占波后无票位、release/终验在即）+ T-467 未派发零沉没成本——**七承滚「M16 未纳入项」（M17+ 候选池）**，BOARD 留痕归 conductor m16-done 收口窗；docker virtual 已执行（T-431，m15-done 前）；Tokens 核验维持候源；**用户点名即翻**（M17+ 首票或 m16-done 前插空——conductor 裁量） |
| Q11（C11） | webhook 57 型休眠裁剪（ADR-0041 决策 7 翻转路径在案）——剔 xray 域后非 xray 触发源裁剪待裁 | ① 裁剪（仅注册有源域——非 xray 域触发源激活清单随票）② 全量注册（含无事件休眠型） | **①裁剪**——与 ADR-0041「有源域」防线一致 | 尾波（随 webhook 域二程评估票）→ **【裁点未至——维持 ADR-0041 决策 7 现行态，滚 M17+】**（v1.2 归位）：M16 全程无 webhook 域票（触发源织入面零变化——audit 词表扩容系 cron 域非 webhook 面）；现行态 = 36 型注册（wired = artifact/artifactProperty/docker 族；dormant 如实标注零伪造）；PM 出口①倾向**维持不变**；裁点滚 M17+ webhook 域二程评估票，与 M17 build-info 域落地联动（dormant→wired 翻转路径在案——schema 零变化） |
| Q12（C12） | t381 事故残留（VM 取证快照 15MB + 空仓；REST 删除被 OSS license 门挡） | M16 清理窗处置 or 维持登记 | conductor 决定项（非 PM 裁量）——建议尾波随 release 票处置 | 尾波（conductor）→ **【移交 conductor（v1.2 归位）】**：维持登记——VM 取证快照 15MB + `t381-ui-probe` 空仓（REST 删除被 OSS license 门挡）；建议随 T-465 release 票处置或继续登记，m16-done 收口窗决定（移交清单位 T-460 日志 §6） |
...
> **v1.2 收口总账（T-460 收口笔，2026-09-06）**：
> ① **Q8~Q12 归位完毕——Q 表 13 项全数闭环**：Q8/Q9 已裁落章核对（PM 倾向出口随 T-437 册冻结 + 批次②③①各票 as-built 全数兑现——Q8 全补限 FE 面修正留痕）；Q10 条件窗关闭（PM 裁：NuGet symbol 不转正，七承滚 M17+ 候选池——用户点名即翻）；Q11 裁点未至（M16 无 webhook 域票——维持 ADR-0041 决策 7 现行态滚 M17+，与 build-info 域 dormant→wired 联动）；Q12 移交 conductor（t381 残留——建议随 T-465 处置或继续登记）。
> ② **B 偏差 47 项收口审计四态核对完毕——零无主**：parity 册 §12 预归属表（48 行）+ 后补 B-3.20（FR-147 票内补行）= **49 行逐行对账**（详表归 T-460 日志 §2，T-466 QA 终验对账输入）；终态分布 = **翻正已落 31 / 翻正部分落（单侧落或候裁缺口登记滚程）8 / 翻正未落滚程 1（B-3.2）/ 豁免·登记 1（B-2.8 本笔核定）/ 豁免·复核维持 3 / stay-out 确认 2 / 对账去重 3**。两无主候选本笔核定：**B-2.8（有效权限渲染重载体）= 豁免·差异登记**——chip 列表维持 + 授予 target 可见性增补并入 M17+ 候选（零用户信号 + 收口窗无票位 + 权限对齐面已由 T-455 五列矩阵承载）；**B-3.2（初始态首仓库自动选中）= 翻正未落滚「M16 未纳入项」**——T-434 票面四项外、T-445 无顺车、FE lane 收官（conductor 可裁 m16-done 前插空 FE 单票，否则 M17+）。
> ③ **K67~K73 回填定案**（§5.6 表内标注）：K67 树栈 + 分页 as-built 冻结（parity §11/§11.2）/ K68 动词域 wire 五词 + Property Set 不建 / K69 四列三分口径 + ?stats 面分治 / K70 Quartz 六域子集自研 + 全量类任务边界 / K71 批 1 三型 + 指针布尔 wire + 降级字段上 wire / K72 QRL 三态 + 与 429 闸正交 / **K73 新增**（repoLayoutRef 布局联动——B-1.5 承接票前置）。
> ④ **勘误登记（正文不改写——验收命令与下游引用以本条为准）**：(a) FR-148.1/LC-93/L42 的 `usageSince` 系起草暂名，Artifactory wire 与 as-built 均为 **`notUsedSince`**（T-435 三源发现 + T-440 落地）；(b) FR-143.3/LC-84/L36 的 modal 880px 系 7.84 估值，7.161 实测 **924px**（T-441 as-built——M1 行再修订已落 parity v1.6）；(c) FR-143.2「Force Authentication（virtual）」域注勘误——实字段为 **forceConanAuthentication（local × conan）**，Artifactory 侧同域（T-439 活体实证）；(d) Environments 在 7.161 已更名 **Stage**（Stages & Lifecycle——T-439；BE 承接票随域定名）；(e) 本 PRD 所称「parity 册 v1.2」= 实序 **v1.5 起（M16 修订版，现行 v1.14）**——版本映射双处留痕（T-437 票内裁决 1）。
> ⑤ **ROADMAP「M16 未纳入项」备稿就绪**（沿 T-395/T-427 先例——m16-done 收口窗启用）+ **M17 衔接核对**：PRODUCT.md「明确不做」五条原样未修订（HA / Xray / LDAP·SAML·OIDC / 洞察报表 / 全量 REST）——**M17 前置门槛（修订解禁）未发生，维持预立项段不启动、不派票**（§0.3 判定兑现；M17 立项稿候 m16-done 后 PM 起草，定容时与未纳入项段双源对账）。
