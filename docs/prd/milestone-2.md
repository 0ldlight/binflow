...
| 状态 | **v1.5**（L002-4 勘误 **C8**：v1.2/C3「/v2/token 错误体全 OAuth」的 **401 臂退役**——差分实证为 Artifactory 通用错误模型 errors 数组 pretty 形态（L000-B E1-4/E1-5 + L001-1 复验 C02 SAME）；C3 的 400 参数臂维持；FR-11/§5.2/NFR-S10/AC4 六处联动收口；服务端 as-built 已先行（L001-1），本版为 PRD 追认。v1.4（T-121：T-105 QA 勘误 E1 收口——D06/D10 的 upload `Location` 为相对路径，命令物化 `$REG` 前缀 + FR-8/DE-02 注记，行为零变更。v1.3（Q1/ADR-0010、§6.5 定案无遗留；T-43 三项勘误已收口；T-44 四项勘误收口：ping 无条件挑战（C6/D44-1）、offline_token 接受并忽略（C5/D44-2）、token POST 表单凭据口径（C4/D44-3）、删光 manifest 后 tags/list 200 空（C7/D44-6））） |
...
| v1.5 | 2026-09-11 | L002-4 勘误一项收口（**C8**——LOOP 001 差分实证推翻 v1.2/C3 的 401 臂；C 编号延续 T-43 C1~C3、T-44 C4~C7 序列）：① **C8 主体**——`/v2/token` 端点 **401 错误体（凭据错与匿名拒绝）退役 v1.2/C3 的 OAuth 形**，改判 **Artifactory 通用错误模型** `{"errors":[{"status":401,"message":"Bad Credentials"}]}`（Jackson pretty、`status` 字段、无 `code`；匿名拒绝 message=`Authentication is required`，E1-4）。**Authority=差分证据（合法证据源）**：reports/compatibility/L000-docker-remote-evidence.md **E1-4/E1-5**（E4 运行时实测，Artifactory-pro 7.161.20 rev 86120900，参照实例 :8082；取证面=remote 仓，但 `/v2/token` 为 local/remote **共享自答端点**——E1-2 同一 DockerV2AuthenticationFilter/DockerResourceBase，裁定适用于端点本体双面）+ L000-docker-remote-diff.md **L001-1 复验段 C02**（E5 双系统差分 **SAME 灭**：BinFlow UAT :8083 回同形 body，真实 docker CLI 27.5.1 `docker login` 坏密码渲染 `unknown: Bad Credentials` 与 E1-5 客户端指纹逐字一致）。v1.2/C3 两条裁定理由随之勘误：**(a)**「docker 客户端不解析 401 body，无兼容性损失」不成立——CLI 实际渲染 body 消息文案（无 `code` 时前缀 `unknown:`），错误体是客户端可见面；**(b)**「与 M1 管理面端点 E-17 同族」的一致性论据让位于参照实例实证（parity 优先）。② **C3 残余有效域显式化**——400 参数错（`unsupported_grant_type`/method gate 等**未观察角落**）**维持 OAuth 形**：E4/E5 未覆盖、无翻案证据，as-built 同口径（internal/adapter/docker/token.go 注释即此分臂；代码注释不具退役 PRD 裁定的效力——Review B 点名，本版使 PRD 与 as-built 对齐）。③ **联动收口六处**：FR-11 行为规格错误体段、FR-11 双入口「统一 OAuth」表述、§5.2 错误契约例外句、NFR-S10、§6 落地影响②、FR-11-AC4 断言（改锚 `unknown: Bad Credentials` 指纹）。对账：known-divergence `docker/remote-credentials-error-shape` 已 resolved(parity)（2026-09-11）、契约条目 `docker/remote-token-bad-credentials` VERIFIED。**服务端行为零变更**（L001-1 已实现并差分复验，本版为 PRD 追认收口） |
...
- **双 token 入口并存（设计而非矛盾）**：
  ...
  - 两入口签发的 token 落同一 token 表、共用 M1 的吊销端点与验签链（FR-11-AC6 联动）；差异在授权模型（admin 管理全量 vs 用户换自身 scope）与错误码语义（管理面非 admin → 403；`/v2/token` 参数错 → 400 / 凭据错 → 401）——错误体格式两入口**各归其位**（v1.5/C8 勘误 v1.2/C3 的「统一 OAuth」）：`/v2/token` 的 401 臂=Artifactory 通用错误模型（errors 数组 pretty 形态）、400 参数臂=OAuth 形；管理面入口维持 OAuth 形（E-17，自有面，无对拍义务）。
...
| FR-11-AC4 | 错误凭据 `docker login` → 非 0 退出码，输出含 `unknown: Bad Credentials`（v1.5/C8：CLI 对无 `code` 错误体显示 `unknown:` 前缀——L001-1 复验 C02 实测指纹，v1.2 的 `unauthorized` 预期随 OAuth 形退役）；服务端 401 body = `{"errors":[{"status":401,"message":"Bad Credentials"}]}`（通用错误模型 pretty 形态；不泄露用户是否存在） | P0 |
...
错误契约（v1.2/C3 收敛）：`/v2/**` **资源端点**的错误体用 **distribution spec 格式** `{"errors":[{"code":"<CODE>","message":"<msg>","detail":<opt>}]}`（如 `BLOB_UNKNOWN`/`DIGEST_INVALID`/`MANIFEST_UNKNOWN`/`NAME_UNKNOWN`/`UNAUTHORIZED`/`DENIED`/`UNSUPPORTED`）；**例外（v1.5/C8 勘误 v1.2/C3）：`/v2/token` 端点按状态码分形——401（凭据错/匿名拒绝）= Artifactory 通用错误模型 `{"errors":[{"status":401,"message":"Bad Credentials"}]}`（pretty、`status` 字段、无 `code`；差分实证 L000-B E1-4/E1-5 + L001-1 复验 C02 SAME），400 参数错（未观察角落）= OAuth 形 `{"error":"<code>","error_description":"<msg>"}`**。M1 管理面 token 端点（`/binflow/api/security/token`，E-17）维持 OAuth 形（自有面，无对拍义务）；docker 域分层（延续 M1 §5.1 三分层注）修正为：资源层=spec 形、token 层 401=通用错误模型、token 层 400=OAuth 形——「同一（端点×状态码族）单一形态」取代 v1.2 的「token 端点全 OAuth」收敛口径（NFR-S10 同步勘误）。
...
| NFR-S10 | docker 域错误体形态按**（端点×状态码族）锚定参照、逐族单一**（v1.5/C8 勘误 v1.2/C3 收敛口径）：`/v2/**` 资源端点全部 spec 错误体；`/v2/token` 401（凭据错/匿名拒）= Artifactory 通用错误模型 errors 数组 pretty 形态、400 参数错 = OAuth 形——同一（端点×状态码族）内不出现双格式；token 端点内 401/400 异形为 Artifactory 实况指纹而非混用违规（remote 面禁推 400 通用模型与 manifest/blob 404 spec 形并存同证，E5-4），DE-16 边界内不出现 E-01 | D12/D24/D04c |
...
**落地影响（随定案生效）**：① M1 E-26 对 `/v2/` 的 404 断言在 M2 起失效，回归基线以 DE-01（200 `{}`）为准；② `/v2/**` 全域是 docker adapter 的路由领地，资源端点错误用 spec 格式、`/v2/token` 401 用 Artifactory 通用错误模型 / 400 参数错用 OAuth 形（NFR-S10，v1.5/C8 勘误口径）；③ `Docker-Distribution-Api-Version: registry/2.0` 头全端点带上（docker-registry.md §0/§10-7）。
