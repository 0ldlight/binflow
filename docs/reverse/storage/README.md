...
| `prune-gc-admin.md` | **prune/gc 管理面**（§1：/api/system/storage 十端点族——gc/prune 三件套/compress/optimize/backup/size/info；§2：PruneStatusReport 26 字段 schema；§3：同步/异步语义与流式体；§5：GC 调度 cron 出厂 `0 0 /4 * * ?`；§6：BinFlow 十条差异） | gc=同步流式 200、prune=202 job 化、并发 412、GC 20 轮夹 FULL | 7.161.20 活体 + 7.161.24 反编译 + docs.jfrog.com |
...
6. **`gcConfig.cronExp=0 0 /4 * * ?` 调度语义**：**已收口**（prune-gc-admin.md §5：模板+活体 config+GcSchedulerHandler 三源——自托管 Quartz cron 即调度源；AoL 才走 gc.intervalSecs 生成）。
