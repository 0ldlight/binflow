...
> 适用版本：M10（license/addon 体系随 M10 交付；ADR-0032/ADR-0033 为设计依据）。REST 面见 [API 参考 · M10 新增端点速览](../api-reference.md#m10-新增端点速览t-296)；行为规格锚：`docs/design/architecture.md` §15。
...
- **装 license 解锁档位**：`pro` 解锁 go/nuget/cargo 三个包型；`enterprise` 追加 HA / Xray 集成槽位（本体 M11+）。
...
| `pro` | 中档 | 追加 go / nuget / cargo 三个包型（M10 试点） |
...
M10 装配的 11 个槽位（`internal/addons/slots.go` 为单一事实源）：
...
| `ha` | feature | High Availability | **enterprise** | 槽位占位（本体 M11+） |
| `xray-integration` | feature | Xray Integration | **enterprise** | 槽位占位（本体 M11+） |

档位 × 解锁数速查：community 6 槽（五核心 + properties）→ pro 9 槽（+go/nuget/cargo）→ enterprise 11 槽（+ha/xray-integration）。
