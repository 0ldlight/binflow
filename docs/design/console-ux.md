...
| 版本 | 日期 | 变更 |
|---|---|---|---|
| v1.47 | 2026-09-07 | T-514 FE bundle 面 + Any Distribution 预置勾选（M17 W7，FR-153.2/.3——B-2.16 预置桶缺位解除〔T-491 wire 三字面 / T-513 查询族的两条消费腿〕）：① **T-514 批 16 名锚入册**（bundle 三视图族 `bundles-page`/`bundles-table`/行族 `bundles-row-<name>`/`bundles-empty` + `bundle-versions-page`/`bundle-versions-table`/行族 `bundle-version-row-<version>` + `bundle-detail-page`/`bundle-detail-info`/`bundle-detail-signature`/`bundle-checksum`/`bundle-artifacts`/行族 `bundle-artifact-row-<i>`/`bundle-not-found`〔404 承载〕/`bundle-denied`〔403 承载——版本/描述符面拒绝即答案〕/`bundle-state`〔状态徽章〕+ 预置桶注记 `perm-buckets-note`——明细见 §10.5 T-514 批块）；**`perm-repo-pick-<key>` 冻结族扩桶字面成员**（ANY LOCAL/ANY REMOTE/ANY DISTRIBUTION——TransferBox label 槽：键 = wire 字面、展示 = §3.8 活体拼写 Any Local 等）；② **导航挂靠定案 = 应用模式「应用」分组第三条目**（/bundles——console-ui §1.1 应用域族；官方 /ui/repobundles 归 Artifactory 应用域，OSS 7.84 册无 Distribution 不列该条目，官方文档归属为据；专域名词对位 Access Tokens/Webhooks 先例不译；新图标 `bundle` 自绘两屉堆叠）；③ **三视图一组件**（ArtifactsBrowser 先例——/bundles〔名单 GET /api/release/bundles〕→ /bundles/:name〔版本单〕→ /bundles/:name/:version〔描述符 + HEAD X-Checksum-Sha256 校验和行〕；读门语义如实：无授予用户名单 200 空集走空态、单查 404 与不存在同形〔bundle-not-found〕；pending 清单行〔sha256=''〕如实呈现 —；创建面缺位登记——POST /api/release/bundle 注记引导 API，控制台创建面归后续票）；④ **三预置桶同场勾选**（两步弹窗选仓步预置行与具体仓库混选；m-holder 403 分支桶照常可勾——桶字面不依赖目录端点；两处「通配桶不建」注记退役换语义注记）；⑤ i18n 新域 `bundles`（21 键，webhooks 一页组一域先例）+ security 域 6 键（三桶 note + 两注记 + chips title）；⑥ e2e 新面 `web/e2e/m17/t514-bundles-presets.spec.ts`（四腿：三桶同场勾选+payload/echo/水合 / Any Distribution 授予面覆盖探针〔被授权三级钻取 + 未授予空集、版本/描述符面 403、admin 真缺 404〕/ 导航入口+名单双主题 axe / 版本单 404（admin 真缺臂））——pro license 自铸安装〔t461 先例〕、bundle 无删除端点故夹具长存留痕；⑦ 服务端 diff=0（纯 FE 票） |
...
└──────────────────────────────────────────────────────────────────────────────┘
┌ 备份 / 恢复（/admin/monitoring/backup）── T-462 回写：定时 CRUD + CLI 两卡 ──┐
│ 定时备份卡：New Backup 表单（Enabled / Backup Key / Cron Expression /        │
│   Next Backup Time / Server Path）→ 列表 Key·cron·下次备份·启用·上次运行·      │
│   路径·操作（编辑/E1 删除）；到点服务端全实例导出（与 CLI export 同载体，       │
│   产物 <path>/<key>-<时间戳>）；仓子集/incremental/retention/zip 无载体缺位    │
│ 导入/导出 CLI 卡：交互式 export/import 不做（/api/export/** 404 有意、        │
│   import CLI-only——ADR-0015 勘误②）；命令块（export / import）+ 定时备份      │
│   同载体注记 + CopyButton                                                     │
└──────────────────────────────────────────────────────────────────────────────┘
...
| `/bundles`、`/bundles/:name`、`/bundles/:name/:version` | **T-514 批** `bundles-*` 名单族 + `bundle-*` 版本单/描述符族（见下）——三视图一组件（ArtifactsBrowser 先例） | **T-514（FR-153.3）**：Release Bundles 应用域新页（应用分组第三条目）——只读查询面消费 T-513 查询族；读门语义如实（无授予 200 空集 / 404 同形） |
...
**T-453 批用户/组路由表单化 + 能力位三旗锚册（9 名新增 + 0 退役，
M16 批次④ B9，FR-145.1/.3——断言反转④〔Q5 出口①路由化〕：parity 册
E5 注销条目的兑现票；7.161.20 活体探针 2026-09-04〔reports/agents/
t453-probe/——/ui/admin/management/{users,groups}/new 整页表单，页脚
Cancel｜Reset〔初始禁置〕｜Save〔初始禁置〕，用户表单 Retype 在场〕；
先入册再落码，v1.40；消费 spec = web/e2e/m16/t453-route-forms.spec.ts
〔本票新增〕+ m8 users-groups / m9 users-groups / m14 t384 / security /
m8 a11y-sweep 翻新）：**

```
路由页根（7.161.20 同构）：
  user-create-page（/admin/security/users/new 页根——静态段优先于
    :name 段，「new」永不落入用户详情路由）
  group-form-page（/admin/security/groups/new 与 /admin/security/groups/
    :name/edit 两态共用页根——组编辑随内联卡〔创建/编辑同卡〕一并
    路由化；E5 404 = 空态回列表、403 = 无权限卡）
只读注记（readonly_admin 深链防御——L4 列表门外的路由页兜底面，
    服务端 403 终裁）：
  user-create-readonly-note（用户创建页）
  group-form-readonly-note（组表单页）
能力位三旗预留位族（FR-145.3——BE 创建臂〔PUT userCreateBody〕与
    部分更新臂〔POST userUpdateBody〕均未承接三域，T-439 预留位
    两档纪律：恒禁用 + 零提交 + hint 如实标注；现值 = BE 回显出厂档
    〔profileUpdatable 恒 true / 两 disable 恒 false〕，与 7.161.20
    活体默认档同值；转正触发器 = t453 spec 的 API 漂移钉腿）：
  user-form-reserved-caps（三旗组根——user-form-section-options 内）
  user-form-profile-updatable（Can Update Profile——语义：取消后用户
    不能自助改档案）
  user-form-disable-ui（Disable UI Access——语义〔登录被拒臂〕：勾选后
    UI 登录被拒，API/Token 面不受影响）
  user-form-disable-internal-password（Disable Internal Password Login
    ——语义〔密码改道臂〕：内部口令登录停用，认证走外部 IdP）
管理位候裁臂（双布尔 vs 枚举——ADR-0026 暂行维持三值枚举 + 差异登记）：
  user-form-role-parity-note（枚举下拉旁附注——7.161 表单 = Administer
    Platform + Manage Resources 双布尔〔另 Platform Auditor / Manage
    Webhook〕；BinFlow readonly_admin 无双布尔对位，不建不伪造）
载体迁移零改名：user-form 与 group-form 两表单卡全族（节锚
  user-form-section-{settings|options|password|groups} /
  group-form-section-{settings|members}；字段锚 user-form-name、
  user-form-email、user-form-role、user-form-enabled、user-form-password、
  user-form-password2、user-form-error 与 group-form-name、
  group-form-description；页脚 user-form-cancel、user-form-reset、
  user-form-submit 与 group-form-cancel、group-form-reset、
  group-form-submit；动态族 user-form-group-<name> 与
  group-form-member-<name>；容器 user-form-groups 与
  group-form-members）自列表内联展开卡迁路由页，锚名零变化
  （m14 t384 断言反转④改写承载）
```

路由/形态留痕（T-453，dev-frontend 回写）：① 创建入口 = 导航（users-
create / groups-create 锚保持，点击即路由——列表页 DOM 无表单卡，
退役断言 = t453 腿②）；② 页脚 **Reset 保留**（7.161 活体实测在场且初始
禁置——Reset dirty 门 + Save 必填门；与 T-439 建仓表单移除重置钮的 Q9
处置为**页面级差异化配置**，差异登记 parity 册 §3 M3 行）；③ 创建表单补
Retype（user-form-password2 复用编辑页锚——7.161 活体在场，T-384 期
「创建态无 Retype」定案翻案留痕）；④ 组表单字段域裁定维持（External
ID / Platform Administrator / Manage Resources / Manage Webhook /
Automatically Join——BinFlow 组模型无对位域〔console-m8 §6.10「无外部
组模型」+ RBAC 模型 §5〔组不承载角色语义——ADR-0026 同源裁定〕〕，不建不做缺位登记）；⑤ 组编辑器数据面 =
E5 单组读 + E2 users〔候选/成员落盘快照〕+ 权限 targets 三请求（m9
spec 编辑腿数据面口径同步改写——still 与用户数无关、零逐用户扇出）；
⑥ 服务端 diff=0（纯 FE 票）。

**T-455 批权限矩阵五列 + 两步弹窗步头锚册（2 名新增 + 0 退役，M16 批次④
B10 票①，FR-146.1 FE 消费面——T-444 wire 翻新〔write→deploy-cache 拆分〕
+ parity B-1.6/B-2.16 翻正；7.161.20 活体探针 2026-09-04〔reports/agents/
t455-probe/——Users/Groups 矩阵五列 Read/Annotate/Deploy-Cache/
Delete-Overwrite/Manage + Add Repositories 双步头常驻可点〕；v1.41；消费
spec = web/e2e/m16/t455-perm-five-columns.spec.ts〔本票新增〕+ m8
permissions / m9 permissions-mholder / security / rbac 翻新）：**

```
两步资源对话框可点步头（对位 7.161.20 活体 Add Repositories 弹窗的
    「1 Select Repositories」「2 Set Patterns (Optional)」双步头——常驻
    可点直跳〔非当前步亦可点〕、aria-current="step" 标当前步；footer 的
    perm-res-next/上一步链与 perm-res-step 步数行维持冻结锚零回退）：
  perm-res-step-1（步头 1「选择仓库」）
  perm-res-step-2（步头 2「设置模式（可选）」——可选语义进步头文案，
    活体同形）
矩阵五列（零新族名——冻结族扩成员）：
  perm-matrix-cell-<kind>-<principal>-<action> 扩 annotate 成员（action
    域 = read/annotate/write/delete/manage——perm-matrix{,-groups} 表锚
    与 {user,group}-perm-matrix 汇总表零改名，列序对位活体；write 列
    testid 维持 write〔兼容窗锚冻结〕，wire 层经 api.ts 归一/序列化）
```

动词域/联动留痕（T-455，dev-frontend 回写）：① wire 双层收口两点——
水合 normalizePermActions（GET 正名单 deploy-cache → UI 词 write，展示面
漂移消解）与保存 wireActions（write → 正名单 deploy-cache，往返稳定；
PUT 的 write 别名仅收不发）；② 勾选联动裁定 = **独立位列零联动**（活体
联动形态在 0 仓参照实例不可观测——矩阵门在资源型+行选定之后，登记不
伪造；T-444 拆分语义 write 不携带 annotate〔零提权〕，UI 联动会越权）；
③ GET echo 断言四处翻新（security:175 / m8 permissions:170,437 / m9
mholder:200）；④ 活体差异登记：7.161.20 权限新建/编辑是三步向导路由页
（Resources/Users/Groups panel-*）+ Builds/Release Bundles 资源型——
...
**T-462 批 GC/备份 cron 消费面锚册（38 名新增 + 0 退役，M16 批次④ B14，
FR-145.7——parity B-1.9 + B-1.10 翻正承载〔M15 Q5 推翻的呈现面〕；契约 =
T-450 三消费面 REST〔维护三槽 GET/PUT /api/v1/system/maintenance、备份
CRUD /api/v1/system/backups、replications cron_exp——wire UAT 实测
reports/agents/t462-probe/〕；形态依据 = console-ui.md §3.9〔维护三区块 +
Run Now + Reset/Save〕/§3.10〔备份列表 + New Backup 表单〕+ ADR-0044
决策 7①②④；v1.45；消费 spec = web/e2e/m16/t462-cron-backups.spec.ts
〔本票新增〕+ t404〔payload 键集 + 预留位组随刻意变更〕+ repo-policy-keys
〔audit picker 计数随刻意变更〕）：**

```
维护（GC）页定时卡（/admin/monitoring/gc——存储概况与迁移面板之间）：
  gc-cron（卡根——三 cron 槽 + 手动面并存注记）
  gc-cron-table（三槽行表）
  gc-cron-row-<slot>（行族——slot ∈ gc | cleanup-unused-cache |
    cleanup-virtual，wire 键原样；GET 投影无行 = 未调度缺省态）
  gc-cron-input-<slot>（表达式输入——mono；Quartz 六/七域，placeholder
    0 0 /4 * * ?；行内错误 = 服务端 400 点名文案〔Invalid cronExp …〕）
  gc-cron-save-<slot>（保存——PUT 单槽臂 {slot:{cronExp}}；空输入禁用
    〔取消调度走「清除」——单态：无行 = 不调度〕）
  gc-cron-clear-<slot>（清除——PUT {cronExp:""} 删台账行；未调度槽禁用）
  gc-cron-run-<slot>（立即清理族——仅两 cleanup 槽：POST /system/cleanup
    {apply:true}〔T-324 手动面，Trigger=manual 与调度 fire 同载体〕+
    危险确认；两槽同走全量 pass〔无 virtual-only 载体，C 层差异注记〕）
  gc-cron-run-gc（gc 槽手动执行——滚向本页危险区的入口〔Run Now =
    既有 dry-run/apply 面，ADR-0044 决策 7①「并存维持」〕）
  gc-cron-next-<slot>（下次运行——mono UTC 秒精度；停用 = Chip）
  gc-cron-last-<slot>（上次运行/结果——mono + lastStatus/lastError title）
  gc-cron-readonly-note（只读管理员注记——写面 system:write 禁用；
    无独立 403 降级锚——卡仅对 admin/readonly_admin 渲染，普通 user 的
    L2 收敛由 stats 无权限卡承载）
  gc-cron-gap（7.161 区块 2/5 缺位注记——Quota 百分比 / Compress 内部库 /
    Prune 无 BinFlow 后端载体，如实缺位 + 服务状态页台账指引）
备份页（/admin/monitoring/backup——定时 CRUD 卡 + CLI 卡两段）：
  backup-crud（定时备份卡根）
  backup-table（列表——§3.10 列集对位：Key/cron/下次备份/启用/上次运行/
    导出路径/操作；Repositories 列无载体缺位〔导出恒全实例快照〕）
  backup-new（New Backup 入口——列表上方 + 空态主行动；readonly 不渲染）
  backup-row-<key>（行族——key 即 backupKey）
  backup-edit-<key> / backup-delete-<key>（行内动作）
  backup-delete-confirm-key（E1 删除确认输入——错名不动/对名放行）
  backup-empty（空态——净实例零预置〔Artifactory 出厂 backup-daily/
    weekly 为有意差异，ADR-0044 登记〕）
  backup-form（New Backup / 编辑表单根——§3.10 Backup Settings 对位）
  backup-form-key（Backup Key——创建态可输〔FE 镜像 validBackupKey〕，
    编辑态锁定 kv）
  backup-form-key-error（key 形态行内预检）
  backup-form-cron（Cron Expression——服务端校验权威，400 点名行内）
  backup-form-next（Next Backup Time——datetime-local 本地时区 →
    RFC3339 UTC〔localInputToRFC3339 复用〕；过去时刻 400 拒；未给 =
    表达式推算——编辑态不回填已算出的 next-run）
  backup-form-path（Server Path For Backup——绝对路径 + 禁 ..，FE 廉价
    形态门 + fire 时服务端边界校验；Browse 钮无服务器侧浏览端点不建）
  backup-form-enabled（启用复选——停用 = 配置保留不调度）
  backup-form-gap（§3.10 Advanced/Repositories 双列缺位注记——仓子集/
    incremental/retention/zip/邮件告警无载体）
  backup-form-error（保存错误行内——400 族原样〔lang=en〕）
  backup-form-save / backup-form-cancel（表单动作对）
  backup-denied / backup-readonly-note（无权读注记 / 只读注记）
  backup-cli（导入/导出 CLI 卡根——ADR-0015 勘误②边界维持：交互式
    export/import 不做〔/api/export/** 404 有意、import CLI-only〕；
    一次性 export / 恢复命令块 + 定时备份同载体注记；backup-cmd-*
    维持 §10.6 退役不复活）
复制域（cron 字段消费——M15 Q5 推翻 / Q1 终裁，FR-150.4）：
  repl-form-cron（预留位转正——真输入、随体提交〔cron_exp 键，空 = 纯
    事件轨〕、编辑态预填；锚名零改，t404 断言翻新〔disabled → enabled +
    payload 键集 +cron_exp〕）
  repl-form-cron-hint（双轨语义注记——调度只触发全量对账，事件轨承载
    增量，同制品不双推）
  repl-row-sched-<name>（仓编辑节列表「调度」列——cron + 下次同步 /
    事件驱动；启停镜像台账〔停 = park、启 = 重臂〕）
  repl-sched-<i>（治理复制页 targets 行调度列——GET /v1/replications
    cron 投影按 id join〔status 面不携 cron〕；读失败降 '—' 不牵连
    状态面）
```

路由/形态留痕（T-462，dev-frontend 回写）：① 维护面形态 = 三槽行表非
7.161 的整页表单+底部 Reset/Save——BinFlow 的 PUT 是逐槽臂（先验后写），
行级保存/清除即最小臂面；§3.9 区块序对位〔GC → 危险区、两 cleanup →
立即清理、Quota/Compress/Prune → 缺位注记〕。② 备份 nextBackupTime 是
「首跑时刻」可写位（§3.10 表单在案）——服务端语义：显式给则覆盖推算值
（过去 400）；列表 Next Schedule Backup 列即台账 next_run 回显。③
runCleanupNow 的 Toast 汇总（objectsCleaned/bytesReclaimed/gracePending）
= repo.CleanupReport 聚合字段——手动面不写台账行（lastRun 仍属调度
...
**T-514 批 FE bundle 面 + Any Distribution 预置勾选锚册（16 名新增 + 0
退役，M17 W7，FR-153.2/.3——B-2.16 预置桶缺位解除；wire 依据 T-491〔通配
桶三字面〕+ T-513〔查询族端点——版本/描述符面对无读门会话 403 的实态与
T-513 报告「版本面 404」表述不符，票日志契约漂移登记；list 行形为平台
自有 echo 形 spec-pending 在案〕；v1.47；消费 spec = web/e2e/m17/
t514-bundles-presets.spec.ts〔本票新增〕）：**

```
Release Bundles 三视图（新页组 web/src/pages/bundles/——三视图一组件，
  URL 即状态；应用分组第三条目，导航挂靠票内定案见版本行②）：
  bundles-page（名单视图根）/ bundles-table（名单表）/
    bundles-row-<name>（名行族——GET /api/release/bundles 行）/
    bundles-empty（可见集空态——服务端过滤如实呈现，不伪造 403）
  bundle-versions-page（版本单根）/ bundle-versions-table（版本表）/
    bundle-version-row-<version>（版本行族——state 徽章 + created）
  bundle-detail-page（描述符根）/ bundle-detail-info（kv 信息卡）/
    bundle-detail-signature（签名 = 清单身份摘要占位，mono+拷贝）/
    bundle-checksum（HEAD X-Checksum-Sha256 描述符校验和，64 hex mono）/
    bundle-artifacts（清单表——Pager 页窗）/ bundle-artifact-row-<i>
    （清单行族——sha256/size 快照行在场；pending 行 sha256='' 如实 —）/
    bundle-not-found（404 承载——系统读权限持有者的真缺/可见名零版本）/
    bundle-denied（403 承载——版本/描述符面对无读门会话：拒绝即答案，
    不与 404 混同）/ bundle-state（状态徽章：COMPLETE success /
    INPROGRESS warning / 闭集外 default——未来状态机扩词零破相）
两步弹窗预置桶勾选腿（B-2.16 缺位解除）：
  perm-buckets-note（选仓步预置桶语义注记——正常与 m-holder 403 两分支
    各一；T-455 期「通配桶不建」注记退役换语义注记）
  perm-repo-pick-<key> 冻结族扩桶字面成员（ANY LOCAL / ANY REMOTE /
    ANY DISTRIBUTION——TransferBox 新 label 槽：键 = wire 字面、展示 =
    console-ui §3.8 活体拼写 Any Local / Any Remote / Any Distribution）
```

动词域/联动留痕（T-514，dev-frontend 回写）：① 预置桶 wire 语义 =
T-491 承接（repos[] 收三字面、GET echo 原样、第四桶 ANY 拒收）——FE 零
判定，勾选即提交字面；② Any Distribution 授予面探针在 e2e（被授权用户
/bundles 三级钻取可见 + 未授予用户空集与单查 404 同形）；③ bundle 创建
面（POST /api/release/bundle）控制台不建——名单页注记引导 API（pro 槽位
门 + 显式清单形），创建腿归后续票；④ i18n 新域 bundles（一页组一域，
webhooks 先例）——en 双语填充 21 + security 6 键；⑤ 服务端 diff=0
（纯 FE 票）。
...
族，翻页由页码序列/边界钮承载）；T-453〔v1.40〕**零退役**（user-form-*/
group-form-* 全族载体迁移路由页零改名）
