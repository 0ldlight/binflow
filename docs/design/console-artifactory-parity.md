...
| 状态 | v1.14（2026-09-05，T-462 as-built 留痕：**B-1.9 + B-1.10 翻正收官**——维护面三 cron 槽〔Run Now=既有手动面并存〕+ 备份定时 CRUD + import/export CLI 卡 + 复制 cron 转正三域消费〔FR-145.7，M15 Q5 推翻的呈现承载〕；此前 v1.13 T-461 远端浏览可选档 B-3.20：**B-3.20 新行——remote 远端浏览可选档已落**：表单 Advanced 复选〔批 1 型 helm/debian/rpm 呈现域——与官方五型开放面刻意不同，generic/maven HTML 抓取明确不做差异登记〕+ 详情回显 + 树 on/off 双态消费〔display-only 派生行 + 点击回源 + ?stats 单源计数 + §8.5 virtual 扩面〕+ 降级三面〔缓存行不整树塌/点击错误态/wire note FE 已消费——**BE httpapi note 渲染腿在途契约缺口登记**〕；D-T456-1 wire 腿端到端实证；锚册 v1.44；此前 v1.12 T-459 监控组三页 + 导航分组：**B-1.11 + B-2.18 翻正已落**——监控组扩为服务节点组六页〔存储/服务状态/系统日志/系统信息/维护（GC）/备份恢复——后三页归位迁址，T-462 cron 消费面同场挂靠〕+ Service Status 页〔health/version/schedules 三现役读面零新端点；Uptime 无端点如实缺位〕+ System Logs 查看器〔**载体票内定案：日志源 = 审计跟踪 GET /api/v1/audit**——服务进程日志无 REST 端点〔契约漂移登记 + BE 建议票〕；7.161 三选择器如实降为单源说明行；尾随 7s 倒计时 + Pause/Refresh now 三件套对位；下载 = 当前窗口 Blob 导出〔BinFlow 无 Support Zone 如实换形〕〕+ 导航分组对齐〔Webhooks 归常规组 / 治理收四页 / 管理侧栏 16→18 条目 + 五路由迁址旧深链一轮折入〕+ 认证组子项形态缺位登记〔7.161 Authentication 六子项——LDAP/SAML SSO/OAuth SSO/HTTP SSO/Crowd·JIRA/SCIM 探针实据；后三域 BinFlow 不存在〔FR-92 闭集〕单页三页签维持〕+ 侧栏 Search Admin Resources 过滤框〔**位置按 7.161 活体勘误 = 管理态顶栏 340px 单框**〔A1-5〕——票面原词「侧栏」为 7.84 经典审计口径；活体上过滤无可观测效果，过滤语义按经典行为〔客户端子串窄化管理侧栏〕承载，差异留痕〕；此前 v1.11 T-453 用户/组路由表单化〔断言反转④兑现〕） |
| 维护者 | ux-designer |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.14 | 2026-09-05 | **T-462 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B14 承载——FR-145.7 落地，**M15 Q5 推翻的呈现承载**——B-1.9 + B-1.10 翻正收官）。§12 两行翻「已落」：**B-1.9**（维护面三 cron 槽——GC/Cleanup 两族各带表达式 + next-run + Run Now；Run Now=既有手动面并存〔ADR-0044 决策 7①〕：gc 槽即危险区 dry-run/apply、cleanup 两槽 POST /system/cleanup；Quota/Compress/Prune 无载体缺位注记）+ **B-1.10**（备份定时 CRUD〔New Backup 表单软缝⑦四字段 + exportPath + nextBackupTime 可写位——过去 400；列表 + E1 删除〕+ import/export 管理页 = CLI 卡同页〔ADR-0015 勘误②边界维持〕）；**复制 cron 字段 FE 兑现**（R3/R4 v1.5 对账注的 T-462 承载落地：repl-form-cron 预留位转正——真输入、随体提交、编辑预填 + 下次同步回显；治理复制页 targets 调度列 join 配置面投影〔status 面不携 cron〕；t404 payload 键集断言随刻意变更 9→10）。锚册 v1.45 联动（37 名新增 + repl-form-cron 转正零改名 + 零退役）。e2e 新面 t462-cron-backups（六腿）+ t404 / repo-policy-keys 随刻意变更 + 回归面全绿。证据：`reports/agents/T-462.md` + `web/e2e/m16/t462-cron-backups.spec.ts` |
| v1.13 | 2026-09-05 | **T-461 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B13 承载——FR-147.3 落地，**§12 新增 B-3.20 行**〔remote 远端浏览可选档——表单/详情/树消费三面 as-built；审计期 47 项表外新增行，FR-147 在 v1.5 B 表冻结时未立项——票内补行登记非翻案〕）：**表单面** = Advanced 步复选「列出远端目录条目（listRemoteFolderItems）」〔off 默认 diff=0；显式 false 恒提交 flip-off 过 round trip；**呈现域 = BinFlow 引擎批 1 型 helm/debian/rpm**——与 Artifactory 官方开放面〔deb/generic/maven/Opkg/rpm 五型，remote-browsing.md §2〕刻意不同：generic/maven 的 HTML 目录抓取 BinFlow 明确不做〔§6 取舍基线〕不建禁用占位，helm 为 BinFlow L2 增强超越官方面；差异如实登记不追随〕；**详情面** = repo-remote-card 回显行；**树消费面** = on 时 listing 并入 display-only 派生行〔T-442 引擎〕+ 点击回源〔item-info GET 即 pull-through〕+ ?stats 计数联动〔T-438 单源〕+ virtual 含 remote 成员行〔§8.5 扩面〕——呈现形态 = BinFlow e2e 定案〔规格 §7 R-a：开档形态官方单源无活体——t226 Pro 建仓门结构性不可达，留痕在案〕；**降级** = listing 不整树塌 + 派生行点击错误态 + wire note（FolderInfo.remoteDegraded）FE 侧已消费〔**BE httpapi 渲染腿在途——契约缺口登记：T-448 §5-2 缝（storage.go 三个 List 调用点各一行 + 响应体一字段）**〕。wire 腿 = D-T456-1 修复端到端实证（PUT→GET echo 通）。锚册 v1.44 联动（6 名新增 + 零改名 + 零退役）。e2e 新面 remote-browse-tree（六腿）+ 回归面全绿〔两条共享红除外——工作树并行票在途足迹，基线对照定谳非本票回归，票内留痕〕。证据：`reports/agents/T-461.md` + `web/e2e/m16/remote-browse-tree.spec.ts` |
| v1.12 | 2026-09-05 | **T-459 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B12 承载——FR-145.5/.6b 落地，parity B-1.11 + B-2.18 翻正）。§12 两行翻「已落」：**B-1.11**（监控组六页——服务状态〔GET /api/v1/health + /api/system/version + /api/v1/system/schedules 三现役读面零新端点；AC4「对位既有 metrics/health 端点」以 health 族兑现——根级 /metrics 是 Prometheus 抓取面〔Basic-auth 族 cookie 结构性不达〕console 不消费，§3.6.2 /healthz 同款口径〕+ 系统日志〔载体票内定案：日志源 = 审计跟踪——服务进程日志〔slog 文件〕无 REST 端点〔internal/httpapi 全量无日志尾随面〕，**契约漂移登记 + BE 建议票**〔日志尾随/下载端点族〕；7.161 System Logs Viewer 三选择器〔Service/Node/LogFile〕如实降为单源说明行 logs-source 不伪造；尾随 7s 倒计时 + Pause/Refresh now〔探针 reports/agents/t459-probe/ 逐字对位〕；过滤 = 窗口客户端子串〔服务端精过滤仍在审计页〕；下载 = 当前窗口 Blob 导出〔7.161 走 Support Zone——BinFlow 无对位服务如实换形〕〕+ 系统信息归位〔/admin/monitoring/system-info，settings-* 冻结锚零改名；健康卡保留为实例信息摘要〕+ 维护（GC）/备份恢复自治理组迁入〔服务节点挂靠，T-462 cron 消费面同场〕）；**B-2.18**（Webhooks 归常规组 /admin/general/webhooks；管理侧栏 16→18 条目〔监控 6/治理 4/常规 2/仓库 1/用户与权限 5〕；五路由迁址 + 四旧深链 replace 折入一轮〔T-434 ?focus= 同款纪律〕；认证组子项形态——7.161.20 活体 Authentication 组 = LDAP/SAML SSO/OAuth SSO/HTTP SSO/Crowd·JIRA/SCIM 六子项〔t459-probe s4-sidebar-groups 实据〕，HTTP SSO/Crowd·JIRA/SCIM 三域不存在（FR-92 闭集 = LDAP/OAuth/SAML）——单页三页签维持缺位登记不伪造；**侧栏 Search Admin Resources 过滤框位置勘误**：7.161.20 活体 = 管理态**顶栏** 340px 单框〔A1-5 / t459-probe s3-admin-filter：x=1156 y=48 w=340〕，B-2.18 原始审计词「无侧栏过滤」系 7.84 经典口径——BinFlow 按活体落管理态顶栏（制品搜索让位单框），过滤语义〔客户端子串窄化侧栏条目 + 整组隐藏 + 无匹配注记〕按经典行为承载——**活体差异**：7.161.20 实测输入/Enter 均无可观测过滤效果〔t459-probe s3/s5〕，BinFlow 过滤真生效系对 7.84 语义的回归，差异留痕）。锚册 v1.43 联动（24 名新增 + 五族迁址零改名 + 零退役）。e2e：新 spec t459-monitoring-nav（六腿：health/version/schedules 三面对账 + 行对账/尾随/停拍/过滤/下载 + 分组断言 + 旧深链折入 + 过滤框全链 + axe 双主题）+ 既有 spec 路由翻新 ×8（m8 shell/governance-monitoring/a11y-sweep + security/auth-shell/governance/storage_migration + m13 两 spec）。证据：`reports/agents/T-459.md` + `web/e2e/m16/t459-monitoring-nav.spec.ts` |
...
| v1.6 | 2026-09-03 | **T-441 as-built 翻转③留痕**（dev-frontend 执笔，M16-SPLIT §1.2 AC1 承载——FR-143.3 / B-2.6 翻正落地；翻案依据 = PRD FR-143.3 + 7.161.20 活体勘误，非 ux 自创）。**M1 行再修订**（B-2.6 行 v1.5 预告的「M1 行再修订留痕归 T-441 落地时」兑现）：① **宽度档翻转③**——v1.4 定案 440px 紧凑档按 7.161.20 活体实测勘误翻至 **924px 居中档**（`min(924px, calc(100vw - 48px))` + MUI `maxWidth={false}` 解 600px 钳制；7.84 锚 880px → 7.161 实测 924px，证据 `reports/agents/m16-baseline-refresh.md` §A3-7）；② **八门控型磁贴开禁**（go/nuget/cargo/conan/helm/helmoci/rpm/debian）——**纯前端门开禁**：license 门系后端 ADR-0033 域（repo.Service D3 建仓 400 终裁），前端只保留 pkg-tier-* 档位徽章（D5 可见性口径不变）；门控三件套（disabled/mono/opacity 0.4）随 CSS 规则退役；③ 型录维持 BinFlow 实有 13 型（不伪造 7.161 的 41 型）；④ **对比度新义务收口**：开禁后磁贴进 WCAG 义务面，raw info 主色在 surface-2 上 4.48:1——磁贴内徽章文字沿语义徽章家族「78% 语义色 + 22% 正文色」收敛配方（light ≥5.4:1）。B-2.6 归属行同步翻「已落」。证据：`reports/agents/T-441.md` + `web/e2e/m16/t441-pkg-modal-open.spec.ts` |
| v1.7 | 2026-09-03 | **T-443 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 AC1/AC2 承载——FR-143.4/.5 落地）。§12 四行翻「已落」：**B-3.8**（入口 = 「＋ 新建仓库」下拉三预选〔型名+一句描述——7.161.20 s3e-create-dropdown 证据：Local "Upload and resolve your own packages" 族〕→ `/admin/repositories/<rclass>/new` 三静态路由 + `/new`+`?rclass=` 兼容映射 replace〔7 处跨页 emitter 零改动〕+ 非法段 404 + 表单内 rclass 控件移除〔`form-rclass-*` 三锚退役，锚册 v1.35〕）；**B-3.7**（dirty-gating：基线 = GET 回显预填同一对象 deep-equal，进入 disabled/变更 enabled/改回再 disabled——无变更提交不可达）；**B-3.6**（remote Test FE 三臂：成功绿/凭据被拒红 message 原文/不可达红 status_code 0——消费 T-442 端点；落 Basic 步来源节〔§A3-8 实测同位〕；仅编辑态在场〔端点按已存 key 寻址〕；草稿臂 = url/username/password 对基线 diff〔带密码=明文凭据对/仅改 url 或 username=匿名探测/零改动=已存配置探测〕）；**B-3.9**（类型列收敛〔Q9——`repos-columns-item-type` 退役〕+ **Remote 页签 Replications 列本票新增**——v1.5 预归属行「已随 T-404 落」系**对账勘误**〔T-404 实作仅 local Tab 且其 spec 有「remote 无列」空洞断言，本票翻转〕+ push-only 口径注记 ADR-0021/R10 + Project/Shared With/Environment 缺位登记不伪造）。B-3.10（行操作超集豁免）AC3 零倒退断言钉在 `web/e2e/m16/t443-list-entry-dirty-test.spec.ts`。证据：`reports/agents/T-443.md` + 同 spec |
| v1.8 | 2026-09-03 | **T-447 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B6 承载——FR-144.4/.5 落地）。§12 两行翻「已落」：**B-2.9**（属性编辑解剖 = **常显 Property/Value 输入 + Add + 网格搜索**——7.161.20 活体实证〔conductor :8082，example-repo-local Properties 页签：placeholder 逐字 "Property name"/"Property value" + Add Property 钮 + 网格 Search 输入 + 行选网格 + Property\|Property Set 分段钮〕；隐藏「+ 新增属性」旗标表单与逐行 ✎/🗑 编辑行退役〔锚册 v1.37 四名入退役表〕；同名键 Add = 值集整体替换〔§11.40，replaceHint 语义可见〕；**行内删除与 E1 统一（Q2 出口①）**——删除钮保留在行内但过 ConfirmDialog 危险确认、可拒绝，T-291 期「轻交互无确认」姿态退役；**Property\|Property Set 分段 = K68 候裁臂不建不做缺位登记**〔Property Set 须 BE 属性集小域扩列另立票，裁做时再入册〕）；**B-2.12**（下载形态 = **单 24px 图标钮**〔直接下载，浏览器原生落盘锚点；两带文字按钮「下载并校验/直接下载」收敛〕+ **伴随菜单**〔Q9「校验块收进伴随形态」：verify 动作 + 结果块 + checksums〔sha256/sha1/md5 截断+复制+上传时提供徽标〕+ mimeType 全部收进 Popover——General 页平铺退役；t447 spec 反断言钉死〕+ **下载动作计数联动**〔T-438 埋点单源：校验下载完成触发 ?stats 重读、UI 计数随增；直接下载无 JS 完成回调不触发、服务端内容面计数照落——零 FE 第二通道〕）。§9A 候裁子项「mimeType 与校验徽标块」随之清账（PM 倾向出口兑现）。证据：`reports/agents/T-447.md` + `web/e2e/m16/t447-props-download.spec.ts` |
| v1.11 | 2026-09-04 | **T-453 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B9 承载——FR-145.1/.3 落地，**断言反转④兑现〔Q5 出口①——T-437 联动留痕：本票即 v1.5 E5 注销行的落地票〕**）。§9 E5 行 + §3 M3 三行（Artifactory 行为 7.161.20 增补 / BinFlow 载体 / 差距）+ §12 B-2.15 行翻「已落」：**三路由** `/admin/security/users/new` + `/admin/security/groups/new` + `/admin/security/groups/:name/edit`（组编辑随内联卡创建/编辑同卡一并路由化——BinFlow 内部形态统一：用户/组/权限三域创建 + 编辑全路由页）；**内联展开卡退役**（列表页 DOM 零表单卡——断言反转④；users-create/groups-create 锚保持为导航入口）；**页脚按 7.161.20 活体**（2026-09-04 探针 evidence reports/agents/t453-probe/）：Cancel｜Reset〔**初始禁置**——dirty 门；用户/组表单**保留 Reset**，与 T-439 建仓表单移除重置钮的 Q9 处置为**页面级差异化配置**，差异留痕〕｜Save〔初始禁置——必填门〕；**用户创建表单补 Retype**（`user-form-password2` 复用编辑页锚——7.161 活体在场，T-384 期「创建态无 Retype」定案翻案）；**能力位三旗 = 预留位**（Can Update Profile / Disable UI Access / Disable Internal Password——BE create〔PUT userCreateBody〕与部分更新〔POST userUpdateBody〕均未承接三域：decode 位静默吞咽、GET 回显恒出厂档、零行为联动；T-439 预留位两档纪律：恒禁用 + 零提交 + hint 如实标注〔登录被拒臂/密码改道臂语义进 hint〕+ **API 漂移钉 tripwire**〔t453 spec：PUT 携带三域 201 而 GET 回显不动——BE 承接落地日本腿翻红即提示转正〕）；**管理位双布尔 vs 枚举 = 候裁臂**（7.161 = Administer Platform + Manage Resources 双布尔 + Platform Auditor / Manage Webhook；BinFlow 按 ADR-0026 暂行维持三值枚举 + `user-form-role-parity-note` 差异登记附注——双布尔不建不伪造）；组表单字段域裁定维持（External ID / 组级管理位 / Auto Join = BinFlow 组模型无对位域，不建不做缺位登记）。锚册 v1.40 联动（9 名新增 + 载体迁移零改名 + 零退役）。e2e：新 spec t453-route-forms（三腿：深链+能力位三件套 + 闭环/退役断言 + readonly 深链 + axe 双主题）+ 既有 spec 翻新 ×5（m8 users-groups / m9 users-groups〔编辑器数据面改路由页口径——E5+E2+targets 三请求、零逐用户扇出维持〕/ m14 t384〔断言反转④改写〕/ security / m8 a11y-sweep〔+2 路由〕）+ rbac 零改动核验。证据：`reports/agents/T-453.md` + `web/e2e/m16/t453-route-forms.spec.ts` |
| v1.10 | 2026-09-04 | **T-451 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B8 承载——FR-144.7 / LC-98 落地，**E2 翻案兑现〔Q4 出口①——T-437 联动留痕：本票即 v1.5 翻案行的落地票〕**）。§9 E2 行 + §5 L4 差距行 + §12 B-3.3 行翻「已落」：**共享分页组件**（web/src/components/Pager.tsx——§11.2 冻结形态逐项：页码序列〔MUI Pagination，当前页 aria-current 高亮〕+ 首/上一页/下一页/末页钮 + 每页行数选择器；单页全量控件整体呈现全链禁置——禁置不隐藏）+ **「加载更多」族票内 grep 盘点 = 9 面**（B-3.3 实测基数一致）：**8 面迁移**（搜索基本模式〔客户端页窗〕/ AQL 模式〔页码 → .offset() 尾缀重写、每页行数 → .limit() 重写，查询文本仍唯一事实源〕/ 审计〔keyset 游标链页窗——页码 N = 第 N-1 跳；语义 C 注兑现〕/ repos/users/groups/perms/tokens 计数行升级）+ **1 面分治豁免**（制品树 children 表 tree-load-more 维持增量——L4 分治口径，断言锚 = e2e/artifacts.spec.ts 既有腿加注）；**每页行数档位票内冻结 = [20, 50, 100, 200, 1000]**（缺省 100 = docker n / audit 缺省同源；1000 = AQL 引擎上限——7.161.20 活体复核：管理列表〔ag-grid〕分页 selector 槽在场未启用〔pageSizeComp 空〕、单页全量面无档位可证，**非逐字对位系 BinFlow 自有冻结**——形态漂移观察留痕：7.161 管理列表实为「1 to N of M + Page x of y」ag-grid 翻页器、无页码序列，形态锚仍以 7.84〔t226〕页码控件实测为准，版本线差异候 ux 复核，不动契约）；**流式末页语义**（keyset/AQL total = 本页行数非全量计数 → 页数 = 前沿 + 1 逐页揭示、末页钮 title 注记——呈现对齐语义自有）。锚册 v1.39 联动（pager 族 9 名 + audit-pager；audit-more/search-more/search-aql-prev/search-aql-next 四名退役）。e2e：新 spec t451-pager-unification（四腿：页窗跳转/档位/边界禁置 + 单页全链禁置 + AQL offset/limit 重写 + axe 双主题）+ governance 审计腿窗口化改写 + t419 分页腿翻新 + 计数行文案 ×5。证据：`reports/agents/T-451.md` + `web/e2e/m16/t451-pager-unification.spec.ts` |
| v1.9 | 2026-09-04 | **T-449 as-built 留痕**（dev-frontend 执笔，M16-SPLIT §1.2 B7 承载——FR-144.6 落地，**断言反转②归一**）。§12 六行翻「已落」：**B-2.11**（结果列集三源归一 = 选择列〔固定〕+ Artifact〔name 链接〕\|Path\|Repository\|Modified 默认在场，大小/sha256 移列选器**可选项不默认呈现**〔useColumnPrefs 扩 defaultHidden 第三参，缺省 [] = 既有四页零变化；reset = 恢复默认列集非全选〕）；**B-2.13**（查询位置 = 顶栏驻留〔Enter → /search?q=，/search 上 replace 接替页内 replaceState 写回环；驻留回显 = location.key + 草稿双层派生态〕+ 网格内快滤〔name/dir/repo 客户端子串——SR-01 全量返回，客户端窄化即全量语义〕；页内关键词/仓库过滤输入退役〔锚册 v1.38 五名入退役表〕，?repos= 深链退役不猝死；**AQL 编辑器共存形态票内定案**：编辑器 = AQL 服务端查询面 + 顶栏全局入口 + 快滤两层正交〔编辑器重放不清快滤〕）；**B-2.14/B-3.16**（快搜空历史占位恒渲染——「暂无最近搜索」对位 "No recent searches yet"〔7.161.20 活体复证 + 紧凑形态更近新基线的 A4-5 裁定〕；清除钮复役、零历史不渲染〔零死控件〕；Esc 两段含占位态——preventDefault 阻断 input[type=search] 原生清空侧效应）；**B-3.14**（行体 inert，深链唯一载体 = name 单元格 `search-result-link-<i>`〔href 属性级断言〕；**?focus= 发射端翻新**——SearchPage/AqlPanel/DashboardPage 改发 K67-3 路径段规范形，T-434 兼容重定向维持一轮〔B-1.3 遗留归位〕）；**B-3.15 结果表腿**（modified = `dd-MM-yy HH:mm:ss +ZZZZ`——formatStamp 两模式单源，浏览器本地时区 + 显式偏移，spec 正则断言）。AQL 行复用锚〔search-result-\<i\>/search-count/search-aql-range/sort 族〕全数保持（t419 零断链——spec 随列框架收敛翻新 ×7）。证据：`reports/agents/T-449.md` + `web/e2e/m16/t449-search-stack.spec.ts` |
...
| | |
|---|---|
| Artifactory 行为（**V2 已核验 2026-08-31，逐项实测**；**宽度档 7.161.20 勘误 2026-09-03**） | ① 入口：仓库列表页 **「Create a Repository」下拉**（7.161 实测文案；7.84 为「Add Repositories」，Local / Remote / Virtual 三项带一句描述）——rclass 在进向导前选定，**不在 modal 内以 Tab/分段控件出现**；② 「Select Package Type」**modal 网格**：宽 **924px** 居中（7.161.20 实测 924×760 居中 el-dialog——7.84 锚 880px 勘误，m16-baseline-refresh §A3-7）、右上 X，包型磁贴（inline SVG 官方标；enterprise license 下**全可用、零禁用态**）；③ 选磁贴后 **modal 关闭、落在整页路由表单**（`/ui/admin/repositories/local/new`，标题 "New Local Repository"）——**不是单 modal 全程**；④ 表单字段：Repository Key / Environments(Stage) / Repository Layout / Public Description / Internal Description + 折叠高级节（Disable Artifact Resolution… / Allow Content Browsing / Enable Event Replication 等复选项）；⑤ 页脚：**Cancel（左，transparent）+ Create Local Repository（右，primary）**。 |
| MUI 映射 | 网格步 `Dialog`（**v1.6 翻转③：924px 居中档**——paper sx `min(924px, calc(100vw - 48px))` + `maxWidth={false}` 解 MUI 'sm'〔600px〕钳制；v1.4 的 440px 紧凑档定案被 7.161 实测勘误翻平，见差距行留痕）+ 表单步**路由页**分节 `Paper` |
| BinFlow 载体 | `web/src/pages/repositories/RepositoryFormPage.tsx`——已是两段式：`pkg-grid` Dialog（选择包类型，带 `pkg-tier-*` 档位徽章；**T-441 开禁后形态**：`.pkg-grid-item` 卡面 + hover 生效，**全磁贴恒 brand 图标 22px + enabled**——八门控型的 disabled/mono/opacity 0.4 三件套随前端槽位门退役，徽章文字色沿语义徽章家族收敛配方保 surface-2 上对比度；**Dialog 宽 924px 居中**）→ **路由页** `/admin/repositories/new` 三段步进表单（T-439）；rclass 由 URL query 预选 + 单选组。 |
| 差距 | ✅ 已有（**v1.1 核验改判：决策项 A 撤销**——7.84 实测即「网格 modal → 整页表单」两段式，BinFlow 现形态与 Artifactory 一致，无需收单 Dialog）。保留微差注记：Artifactory 的 rclass 由入口下拉选定，BinFlow 用 URL query + 页内单选组——手势等价，不改。网格 modal 宽度**v1.4 定案 440px 紧凑档**（33 包型 880px 网格 vs BinFlow 5 核心 + 8 门控槽位，追平即大面积留白；T-383 票内定案 + e2e 断言 `toBeCloseTo(min(440, vw-48))`）——**v1.6 翻转③（T-441 as-built，FR-143.3/B-2.6）**：按 7.161.20 活体实测勘误（924×760 居中）翻至 **924px 居中档**，断言改 `toBeCloseTo(min(924, vw-48))` + 视口居中腿；v1.4「追平即留白」前提随 7.161 磁贴形态复核失效（磁贴数量与密度都是实有口径，留白可控），翻案依据 = M16-SPLIT §1.2 AC1，**非默认推翻**。**八门控型磁贴开禁（v1.6）**：license 槽位的前端禁用态整族退役——Artifactory enterprise 形态「零禁用 tile」的对位；license 门系后端 ADR-0033 域（repo.Service D3 建仓 400 终裁，community 实例提交时原文回显 form-error）。**v1.5 注（T-439 as-built，B-3.11/Q9 兑现 + B-2.5 三段）**：表单已升级 **Basic \| Advanced \| Replications 三段步进条**（Artifactory 7.161.20 :8082 活体复核——jf-steps 三步条形态证实，页脚 = Cancel + Create 两钮**无 Reset**）；BinFlow footer 重置钮随票移除（页脚两钮对齐 M1 锚点）；复制配置内嵌节迁第三步（M6 语义零变化）。 |
...
| | |
|---|---|
| Artifactory 行为（置信度：**高**；**V6 已核验 2026-08-31，实测修正**） | 用户/组**创建 = 整页路由表单**（`/ui/admin/management/users/new`、`/groups/new`），**不是 modal**——用户表单字段：User Name / Email Address / Password / Retype Password + 复选项（Administer Platform / Manage Resources / Can Update Profile / Disable UI Access / Disable Internal Password）；组表单：Group Name / Description / External ID + 复选项 + 成员用户选择列表；页脚 **Cancel（最左）/ Reset / Save（右）**。Token 面：**用户 profile 页**（`/ui/user_profile`）的 "Authentication Settings → Generate an Identity Token" 区 + **Identity Tokens 表**（Description / Token ID / Issued At / Expiry Date，带列选器）；admin 侧集中 token 管理页在 OSS 7.84 导航不可达（可能为 Pro 门）。**7.161.20 复核增补（2026-09-04 活体探针——reports/agents/t453-probe/ + m16-baseline-refresh §A4-2）**：用户表单另有 Platform Auditor / Manage Webhook 管理位 + MFA 区 + Related Groups 双列 + User Permissions 矩阵；**Reset 与 Save 初始均禁置**（Reset dirty 门 / Save 必填门）；组表单复选项 = Platform Administrator / Manage Resources / Manage Webhook / Automatically Join New Users to this Group；能力位三旗默认档 = Can Update Profile 勾选、其余两旗不勾。 |
...
| BinFlow 载体 | 用户：`UsersPage`（列表）+ **`UserCreatePage`（/admin/security/users/new 路由创建——T-453）** + `UserDetailPage`（路由页编辑）；组：`GroupsPage`（列表）+ **`GroupFormPage`（/admin/security/groups/new 创建 + /groups/:name/edit 编辑两态路由页——T-453，内联卡退役）**；权限：`PermissionEditorPage`（路由页——BinFlow 自有的模式测试器/diff 确认是增强，**豁免**）；Tokens：`TokensPage`（M14 真身）。 |
| 差距 | ✅ 形态已对齐（**v1.1 核验改判：决策项 B 撤销**——7.84 用户/组创建即整页路由表单，BinFlow 无需 modal 化）。**v1.5 勘误（Q5，2026-09-02 终裁路由化）**：「列表页内建与路由页同档形态，可保持」的 BinFlow 侧前提不实——用户/组创建实为**列表页内联展开卡**（M16 审计 B-2.15），E5 豁免前提失效（§9 E5 注销）。**v1.11 已落（T-453，断言反转④）**：`/users/new`、`/groups/new` 深链整页路由表单 + 内联卡退役断言 + 组编辑路由化（`/groups/:name/edit`）；页脚按 7.161.20 活体（2026-09-04 复核）：Cancel｜**Reset〔初始禁置——与 T-439 建仓表单移除重置钮的 Q9 处置为页面级差异化配置，差异留痕〕**｜Save〔初始禁置〕；用户表单补 Retype（T-384 期缺位定案翻案）；能力位三旗 = 预留位（BE create/partial-update 均未承接——恒禁用零提交 + API 漂移钉 tripwire，T-439 两档纪律）；管理位双布尔（Administer Platform + Manage Resources）vs 三值枚举 = 候裁臂（ADR-0026 暂行维持枚举 + `user-form-role-parity-note` 附注，双布尔不建不伪造）。Tokens 页真身 M14 已落（T-386）。 |
...
| 差距 | ~~✅ 已有（有意偏离：keyset 分页对页码不友好，BinFlow 形态保留；豁免登记 §9）~~ **v1.5 翻案（Q4，2026-09-02 用户终裁，出口①翻正）**：E2 豁免注销——**分治口径 = 「管理列表/结果表用页码控件，制品树/大目录深浏览维持增量加载」**（Artifactory 同为树增量 + 表页码双轨）。**v1.10 已落（T-451）**：共享 Pager + 「加载更多」族盘点 9 面〔8 迁移 + tree-load-more 分治豁免〕；后端维持 keyset 游标 + 页窗映射（呈现对齐、语义自有 C 注）；console-ux §6 条款回写随票落笔（锚册 v1.39）。 |
...
| BinFlow 载体 | ~~无用户级 cron~~ **Q1 翻案后现值（v1.14 已落）**：cron 双轨——021 台账 domain=replication 行到点 TriggerFullSync（全量对账），事件轨（上传 hook 入队）+ sweep 兜底仍是增量唯一引擎（同制品不双推，L48）；FE 面 = 表单 cronExp 真字段 + 治理页调度列（T-450 BE / T-462 FE）。 |
...
| E2 | ~~「加载更多」增量分页（非页码控件）~~ **v1.5 翻案改写（Q4，2026-09-02 用户终裁，出口①翻正）——豁免注销** | **页码控件 ×9 处统一**：FR-144.7 / LC-98 承载（T-451 共享分页组件 + ×9 消费点迁移——消费点清单票内 grep 盘点入锚册，实际多于 9 以盘点为准）。**v1.10 已落（T-451，E2 翻案落地票——T-437 联动留痕）**：共享 Pager〔§11.2 形态锚〕+ 盘点 9 面 = 8 迁移〔搜索两模式/审计/repos/users/groups/perms/tokens〕+ 1 分治豁免〔tree-load-more〕；**语义自有 C 注兑现**：后端维持 keyset 游标 + 页窗映射（呈现对齐、深翻页 offset 扫描成本规避）——审计页码 N = 游标链第 N-1 跳、AQL 页码 = .offset() 尾缀重写；console-ux §6 条款回写已随票落笔（锚册 v1.39） |
...
| E5 | ~~用户/组创建与编辑均保留页面形态~~ **v1.5 注销（Q5，2026-09-02 终裁路由化）——豁免条目除名** | **前提失效**：v1.1 改写所依据的「BinFlow 已用路由实体表单」被 M16 审计推翻（用户/组创建实为列表页内联展开卡——B-2.15；V6 只核了 Artifactory 侧）。Q5 出口①：`/users/new`、`/groups/new` 整页路由表单（FR-145.1 / T-453——断言反转④），内联卡退役。**v1.11 已落（T-453）**：三路由 `/admin/security/users/new` + `/admin/security/groups/new` + `/admin/security/groups/:name/edit`（组编辑随内联卡创建/编辑同卡一并路由化）+ 页脚 Cancel｜Reset〔初始禁置〕｜Save〔初始禁置〕（7.161.20 活体复核 2026-09-04——reports/agents/t453-probe/）+ 内联卡退役断言（列表页 DOM 零表单卡）；**路由化兑现后该面为对齐项非偏离项**（Artifactory 创建即路由表单——V6）；本条目自豁免清单除名留档，M3 勘误同场（§3） |
...
| S8 | 无数据源不伪造族：Module ID 字段 / Any Distribution 预置 / Project·Environment·Shared With 列 / Realm 列 | **部分解禁**（Any Distribution 预置 T-514 已落；其余维持缺位登记） | Release Bundle 域 M17 落地（T-513 BE + T-514 FE）：**Any Distribution 预置已随 B-2.16 补齐**（权限两步弹窗三预置桶同场——缺位解除归位）；Module ID 字段 dep Build-info 域（M17 在途）；Project·Environment·Shared With 列 / Realm 列维持缺位（PRD §1.4 条款 2 / §0.3——Projects 域面外；Environment 列与 FR-143.2 Environments 字域联动，缺位不伪造） |

**候裁子项（非 stay-out——Q9 裁点挂起，批次②③断言冻结前裁，裁毕回写本表）**：表单 footer 重置钮（PM 倾向移除——T-439 兑现 + M1 行联动留痕）/ mimeType 与校验徽标块（PM 倾向收进下载伴随——**T-447 已兑现**：校验块/mimeType 收进下载伴随菜单，General 页平铺退役〔v1.8〕）/ 详情「类型/子项/修改时间」列去留（T-445 票内——维持自有）/ 组·权限列表行内「编辑」钮超集（L2/E1 家族延伸——V4 实证 Artifactory 行尾仅 trash，候 Q9 复核收敛或维持）。
...
| 断言面 | 冻结形态 | 置信度 |
|---|---|---|
| 控件品种 | 页码数字序列（当前页高亮）+ 首/上一页/下一页/末页按钮 + 每页行数选择器 | 形态**高**（Artifactory 管理列表通行形态 + t226 列表实测页码控件在场——L4 v1.1 中置信升格）；**每页行数档位 T-451 票内冻结 = [20, 50, 100, 200, 1000]**（缺省 100；7.161.20 活体复核管理列表 selector 槽在场未启用、无逐字对位——BinFlow 自有冻结留痕。**形态漂移观察〔v1.10〕**：7.161 管理列表〔ag-grid〕翻页器实为「1 to N of M + Page x of y」无页码序列——形态锚以 7.84〔t226〕实测为准不动契约，版本线差异候 ux 复核） |
| 边界态 | 首页时「首页/上一页」禁置、末页时「下一页/末页」禁置；单页全量时控件整体呈现、全链禁置——**禁置不隐藏**（P7 键盘可达） | 高（本册族通用规格；T-451 已落——t451 spec 断言） |
| 语义 C 注 | 后端维持 keyset 游标 + **页窗映射**（页码跳转 = 前端游标链推进，深翻页 offset 扫描成本规避）；呈现对齐、语义自有——LC-98 C 注留痕 | 定案（Q4 终裁附带；**T-451 已落**——审计游标链页窗 + AQL offset 重写 + 流式末页〔页数 = 前沿+1 逐页揭示，末页钮 title 注记〕） |
| 适用范围 | 管理列表 + 结果表（×9 消费点）；**制品树/大目录深浏览维持增量加载**（分治口径见 §5 L4 v1.5 注——Artifactory 同为双轨） | 定案（T-451 已落——分治豁免面 tree-load-more 断言锚 = e2e/artifacts.spec.ts 既有腿） |
| 消费点清单 | **T-451 票内盘点已落（v1.10）**：grep「加载更多」族全量 = **9 面**（与 B-3.3 基数一致）——迁移 8 面〔搜索基本模式（客户端页窗）/ AQL 模式（.offset()+.limit() 重写）/ 审计（keyset 游标页窗）/ repos / users / groups / perms / tokens 计数行升级〕+ 分治豁免 1 面〔制品树 children 表 tree-load-more〕；明细入锚册 §10.5 T-451 批块 | 已盘点（T-451，锚册 v1.39） |
...
| B-1.3 | URL/状态模型（页签不进 URL + `?focus=`） | **翻正 · 已落** | T-434 done（K67-3——TAB 省略规范形）；`?focus=` **发射端**翻新已落 T-449 AC2（三发射端改发路径段深链，兼容重定向维持一轮） |
...
| B-1.6 | 权限动词集（无 Annotate；write 未拆 Deploy/Cache） | **翻正 · 已落（T-444 + T-455）** | T-444（BE 动词域 + 迁移零提权 NFR-S77）+ T-455 FE 消费面（LC-88）：编辑器/汇总矩阵五列 read/annotate/write/delete/manage（列序对位 7.161.20 活体 Repositories 资源型 Read/Annotate/Deploy-Cache/Delete-Overwrite/Manage——探针 reports/agents/t455-probe/；列头用 wire 正名单词形，7.161 标签进 title）；wire 双层收口（GET 正名单 deploy-cache / PUT 别名 write 收词，FE 水合归一 + 保存正名单形）；**联动裁定 = 独立位列零联动**（活体联动形态在 0 仓参照实例不可观测——矩阵门在资源型+行选定之后，登记不伪造；且 T-444 拆分语义 write 不携带 annotate〔零提权〕，UI 自动联动 = 越权授予 annotate，独立位列是唯一正确形态） |
| B-1.7 | 用户表单能力位（缺三旗；双布尔被三值枚举替） | **翻正** + 候裁臂 | T-453（三旗 A 腿 + 行为联动断言）；双布尔 vs 枚举 = **候裁挂起**（ADR-0026 闭集——v1.0 暂行维持枚举 + 差异登记） |
| B-1.8 | Profile 无自助 token/SSH（签发指到 admin 页） | **翻正 · 已落（T-457，token 腿）** + 缺位登记（SSH 腿） | as-built：① Identity Token 自助签发真身进 Profile（`profile-token` 卡升格 + `profile-token-*` 弹窗族 15 锚——console-ux v1.42 T-457 批块）；一次性明文〔关弹窗即不可再取——7.161 形态〕+ step-up 口令腿内联〔ADR-0027〕+ 即用 curl 样例；e2e 即时可用对账 = Bearer 臂 + curl `-u` Basic 双臂真发 API 请求（t457 spec 腿①②）；「指到 admin 页」推翻——`profile-token-goto` 收窄为管理面二级入口（台账/按 id 吊销/代人签发仍归 Access Tokens 页：E-18 吊销是 CapSecurityWrite 管理面写动作）；② SSH key 增删 **后端无端点——契约缺口不伪造**（internal/httpapi 全量无 SSH 面；console-ux §9-R11 登记 + Profile SSH 卡如实缺位 `profile-ssh-gap` 零表单——FE 摆不出没有的端点）；③ Access Tokens L1 列选器重评 = **A7 联动登记在案**（服务端令牌清单端点〔§9-R6〕落地 → 台账转持久列表 → L1 推广〔T-468 条件池同域〕——票内留痕不承载） |
| B-1.9 | GC 维护面（无 cron / Cleanup 两族 / Quota 百分比） | **翻正 · 已落（T-462）** | as-built：维护（GC）页〔/admin/monitoring/gc〕新增「定时维护（cron）」卡——三槽行表〔gc / cleanup-unused-cache / cleanup-virtual——wire 键〕×（表达式输入 + 保存/清除 + 下次/上次运行）+ **Run Now=既有手动面并存**〔gc 槽滚向危险区 dry-run/apply；两 cleanup 槽 POST /system/cleanup {apply:true}——T-324 手动面与调度 fire 同载体〕；Quota 百分比 / Compress / Prune 无 BinFlow 后端载体——gc-cron-gap 缺位注记不伪造；两 cleanup 槽同走全量 pass（无 virtual-only 载体——ADR-0044 决策 8① C 层差异，行内注记）（v1.14） |
| B-1.10 | 备份页（无定时 CRUD / import/export 页） | **翻正 · 已落（T-462）** | as-built：备份页〔/admin/monitoring/backup〕两卡——**定时备份 CRUD**〔New Backup 表单：Enabled/Backup Key/Cron Expression/Next Backup Time（datetime-local→RFC3339，过去 400）/Server Path；列表 Key/cron/下次备份/启用/上次运行/路径/操作；E1 输入 key 删除〕+ **导入/导出 CLI 卡**〔交互式 export/import 维持不做——ADR-0015 勘误②：/api/export/** 404 有意、import CLI-only；一次性 export/恢复命令块 + 定时备份同载体注记〕；§3.10 无载体字段（仓子集〔导出恒全实例快照〕/incremental/retention/zip/邮件告警/Browse 钮）缺位注记不伪造（v1.14） |
| B-1.11 | 监控组仅存储页（无 System Logs / Service Status） | **翻正 · 已落（T-459）** | as-built：监控组扩为**服务节点组六页**（存储/服务状态/系统日志/系统信息〔归位〕/维护（GC）/备份恢复〔迁入〕）；Service Status = health + version + schedules 三现役读面（零新端点；Uptime 无端点如实缺位）；System Logs = **载体票内定案：日志源 = 审计跟踪**（服务进程日志无 REST 端点——契约漂移登记 + BE 建议票；7.161 三选择器如实降为单源说明行）+ 尾随/过滤/下载三件（v1.12） |
...
| B-2.6 | 包类型弹窗（440px/13 tiles/8 禁用 vs 880px 全可用） | **翻正 · 已落（T-441）** | T-441（M1 行 v1.6 再修订留痕已兑现：**924px 居中**〔7.161 勘误，非 880——票面按勘误执行〕+ 八型开禁〔纯前端门；后端 license 门 ADR-0033 域独立，D3 终裁〕+ 门控三件套退役 + pkg-tier-* 徽章保留；型录维持 13 实有不伪造） |
...
| B-2.9 | 属性编辑解剖（隐藏表单 + 逐行 ✎/🗑 vs 常显输入 + Add + 网格搜索） | **翻正 · 已落（T-447）** + 候裁臂 | 常显 Property/Value + Add + 网格搜索已落（v1.8——7.161.20 活体实证同构）；行内删除过危险确认（E1 统一，Q2 出口①）；Property\|Property Set 分段 = 候裁维持（K68——裁做须 BE 属性集小域另立票，不做缺位登记） |
| B-2.10 | General 字段集（独立取证口径——同 B-2.3 主题） | **对账去重行** | 归 B-2.3 / T-445 |
| B-2.11 | 搜索结果列集（三源不一致：现行五列 vs T-414 注释 vs console-m8 §6.4） | **翻正 · 已落（T-449）** | 断言反转②归一：默认列 = 选择列（固定）+ Artifact（name 链接）\|Path\|Repository\|Modified；大小/sha256 = 列选器可选项不默认呈现（defaultHidden——共享层第三参，reset = 恢复默认列集）——v1.9 |
| B-2.12 | 下载形态（两带文字按钮 vs 单 24px 图标钮） | **翻正 · 已落（T-447）** | 单 24px 图标钮（直接下载）+ 伴随菜单收校验能力/checksums/mimeType（Q9 终裁兑现）+ 下载计数联动（T-438 埋点单源）——v1.8 |
| B-2.13 | 查询位置（页内输入 vs 顶栏驻留 + 网格快滤） | **翻正 · 已落（T-449）** | 查询面 = 顶栏驻留（Enter → /search?q=、/search 上 replace + 驻留回显；页内关键词/仓库过滤输入退役，?repos= 深链退役不猝死）+ 网格内快滤（name/dir/repo 客户端子串）；**AQL 编辑器共存形态**（票内设计）：编辑器 = AQL 模式服务端查询面，顶栏保持全局入口，快滤两层正交——v1.9 |
| B-2.14 | 快搜入口（253px 紧凑 vs 815px overlay + 范围页签） | **翻正 · 已落（T-449）** + stay-out 分拆 | 翻正腿已落：空历史「暂无最近搜索」占位恒渲染（对位 "No recent searches yet"，`topbar-search-recent-empty`；B-2.14 重写裁定 = BinFlow 253px 紧凑形态更近 7.161 内联展开基线——A4-5）；stay-out：范围页签缺位维持（§9A-S7——R2 既定 + Builds dep M17）——v1.9 |
| B-2.15 | 用户/组创建内联卡（与 E5/V6 决策前提矛盾 + BinFlow 内部不一致） | **翻正 · 已落（T-453）** | T-453（断言反转④——/users/new、/groups/new 深链整页表单 + 内联卡退役断言〔列表页 DOM 零表单卡〕+ 组编辑 `/groups/:name/edit` 同场路由化；E5 注销 §9 + M3 勘误 §3；能力位三旗预留位族 + Retype 补齐——v1.11） |
| B-2.16 | 权限编辑器内联形态（vs 两步弹窗） | **翻正 · 已落（主体 T-455；预置桶 T-514 补齐）** | T-455 as-built：① 选仓双列穿梭 + **可点双步头**（「1 选择仓库」/「2 设置模式（可选）」常驻可点直跳、aria-current 标当前步——7.161.20 活体 Add Repositories 弹窗同形〔探针 reports/agents/t455-probe/：1 Select Repositories / 2 Set Patterns (Optional)，footer Cancel/OK〕；既有 Next/上一步/perm-res-* 锚链零回退）→ ② Set Patterns 维持；**Any Local/Any Remote/Any Distribution 三预置桶 T-514 已落**（B-2.16 缺位解除——T-491 BE 承接通配桶语义后 FE 转正：预置行与具体仓库同场可勾选/混列，wire 字面 ANY LOCAL/ANY REMOTE/ANY DISTRIBUTION，展示名 = §3.8 活体拼写；m-holder 403 分支桶照常可勾——桶字面不依赖目录端点；授予面覆盖探针在 e2e t514〔Any Distribution 被授权三级钻取可见 + 未授予空集/404 同形〕；两处「通配桶不建」注记退役换语义注记 perm-buckets-note）；E4 本体维持 §9A-S1（编辑器整页 + 测试器 + diff 确认零删除断言承载）；活体另证：7.161.20 权限新建/编辑 = **三步向导路由页**（Resources/Users/Groups 面板，panel-* data-cy）——BinFlow 编辑器本体 stay-out，向导分步不建（差异留痕） |
| B-2.17 | 帮助钮（纯链接 vs ? 下拉 + About 弹窗） | **翻正 · 已落（T-457）** + 活体差异留痕 | as-built：? 下拉四项〔FR-145.6a 定案集——console-ux §3.5/v1.42〕：Documentation〔/binflow/docs/ 外链——ux 定案形态〕/ Online Training〔无对应服务：禁用占位 + 行内注记，不伪造外链〕/ Release Notes〔实链 /binflow/docs/install/upgrade「升级与版本说明」〕/ About〔版本弹窗〕；About 弹窗消费 /api/system/version 三值〔失败 = — 不伪装 Q4〕，双入口 = help-about + 侧栏脚注 vdev 行升格〔nav-about，nav-version 锚不动〕。**7.161.20 活体差异**（探针 reports/agents/t457-probe/）：平台菜单实为 JFrog Documentation / JFrog Academy / Navigation Tour 三项——无 About/Release Notes（经典 UI〔7.84 逆向〕成员）；四项集 = PRD 定案、Academy→Training 映射、Release Notes/About 为经典成员回填，差异不追随 |
| B-2.18 | Admin 导航分组（认证单页三页签 / Webhooks·维护·备份挂治理 / 无侧栏过滤） | **翻正 · 已落（T-459）** | as-built：Webhooks 归常规组（/admin/general/webhooks）；维护·备份·系统信息归监控组〔服务节点——与 T-462 cron 消费面同场挂靠〕；管理侧栏 16→18 条目 + 五路由迁址（旧深链一轮 replace 折入）；认证组子项形态——7.161.20 活体六子项中 HTTP SSO/Crowd·JIRA/SCIM 三域不存在（FR-92 闭集）缺位登记不伪造；Search Admin Resources 过滤框按 **7.161 活体位置勘误落管理态顶栏**（A1-5；活体无可观测过滤效果——过滤语义按 7.84 经典行为承载，差异留痕 v1.12）（v1.12） |
...
| B-3.3 | 空态/上限/分页（「（空）」占位 / CAP=300 / BIG_DIR=2000 / 表 100/页加载更多） | **翻正已落（T-451）** + 豁免·复核 | 翻正：E2→页码控件 ×9（Q4——T-451 已落，断言锚 §11.2 + 锚册 v1.39 pager 族）；豁免维持：TREE_LEVEL_CAP/BIG_DIR 自工程通知（无害自设计——K67-1 口径冻结） |
...
| B-3.6 | remote 表单无 Test 连通性（唯一 Test 在复制子表单） | **翻正 · 已落（T-442+T-443）** | T-442（BE 端点——Engine.TestTarget 同构复用；wire 归属〔歧义⑥〕票内与 conductor 核定落 LC）+ T-443（FE 三臂消费 + 零副作用）。as-built：Test 落 **Basic 步来源节凭据组旁**（7.161.20 实测同位，m16-baseline-refresh §A3-8）；仅编辑态在场（端点按已存 key 寻址），建仓态给 hint 不给死按钮 |
| B-3.7 | 编辑表单 Save 无 dirty-gating | **翻正 · 已落（T-443）** | T-443（进入 disabled / 变更 enabled / 无变更提交不可达）。as-built：基线 = GET 回显预填同一对象，deep-equal（键排序规范形）比较；密码基线恒空（NFR-S14 不回显）——输入即 dirty |
| B-3.8 | 创建入口拓扑（单「＋ 添加仓库」+ 表单内 rclass vs 下拉三预选分路由） | **翻正 · 已落（T-443）** | T-443（/new 直链兼容映射；表单内 rclass 控件移除）。as-built：入口 = 「＋ 新建仓库」下拉三预选（型名 + 一句描述——7.161.20 s3e-create-dropdown 证据）→ `/admin/repositories/<rclass>/new` 三静态路由；`/new`+`?rclass=` 兼容映射 replace（7 处跨页 emitter 零改动）；非法段落 404；`form-rclass-*` 三锚退役（锚册 v1.35 §10.6） |
| B-3.9 | 仓库列表列集（缺 Project/Environment/Shared With；Remote 页签缺 Replications 列；多出冗余「类型」列） | **翻正 · 已落（T-443）** + stay-out 注记 | T-443 as-built：冗余「类型」列收敛〔Q9——三 Tab 子路由即类型，列+列选项退役 `repos-columns-item-type`〕+ Remote 页签 Replications 列**本票新增**（v1.5 预归属行「已随 T-404 落」系**对账勘误**——T-404 实作仅 local Tab，其 spec 且有「remote 无列」空洞断言，本票翻转）；**push-only 口径注记 ADR-0021/R10**：BinFlow 无 pull 复制（remote 缓存另一能力域），remote 页签如实呈现以该仓为源的 push 配置（模型 FK 允许任意仓型作源；配置面 UI 自 T-439 仅 local 编辑态暴露——remote 通常 0，同 OSS「0」cell）；Project/Shared With 缺位登记不伪造（§9A-S8）；Environment 列 7.161 视口截断未定论——缺位同口径 |
...
| B-3.14 | 搜索行导航（整行可点 vs 仅 name 单元格深链） | **翻正 · 已落（T-449）** | 行体 inert（无 onClick/tabIndex）；深链唯一载体 = name 单元格链接（`search-result-link-<i>`，href = K67-3 路径段规范形）；?focus= 发射端全数翻新（SearchPage/AqlPanel/DashboardPage，兼容重定向维持一轮）——v1.9 |
| B-3.15 | 日期格式（结果表无时区偏移；详情裸 ISO） | **翻正 · 结果表腿已落（T-449）** | 结果表 modified = `dd-MM-yy HH:mm:ss +ZZZZ`（formatStamp 两模式单源，正则断言含时区偏移——v1.9）；详情腿 T-445（ISO 格式化——T/.000Z 不裸显）；en 变体归 T-464（locale 化，FR-149.4） |
| B-3.16 | 快搜空历史不渲染下拉 | **翻正 · 已落（T-449）** | 聚焦恒渲染下拉——空历史给「暂无最近搜索」占位（对位 "No recent searches yet"）、子串无匹配给无匹配占位；与 B-2.14 翻正腿同源——v1.9 |
...
| B-3.20 | remote 远端浏览可选档（List Remote Folder Items——Artifactory 官方开放 deb/generic/maven/Opkg/rpm 五型；开档形态官方单源无活体〔t226 Pro 建仓门〕） | **已落 · 呈现域自有**（T-442/T-448 + T-461） | **v1.13 新行**（FR-147 审计期 47 项表外——票内补行）。as-built：表单 Advanced 步复选〔off 默认 diff=0；显式 false 恒提交；**呈现域 = 引擎批 1 型 helm/debian/rpm**——与官方五型刻意不同：generic/maven 的 HTML 目录抓取明确不做〔remote-browsing.md §6〕不建禁用占位；helm = BinFlow L2 增强超越官方面，差异登记不追随〕+ 详情/编辑回显 + 树 on/off 双态〔display-only 派生行 + 点击回源 + ?stats 单源计数 + §8.5 virtual 扩面〕+ 上游停机降级〔缓存行不整树塌 / 派生行点击错误态 / wire note FolderInfo.remoteDegraded FE 已消费——BE httpapi note 渲染腿在途契约缺口登记（T-448 §5-2 缝：storage.go 三个 List 调用点各一行 + 响应体一字段）〕；开档呈现形态 = BinFlow e2e 定案（规格 §7 R-a——官方配图文字单源）；wire 腿 D-T456-1 端到端实证 |
