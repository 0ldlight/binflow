...
| B-2.16 | 权限编辑器内联形态（vs 两步弹窗） | **翻正 · 已落（主体 T-455；预置桶候裁）** | T-455 as-built：① 选仓双列穿梭 + **可点双步头**（「1 选择仓库」/「2 设置模式（可选）」常驻可点直跳、aria-current 标当前步——7.161.20 活体 Add Repositories 弹窗同形〔探针 reports/agents/t455-probe/：1 Select Repositories / 2 Set Patterns (Optional)，footer Cancel/OK〕；既有 Next/上一步/perm-res-* 锚链零回退）→ ② Set Patterns 维持；**Any Local/Any Remote 预置未建 = 契约缺口**（BinFlow wire repos[] 必须逐个指名现存仓库，服务端 400 unknown repository——预置桶需 BE 先承接通配桶语义；0 仓参照实例亦不可观测其呈现形态。候裁：BE 扩列承接 或 与 Any Distribution 同口径 §9A-S8 缺位登记）；E4 本体维持 §9A-S1（编辑器整页 + 测试器 + diff 确认零删除断言承载）；活体另证：7.161.20 权限新建/编辑 = **三步向导路由页**（Resources/Users/Groups 面板，panel-* data-cy）——BinFlow 编辑器本体 stay-out，向导分步不建（差异留痕） |
