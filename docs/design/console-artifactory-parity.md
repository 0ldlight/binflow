...
| 状态 | v1.11（2026-09-04，T-453 as-built 留痕：**断言反转④兑现〔Q5 出口①〕——§9 E5 + §3 M3 + §12 B-2.15 翻已落**：/users/new、/groups/new、/groups/:name/edit 三路由整页表单 + 列表内联展开卡退役〔断言反转④〕+ 页脚 Cancel｜Reset〔初始禁置·7.161 活体〕｜Save + 用户创建表单补 Retype + 能力位三旗预留位〔BE 未承接——恒禁用零提交 + API 漂移钉〕+ 管理位双布尔 vs 枚举候裁臂〔ADR-0026 暂行枚举 + 附注〕；此前 v1.10 T-451 as-built 留痕：**E2 翻案兑现〔Q4 出口①〕——§9 E2 + §5 L4 + §12 B-3.3 翻已落**：共享 Pager 分页组件〔§11.2 冻结形态〕+ 「加载更多」族盘点 9 面〔8 迁移 + 制品树分治豁免〕+ 档位 [20/50/100/200/1000] 票内冻结〔7.161.20 复核无逐字对位，自有冻结留痕〕+ keyset/AQL 页窗映射〔语义 C 注〕；此前 v1.9 T-449 as-built 留痕：**B-2.11/13/14 + B-3.14/15/16 六行翻正已落**——搜索栈断言反转②归一〔列集 = 选择列 + Artifact 链接\|Path\|Repository\|Modified，大小/sha256 列选器可选项〕+ 顶栏驻留查询 + 网格内快滤〔AQL 编辑器共存两层正交〕+ 快搜空历史占位恒渲染 + 行导航仅 name 深链〔?focus= 发射端翻新，B-1.3 遗留归位〕+ 结果表日期 +ZZZZ；此前 v1.8 T-447 as-built 留痕：**B-2.9/B-2.12 两行翻正已落**——属性编辑解剖常显〔Property/Value 输入 + Add + 网格搜索，7.161.20 活体实证 placeholder 逐字对齐；隐藏旗标表单与逐行 ✎/🗑 退役；行内删除过危险确认——E1 统一 Q2 出口①；Property\|Property Set 分段 = K68 候裁臂不建不做缺位登记〕+ 下载形态单 24px 图标钮〔两带文字按钮收敛；校验能力收进伴随菜单——Q9；mimeType/校验徽标块/下载校验块自 General 页随迁伴随；下载计数联动 T-438 埋点单源〕；此前 v1.7 T-443 as-built 留痕：**B-3.6/3.7/3.8/3.9 四行翻正已落**——入口下拉三预选分路由〔B-3.8〕+ 表单内 rclass 控件移除 + dirty-gating〔B-3.7〕+ remote Test 三臂 FE 消费〔B-3.6，T-442 端点〕+ 列表类型列收敛与 Remote 页签 Replications 列〔B-3.9，push-only 口径 ADR-0021/R10〕；此前 v1.6 T-441 as-built 翻转③留痕：M1 行 440px 紧凑档 → **924px 居中档**〔7.161.20 活体勘误〕+ 八门控型磁贴开禁〔纯前端门，后端 ADR-0033 域独立〕——B-2.6 翻正已落；此前 v1.5 T-437 M16 翻案修订：E5/E1/E2/E6/R4 五处翻案改写 + §9A stay-out 登记 + §11 K67 冻结 + §12 B 47 项四态预归属表——ux 主笔 + **PM 会签位**〔按 PRD v1.1 七裁定代笔〕；**版本对账：PRD/审计材料所称「parity 册 v1.2」即 v1.5**——实序为准，§0 留痕；此前 v1.4 T-428 文面回写 / v1.3 T-400 终评落档） |
| 维护者 | ux-designer |
