...
jf c add l023da --url=http://172.16.58.130:8082 --user=admin --password="${ARTIFACTORY_REF_PASSWORD:?set env}" --interactive=false >/dev/null 2>&1
jf c add l023db --url=http://172.16.58.130:8083 --user=admin --password="${BINFLOW_DEV_PASSWORD:-password}" --interactive=false >/dev/null 2>&1

echo "l0232d-jf-wheel-content-v1" > /tmp/l023d-jf-wheel.bin

leg() {  # leg <side: a|b> <logfile>
  local s=$1 log=$2 conn
  if [ "$s" = a ]; then conn="--server-id=l023da"; else conn="--url=http://172.16.58.130:8083/binflow --user=admin --password=${BINFLOW_DEV_PASSWORD:-password}"; fi
  {
...
curl -s -u "admin:${ARTIFACTORY_REF_PASSWORD:?set env}" http://172.16.58.130:8082/artifactory/api/build/l023d-jf-app | python3 -c "import json,sys; d=json.load(sys.stdin); print('A', [x['uri'] for x in d.get('buildsNumbers',[])])"
...
for s in a b; do
  if [ $s = a ]; then AUTH="admin:${ARTIFACTORY_REF_PASSWORD:?set env}"; H=http://172.16.58.130:8082/artifactory; else AUTH="admin:${ARTIFACTORY_REF_PASSWORD:?set env}"; H=http://172.16.58.130:8083/binflow; fi
